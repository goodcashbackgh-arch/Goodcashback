const UI_TEXT_REPLACEMENTS: Array<[RegExp, string]> = [
  [/\bMindee direction\b/gi, "Document direction"],
  [/\bMindee row\b/gi, "Document row"],
  [/\bMindee fields\b/gi, "document fields"],
  [/\bPDF OCR control\b/gi, "PDF statement extraction"],
  [/\bRun, fetch and parse statement OCR\b/gi, "Run document extraction and parse statement"],
  ...
  [/\bDVA\/card allocation\b/gi, "payment matching"],
  [/\bDVA\/card control hub\b/gi, "payment control hub"],
  ...
  [/\bposted shipper AP invoices\b/gi, "approved shipper charge records"],
