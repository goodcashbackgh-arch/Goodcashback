  ...
  WITH src AS (
    ...
    SELECT s.*, r.inv_type, r.inv_status, r.route_value, r.si_state,
      ...
      CASE WHEN r.inv_type='supplementary'
        THEN 'Supplementary export sale charge'
        ELSE COALESCE(NULLIF(s.item_description,''),'Goods')
      END::text AS sage_line_description
    ...
    SELECT COALESCE(SUM(l2.goods_evidence),0) goods_total, COALESCE(SUM(l2.shipping_evidence),0) shipping_total, COALESCE(SUM(l2.bundled_amount),0) invoice_total,
      jsonb_agg(jsonb_build_object(
        ...
        'tax_rate_id', 'GB_ZERO',
        'tax_rate_display', 'Zero Rated 0.00%',
        'display_vat_code', 'T0',
        'ledger_account_role', 'export_sale_income'
      ) ORDER BY l2.order_ref NULLS LAST, l2.booking_ref NULLS LAST, l2.sage_line_description NULLS LAST) payload
    FROM l l2
  )
