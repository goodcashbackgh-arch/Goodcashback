...
-- Fix: live-ready rows must not reappear if the same exact source row is already
-- locked inside a non-cancelled posting batch.
--
-- Previous live-ready guard only checked active frozen snapshots. After local
-- snapshot supersede/repair, a source already held by an older validated/draft
-- posting batch could reappear as live_ready_not_frozen.
--
-- This patch adds source-level posting-batch locks to BOTH:
-- 1) bulk candidate resolver
-- 2) accounting command centre grid
--
-- Exact-match only: source_table + source_id + document_lane.
-- This means a customer_sales £161.99 row locked in batch ...8859 is blocked,
-- but a separate supplier_goods_ap £161.99 row with a different source table/id
...
BEGIN
  ...
  WITH frozen_rows AS (
    SELECT
      ...
      CASE
        WHEN sq.posting_gate_status = 'ready_to_post' THEN 'Create lane posting batch later'
    ...
    WHERE (v_lane = 'all' OR ar.out_document_lane = v_lane)
      ...
      AND (
        v_search IS NULL
        OR lower(concat_ws(' ', ar.out_queue_row_id, ar.out_order_ref, ar.out_reference_text, ar.out_counterparty_name, ar.out_document_lane, ar.out_document_type, ar.out_batch_ref, ar.out_idempotency_key, ar.out_source_id::text)) LIKE ('%' || v_search || '%')
      )
  ), summary AS (
