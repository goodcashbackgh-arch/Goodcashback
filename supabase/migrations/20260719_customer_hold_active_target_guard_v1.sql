...
--------------------------------------------------------------------------------
-- Preserve the audit trail while repairing exact-target duplicates that would
-- otherwise prevent the permanent unique indexes from being created.
--------------------------------------------------------------------------------
...
WITH ranked AS (
  SELECT
    ...
    first_value(h.id) OVER (
      ...
      ORDER BY
        CASE h.status WHEN 'supervisor_approved' THEN 1 ELSE 2 END,
...
SET status = 'superseded',
    ...
    updated_at = now(),
    supervisor_review_note = concat_ws(
      ' ',
      NULLIF(h.supervisor_review_note, ''),
      'Automatically superseded as a duplicate active order hold.'
    )
FROM ranked r
WHERE r.id = h.id
  AND r.rn > 1;
...
SET status = 'superseded',
    ...
    supervisor_review_note = concat_ws(
      ' ',
      NULLIF(h.supervisor_review_note, ''),
      'Automatically superseded as a duplicate active package hold.'
    )
FROM ranked r
WHERE r.id = h.id
  AND r.rn > 1;
...
SET status = 'superseded',
    ...
    supervisor_review_note = concat_ws(
      ' ',
      NULLIF(h.supervisor_review_note, ''),
      'Automatically superseded as a duplicate active item hold.'
    )
FROM ranked r
WHERE r.id = h.id
  AND r.rn > 1;
...
--------------------------------------------------------------------------------
-- Cross-scope protection. The order advisory lock serialises stale-page and
-- concurrent RPC submissions. Existing narrowing RPCs are explicitly allowed
-- to replace their source hold through narrowed_from_hold_request_id.
--------------------------------------------------------------------------------
