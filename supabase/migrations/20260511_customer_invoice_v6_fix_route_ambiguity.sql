  ...
  WITH src AS (
    ...
    SELECT COALESCE(SUM(l2.goods_evidence),0) goods_total, COALESCE(SUM(l2.shipping_evidence),0) shipping_total, COALESCE(SUM(l2.bundled_amount),0) invoice_total,
      jsonb_agg(jsonb_build_object(
        'released_qty', l2.qty_allocated,
        'description', l2.customer_desc,
        'customer_charge_amount_gbp', l2.bundled_amount,
        ...
        'source_order_id', l2.order_id,
        'source_order_ref', l2.order_ref,
        'source_shipment_booking_ref', l2.booking_ref,
        'source_tracking_submission_id', l2.tracking_submission_id,
        'source_supplier_invoice_line_id', l2.supplier_invoice_line_id,
        'source', 'shipping_customer_invoice_readiness_preview'
      ) ORDER BY l2.order_ref NULLS LAST, l2.booking_ref NULLS LAST, l2.customer_desc NULLS LAST) payload
    FROM l l2
  )
  SELECT l.shipment_batch_id,l.booking_ref,l.importer_id,l.importer_name,l.shipper_id,l.shipper_name,
    l.inv_type,l.inv_status,l.route_value,l.si_state,'T0 / GB_ZERO'::text,
