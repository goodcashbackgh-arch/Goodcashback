...
-- Governing addendum:
-- docs/governing-pack/ui/REFUND_FINAL_SUPERVISOR_ACCEPTANCE_RESTORATION_ADDENDUM_v1.md
--
-- Corrective scope only:
-- preserve the existing operator/importer retailer-update behaviour,
-- but do not advance the parent dispute status from this RPC.
-- Final refund acceptance remains the existing supervisor action.
...
BEGIN
  ...
  -- Intentionally no UPDATE of public.disputes.status here.
  -- Retailer acceptance records the retailer conversation outcome only.
  -- Final refund acceptance remains the existing supervisor-controlled action,
  -- which follows the legal status spine:
