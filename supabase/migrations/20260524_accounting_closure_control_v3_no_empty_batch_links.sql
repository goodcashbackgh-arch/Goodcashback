...
-- Accounting Closure Control v3.
-- Read-only refinement: only expose batch detail action_href when the matched batch row exists.
-- This prevents closure rows based on posted snapshots from linking to stale/empty batch headers.
  ...
  WITH sage_rows AS (
    SELECT
      ...
      CASE
        ...
        ELSE COALESCE(alloc.allocation_status, 'not_assessed_v3')
      END::text AS cash_or_credit_allocation_status,
      alloc.target_object_id::text AS sage_target_artefact_id,
      CASE
        WHEN s.document_lane IN ('supplier_credit_note','supplier_goods_ap','shipper_ap') THEN 'source_attachment_not_proven_in_closure_v3'
        ELSE 'not_applicable'
      ...
      jsonb_build_object(
        ...
        'batch_row_id', br.id,
        'action_href', CASE WHEN br.id IS NOT NULL THEN ('/internal/accounting-command-centre/batches/' || s.batch_id::text) ELSE NULL END,
        'action_href_status', CASE WHEN br.id IS NOT NULL THEN 'available' ELSE 'not_available_batch_has_no_matching_row' END,
        ...
        'closure_model_version', 'v3_no_empty_batch_links'
      ) AS trace_json
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        'batch_row_id', cbr.id,
        'action_href', CASE WHEN cbr.id IS NOT NULL THEN ('/internal/accounting-command-centre/cash-posting/batches/' || cb.id::text) ELSE NULL END,
        'action_href_status', CASE WHEN cbr.id IS NOT NULL THEN 'available' ELSE 'not_available_cash_batch_row_missing' END,
    ...
    SELECT
      ...
      CASE
        ...
        WHEN a.calculated_state = 'failed' THEN COALESCE(NULLIF(a.raw_error, ''), 'Posting failed. Inspect request/response.')
        WHEN a.calculated_state = 'posted_needs_review' THEN 'Posted/accepted state exists, but Sage object id or required artefact id is missing.'
...
'Accounting Closure Control v3 read-only rows: no empty batch action links; snapshot-based closure rows do not link to stale batch headers without matched batch rows.';
