...
-- Existing Mini Build 1–4 authorities remain untouched.
-- Root cause: on a second idempotent call, the bridge could delete an existing
-- review link when zero new memberships were inserted. Cleanup is now limited
-- to a link created by the same invocation.
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'Protected authority changed during exact review bridge idempotency correction.';
  END IF;
END
$postflight$;
