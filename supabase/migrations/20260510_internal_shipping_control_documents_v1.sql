-- Internal shipping control document status v1
...
BEGIN
  ...
  WITH package_scope AS (
    ...
    LEFT JOIN public.shipper_shipment_batch_packages p ON p.shipment_batch_id = b.id AND p.active = true
  ...
  SELECT
    ...
    CASE WHEN ld.review_status = 'accepted_current' THEN 'shipping_document_ready_for_next_review' ELSE 'not_ready' END::text AS sage_readiness_status,
    CASE
      ...
      ELSE 'shipping_document_uploaded_needs_supervisor_processing'
  ...
  GROUP BY b.id, b.booking_ref, b.shipper_id, s.name, b.importer_id, i.trading_name, i.company_name,
           ...
           ld.shipment_batch_id, ld.review_status, ld.ocr_status
