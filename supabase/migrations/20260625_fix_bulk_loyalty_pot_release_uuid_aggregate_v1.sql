...
-- Fix bulk completion-loyalty funding-pot release UUID aggregate.
-- Previous version used max(uuid), which PostgreSQL does not support.
-- This replacement preserves the same controls and replaces max(uuid) with array_agg(...)[1].
...
'Bulk same-importer completion-loyalty funding-pot release. Uses array_agg for UUID representative values because PostgreSQL does not support max(uuid). Preserves existing single-row release validation.';
