...
-- Canonical status overlay: once final-sale settlement is clear, shipper AP not-ready
-- must remain a real internal blocker before the platform can show the order as
-- operationally complete. This is deliberately small/additive and wraps the
-- existing canonical status read model instead of changing shipper AP, Sage,
-- VAT, DVA/card, funding, or order data.

DO $$
BEGIN
  IF to_regprocedure('public.internal_platform_order_status_v1_before_shipper_ap_blocker()') IS NULL THEN
    ALTER FUNCTION public.internal_platform_order_status_v1()
      RENAME TO internal_platform_order_status_v1_before_shipper_ap_blocker;
  END IF;
END $$;
    ...
    SELECT
      ...
      CASE
        WHEN b.current_stage = 'complete'
          AND COALESCE(b.shipper_ap_state, '') <> 'apportionment_approved'
