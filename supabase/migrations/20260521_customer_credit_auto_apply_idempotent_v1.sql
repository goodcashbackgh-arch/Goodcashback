...
BEGIN
  ...
  IF v_gap <= 0 THEN
    RETURN jsonb_build_object('ok', true, 'applied_gbp', 0, 'already_funded_or_applied', true, 'gap_gbp', v_gap);
  END IF;
  ...
  ON CONFLICT ON CONSTRAINT uq_order_funding_events_source DO UPDATE
