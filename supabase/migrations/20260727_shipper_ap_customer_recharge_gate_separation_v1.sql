...
--
-- Scope:
-- - preserve every existing canonical Sage-ready queue row unchanged;
-- - add only accepted/current shipper invoice rows omitted solely because customer
--   shipping apportionment is not approved yet;
-- - leave customer release, Mini-build 4, UI/actions, freeze payload and Sage posting unchanged.
...
BEGIN
  ...
  IF to_regprocedure('public.internal_freeze_shipper_ap_sage_batch_v1(uuid[],text)') IS NULL
     OR to_regprocedure('public.internal_customer_sales_release_sources_v1(uuid)') IS NULL THEN
    RAISE EXCEPTION 'Required shipper-AP freeze or customer-release function is missing';
  END IF;
...
AS $func$
  WITH preserved_queue AS (
    ...
    SELECT
      ...
      COALESCE(NULLIF(sd.extracted_document_ref::text, ''), NULLIF(sd.document_ref::text, ''))::text AS document_ref,
      COALESCE(sd.extracted_document_date, sd.document_date) AS document_date,
      COALESCE(sd.extracted_total_amount, sd.total_amount)::numeric AS document_total,
      COALESCE(NULLIF(sd.extracted_currency_code::text, ''), NULLIF(sd.currency_code::text, ''), 'GBP')::text AS currency_code,
    ...
    WHERE sd.active = true
      ...
      AND COALESCE(sd.extracted_total_amount, sd.total_amount, 0) > 0
      AND NOT EXISTS (
        SELECT 1
...
COMMENT ON FUNCTION
  public.internal_ready_for_sage_queue_v2_pre_shipper_ap_gate_separation_v1()
IS
  'Private exact canonical Sage-ready queue immediately before shipper AP/customer recharge gate separation.';
...
BEGIN
  ...
  IF v_duplicates <> 0 THEN
    RAISE EXCEPTION 'Canonical queue contains duplicate lane/source identities: %', v_duplicates;
  END IF;
  ...
  WHERE q.document_lane = 'shipper_ap'
    ...
    AND (
      ...
      OR COALESCE(sd.extracted_total_amount, sd.total_amount, 0) <= 0
      OR q.amount_gbp IS DISTINCT FROM COALESCE(sd.extracted_total_amount, sd.total_amount)
      OR EXISTS (
        SELECT 1
        FROM public.shipping_cost_allocations allocation
        WHERE allocation.shipping_document_id = sd.id
  ...
  IF v_invalid <> 0 THEN
    RAISE EXCEPTION 'Invalid additive shipper AP rows detected: %', v_invalid;
  END IF;
END
$verify$;
