...
-- Canonical status fix: confirmed DVA/card final-balance allocations must reduce
-- customer-sales/final-settlement balance due everywhere that consumes
-- internal_platform_order_status_v1(), without changing accepted-estimate funding state.
...
BEGIN
  ...
  WITH order_scope AS (
    ...
    SELECT
      a.order_id,
      COALESCE(SUM(a.allocated_gbp_amount), 0)::numeric AS final_balance_payment_gbp
    ...
    SELECT
      ...
      (COALESCE(f.amount_received_gbp, 0) + COALESCE(fbp.final_balance_payment_gbp, 0))::numeric AS settlement_amount_received_gbp,
    ...
    LEFT JOIN final_balance_payments fbp ON fbp.order_id = o.id
    ...
    SELECT
      ...
      CASE
        WHEN j.posted_sale_charge_docs + j.posted_sale_credit_docs > 0 THEN GREATEST(j.signed_final_sale_value_gbp - j.settlement_amount_received_gbp, 0)
        ELSE 0::numeric
      END AS final_balance_due_gbp,
      CASE
        WHEN j.posted_sale_charge_docs + j.posted_sale_credit_docs > 0 THEN GREATEST(j.settlement_amount_received_gbp - j.signed_final_sale_value_gbp, 0)
        ELSE 0::numeric
