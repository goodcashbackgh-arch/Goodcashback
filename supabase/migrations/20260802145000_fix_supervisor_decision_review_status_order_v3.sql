...
BEGIN
  ...
  IF position('PREAUTHORISE_EXISTING_EXCEPTION_ROUTE_V3' IN v_before) > 0 THEN
    RAISE NOTICE 'Supervisor decision ordering patch v3 is already installed.';
    RETURN;
  END IF;
  ...
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'Expected the exact allocation CTE/update anchor once, found %; refusing unsafe patch.', v_count;
  END IF;
  ...
  IF position('PREAUTHORISE_EXISTING_EXCEPTION_ROUTE_V3' IN v_after) = 0 THEN
    RAISE EXCEPTION 'Supervisor decision ordering patch v3 did not persist.';
  END IF;

  IF position('PREAUTHORISE_EXISTING_EXCEPTION_ROUTE_V3' IN v_after)
     > position(v_anchor IN v_after) THEN
    RAISE EXCEPTION 'Review pre-authorisation was not installed before the allocation CTE/update.';
  END IF;
END
$patch$;
