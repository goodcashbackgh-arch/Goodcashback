...
-- Cash Posting Workbench read model v1.
-- Additive only: mapping seeds + read-only RPC. No freeze/post writes.
...
VALUES
  ('DVA_CASH_BANK_ACCOUNT','cash_posting','DVA/card Sage bank account','Default Sage bank account id for DVA/card/bank IN and OUT cash postings. Populate from Sage bank_accounts.','free_text',ARRAY['cash_posting']::text[]),
  ('CUSTOMER_RECEIPT_PAYMENT_METHOD','cash_posting','Customer receipt payment method','Optional Sage payment method/default for customer/importer receipts.','free_text',ARRAY['customer_receipt_on_account']::text[]),
  ('SUPPLIER_PAYMENT_METHOD','cash_posting','Supplier payment method','Optional Sage payment method/default for supplier/retailer and shipper AP payments.','free_text',ARRAY['supplier_invoice_payment','shipper_invoice_payment']::text[]),
  ('FX_CARD_GAIN_LEDGER','cash_posting','FX/card gain ledger','Ledger for favourable FX/card/provider residuals.','ledger_account_id',ARRAY['fx_card_difference']::text[]),
  ('FX_CARD_LOSS_LEDGER','cash_posting','FX/card loss ledger','Ledger for unfavourable FX/card/provider residuals.','ledger_account_id',ARRAY['fx_card_difference']::text[]),
  ('BANK_FEE_LEDGER','cash_posting','Bank/provider/card fee ledger','Ledger for bank, card or provider fees.','ledger_account_id',ARRAY['bank_fee']::text[]),
  ('UNMATCHED_HOLD_LEDGER','cash_posting','Unmatched/hold suspense ledger','Optional suspense/hold ledger for unmatched balances.','ledger_account_id',ARRAY['unmatched_hold']::text[])
  ...
  WITH cash_defaults AS (
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        'endpoint', CASE WHEN adv.allocation_type = 'supplier_invoice' THEN 'POST /purchase_payments then POST /allocations' ELSE 'endpoint_prove_required' END
      ) AS detail_json
...
'Read-only Accounting Command Centre cash posting rows from confirmed DVA funding and DVA statement-line allocations. No freeze, validate or Sage post.';
