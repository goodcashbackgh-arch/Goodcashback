...
BEGIN
  ...
  WITH src AS (
    ...
    SELECT
      ...
      CASE WHEN EXISTS (SELECT 1 FROM src s2 WHERE s2.blocker IS NOT NULL) THEN 'blocked' ELSE 'draft_preview' END::text AS resolved_invoice_status,
    ...
    SELECT
      ...
      CONCAT(
        CASE WHEN r.resolved_invoice_type = 'supplementary' THEN 'Supplementary export sale' ELSE 'Export sale' END,
        ' - ', COALESCE(NULLIF(BTRIM(s.item_description), ''), 'Goods'),
    ...
    FROM src s
    CROSS JOIN route r
    WHERE public.customer_line_has_active_hold_conflict_v1(s.order_id, s.tracking_submission_id, s.supplier_invoice_line_id) IS DISTINCT FROM true
  ), totals AS (
    SELECT
  ...
  FROM line_calc lc
  CROSS JOIN totals t
  WHERE t.total_bundled_customer_charge_gbp > 0
  ORDER BY lc.order_ref NULLS LAST, lc.booking_ref NULLS LAST, lc.source_product_description NULLS LAST;
END;
$$;
