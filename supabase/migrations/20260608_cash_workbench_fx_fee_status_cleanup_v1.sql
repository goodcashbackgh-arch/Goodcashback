...
-- Cash workbench FX/fee status cleanup.
-- No table changes and no posting-engine changes.
-- Purpose: expose FX/card difference and bank/provider fee rows as ready/selectable
-- in the read model once the already-built Sage posting mappings exist.
...
'Read-only Accounting Command Centre cash posting rows. FX/card and bank/provider fee rows are ready/selectable when their existing Sage bank/ledger/tax mappings are present. No freeze, validate or Sage post.';
