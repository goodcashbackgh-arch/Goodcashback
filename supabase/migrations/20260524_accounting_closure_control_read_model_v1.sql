...
-- Accounting Closure Control read model v1.
-- Read-only diagnostic/closure layer for the Accounting Command Centre.
-- No posting, no allocation, no endpoint expansion.
...
BEGIN
  ...
  IF to_regclass('public.sage_posting_batches') IS NULL THEN
    RAISE EXCEPTION 'Missing public.sage_posting_batches';
  END IF;
  IF to_regclass('public.sage_posting_batch_rows') IS NULL THEN
    RAISE EXCEPTION 'Missing public.sage_posting_batch_rows';
  END IF;
...
BEGIN
  ...
  WITH sage_rows AS (
    SELECT
      ...
      COALESCE(alloc.allocation_status, CASE WHEN s.document_lane = 'supplier_credit_note' AND s.sage_posting_status = 'posted' THEN 'credit_posted_settlement_unproven' ELSE 'not_assessed_v1' END)::text AS cash_or_credit_allocation_status,
      alloc.target_object_id::text AS sage_target_artefact_id,
      'not_assessed_v1'::text AS attachment_state,
      ...
      jsonb_build_object(
        ...
        'resolved_payload', s.resolved_payload,
        'commercial_payload', s.commercial_payload,
        'mapping_snapshot', s.mapping_snapshot
      ) AS trace_json
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        'request_payload', COALESCE(cbr.request_payload, c.request_payload),
        'response_payload', COALESCE(cbr.response_payload, c.sage_response_payload),
        'internal_reference_json', c.internal_reference_json
      ) AS trace_json
    ...
    SELECT
      a.*,
      CASE
        WHEN a.calculated_state = 'duplicate_risk' THEN 'Resolve duplicate/idempotency risk before treating row as closed.'
        WHEN a.calculated_state = 'failed' THEN COALESCE(NULLIF(a.raw_error, ''), 'Posting failed; inspect request/response logs.')
        WHEN a.calculated_state = 'posted_needs_review' THEN 'Sage accepted or status says posted, but required object id/artefact id is missing.'
      ...
      CASE
        ...
        WHEN a.calculated_state = 'posted_not_closed' THEN 'Open trace and prove settlement/allocation'
...
REVOKE ALL ON FUNCTION public.internal_accounting_closure_control_rows_v1(text, text, text, integer, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.internal_accounting_closure_control_rows_v1(text, text, text, integer, integer) TO authenticated;

COMMENT ON FUNCTION public.internal_accounting_closure_control_rows_v1(text, text, text, integer, integer) IS
'Read-only Accounting Command Centre closure control rows. Compares platform posting/cash snapshots, batch rows, Sage object ids, allocation status and duplicate risk. No writes.';
