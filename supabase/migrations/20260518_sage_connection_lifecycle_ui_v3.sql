...
-- Clean Sage connection lifecycle status for Accounting Command Centre.
-- Facts from live OAuth test: Sage is issuing short-lived access tokens. Main UI should show
-- refreshability, not noisy "expires soon" warnings. Pending/abandoned attempts remain audit rows.
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      WHEN MAX(t.expires_at) FILTER (WHERE t.status = 'active') <= now() + interval '60 seconds' THEN 'refresh_due'
      ELSE 'refreshable'
  ...
  ORDER BY
    ...
    COALESCE(MAX(t.expires_at) FILTER (WHERE t.status = 'active'), c.created_at) DESC,
    c.created_at DESC;
END;
$$;
