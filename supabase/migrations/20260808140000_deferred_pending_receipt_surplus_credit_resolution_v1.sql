...
--   5. established settlement arithmetic, legacy rows, physical FX, Sage, funding,
--      invoices, VAT, shipment and tracking remain untouched.
...
-- Preserve the exact public column contracts before replacing either view.
...
WITH pending AS (
  ...
  SELECT
    ...
    ROUND((
      ...
      + CASE
          WHEN COALESCE(p.pending_position_count, 0) > 0
            THEN COALESCE(fb.final_balance_payment_gbp, 0)
          ELSE 0
        END
...
-- ---------------------------------------------------------------------------
-- 2. Add future-only provenance for credits confirmed through the new wrapper.
-- ---------------------------------------------------------------------------
...
REVOKE ALL ON public.order_pending_surplus_credit_resolution_provenance_v1
...
-- ---------------------------------------------------------------------------
-- 3. New atomic wrapper. The established accounting RPC remains unchanged.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Pending-aware surplus evidence position is missing.';
  END IF;

  IF v_evidence.evidence_status NOT IN (
  ...
  IF v_evidence.evidence_surplus_gbp <= 0 THEN
    RAISE EXCEPTION 'No surplus.';
  END IF;

  SELECT public.staff_confirm_surplus_from_evidence_min_v1(
    p_order_id,
    p_reason,
    p_notes
  )
  ...
  IF v_pending.status <> 'credit_confirmed'
     OR v_pending.confirmed_credit_ledger_id IS NULL THEN
  ...
  INSERT INTO public.order_pending_surplus_credit_resolution_provenance_v1 (
    order_id,
    pending_surplus_id,
    confirmed_credit_ledger_id,
    created_by_staff_id
  ) VALUES (
    p_order_id,
    v_pending.id,
    v_pending.confirmed_credit_ledger_id,
    v_staff.id
  )
...
REVOKE ALL ON FUNCTION public.staff_confirm_pending_receipt_surplus_credit_v1(uuid,text,text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.staff_confirm_pending_receipt_surplus_credit_v1(uuid,text,text)
  TO authenticated;

COMMENT ON FUNCTION public.staff_confirm_pending_receipt_surplus_credit_v1(uuid,text,text) IS
'Atomic future-only wrapper around staff_confirm_surplus_from_evidence_min_v1. It requires one pending_evidence receipt residual, calls the established accounting RPC unchanged, then records exact pending-to-credit provenance. Historical credit_confirmed rows are not backfilled.';

-- ---------------------------------------------------------------------------
-- 4. Patch only the supplier-OUT FX input inside canonical settlement.
--    The calculated/resolved/final settlement formulas are not replaced.
-- ---------------------------------------------------------------------------
DO $settlement_patch$
DECLARE
  v_definition text;
  ...
  v_end_count integer;
  v_settlement_start text := 'settlement_actions as (';
  v_settlement_end text := '), blockers as (';
  v_base_start text := 'base as (';
  v_base_end text := '), calculated as (';
  v_new_settlement text := $new_settlement$
provenance_credit AS (
   ...
   SELECT x.order_id,
      round(COALESCE(sum(x.active_action_fx_gbp), (0)::numeric), 2) AS active_action_fx_gbp,
      round(COALESCE(sum(x.raw_supplier_fx_gbp), (0)::numeric), 2) AS raw_supplier_fx_gbp,
     ...
     FROM (
       SELECT a.order_id,
          ...
          (0)::numeric AS raw_supplier_fx_gbp,
       ...
       SELECT supplier_order.order_id,
          (0)::numeric AS active_action_fx_gbp,
...
base AS (
   SELECT o.id AS order_id,
      ...
      COALESCE(pr.pending_evidence_count, 0) AS pending_evidence_count,
      COALESCE(pr.pending_credit_confirmed_count, 0) AS pending_credit_confirmed_count,
      COALESCE(ifx.inbound_fx_receipt_residual_gbp, (0)::numeric) AS inbound_fx_receipt_residual_gbp,
      COALESCE(ifx.inbound_fx_receipt_row_count, 0) AS inbound_fx_receipt_row_count,
      COALESCE(fb.final_balance_payment_gbp, (0)::numeric) AS final_balance_payment_gbp,
      COALESCE(sa.final_sale_document_count, 0) AS final_sale_document_count,
      ...
      COALESCE(oc.customer_credit_row_count, 0) AS customer_credit_row_count,
      round((
        COALESCE(act.active_action_fx_gbp, (0)::numeric)
        + GREATEST(
            COALESCE(act.raw_supplier_fx_gbp, (0)::numeric)
            - GREATEST(
                COALESCE(overlap_calc.supplier_needed_before_pending_credit_gbp, (0)::numeric)
                - COALESCE(overlap_calc.supplier_needed_after_pending_credit_gbp, (0)::numeric),
                (0)::numeric
              ),
            (0)::numeric
          )
      )::numeric, 2) AS settlement_fx_card_difference_gbp,
      COALESCE(act.active_resolution_action_count, 0) AS active_resolution_action_count,
      COALESCE(act.reversed_resolution_action_count, 0) AS reversed_resolution_action_count,
      COALESCE(b.open_dispute_count, 0) AS open_dispute_count,
      COALESCE(b.active_hold_count, 0) AS active_hold_count
     ...
     LEFT JOIN provenance_credit pc ON pc.order_id = o.id
     ...
     ) overlap_input ON true
     LEFT JOIN LATERAL (
       SELECT
         round(LEAST(
           COALESCE(act.raw_supplier_fx_gbp, (0)::numeric),
           GREATEST(
             overlap_input.gross_positive_difference_gbp
             - overlap_input.explicit_before_pending_credit_gbp,
             (0)::numeric
           )
     ...
     ) overlap_calc ON true
    WHERE COALESCE(o.order_type, 'original'::text) = 'original'::text
), calculated AS (
$new_base$;
BEGIN
  ...
  -- The installed view must still contain the locked outbound supplier-FX inference.
  ...
  -- Replace only settlement_actions, adding future-only provenance and preserving
  -- the exact established supplier-order attribution branch.
  v_start_count := (
    length(v_lower) - length(replace(v_lower, v_settlement_start, ''))
  ) / length(v_settlement_start);
  v_end_count := (
    length(v_lower) - length(replace(v_lower, v_settlement_end, ''))
  ) / length(v_settlement_end);
  ...
  v_start := position(v_settlement_start IN v_lower);
  v_end := position(v_settlement_end IN v_lower);

  IF v_start <= 0 OR v_end <= v_start THEN
    RAISE EXCEPTION 'Canonical settlement_actions CTE order drifted. Stop before patching.';
  END IF;

  v_patched := substring(v_definition FROM 1 FOR v_start - 1)
    || v_new_settlement
    || substring(v_definition FROM v_end + length(v_settlement_end));

  -- Replace only base, where the existing settlement_fx_card_difference_gbp input
  -- is assembled. The downstream calculated/resolved/final arithmetic is preserved.
  v_lower := lower(v_patched);
  v_start_count := (
    length(v_lower) - length(replace(v_lower, v_base_start, ''))
  ) / length(v_base_start);
  v_end_count := (
    length(v_lower) - length(replace(v_lower, v_base_end, ''))
  ) / length(v_base_end);

  IF v_start_count <> 1 OR v_end_count <> 1 THEN
    RAISE EXCEPTION 'Canonical base/calculated boundaries drifted. Stop before patching.';
  END IF;

  v_start := position(v_base_start IN v_lower);
  v_end := position(v_base_end IN v_lower);

  IF v_start <= 0 OR v_end <= v_start THEN
    RAISE EXCEPTION 'Canonical base CTE order drifted. Stop before patching.';
  END IF;

  v_patched := substring(v_patched FROM 1 FOR v_start - 1)
    || v_new_base
    || substring(v_patched FROM v_end + length(v_base_end));

  IF lower(v_patched) NOT LIKE '%total_classified_gbp%'
     OR lower(v_patched) NOT LIKE '%remaining_unresolved_gbp%'
     OR lower(v_patched) NOT LIKE '%over_resolved_gbp%'
     OR lower(v_patched) NOT LIKE '%resolution_status%' THEN
    RAISE EXCEPTION 'Locked downstream settlement arithmetic/output markers are missing. Stop before patching.';
  END IF;
...
-- ---------------------------------------------------------------------------
-- 5. Contract and no-backfill guards.
-- ---------------------------------------------------------------------------
DO $post_guard$
DECLARE
  v_before text[];
  ...
  v_count integer;
BEGIN
  SELECT g.column_names
  INTO v_before
  FROM pending_receipt_surplus_contract_guard_v1 g
  WHERE g.object_name = 'order_surplus_evidence_position_v3';
  ...
  IF v_before IS DISTINCT FROM v_after THEN
    RAISE EXCEPTION 'order_surplus_evidence_position_v3 column contract changed. Stop.';
  END IF;

  SELECT g.column_names
  INTO v_before
  FROM pending_receipt_surplus_contract_guard_v1 g
  WHERE g.object_name = 'order_settlement_resolution_position_v1';
  ...
  IF v_before IS DISTINCT FROM v_after THEN
    RAISE EXCEPTION 'order_settlement_resolution_position_v1 column contract changed. Stop.';
  END IF;
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION 'New provenance table must install empty; legacy rows must not be backfilled.';
  END IF;
END
$post_guard$;
