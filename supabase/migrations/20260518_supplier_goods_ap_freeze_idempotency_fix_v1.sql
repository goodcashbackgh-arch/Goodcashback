...
-- Fix supplier_goods_ap freeze failure:
-- column reference "idempotency_key" is ambiguous.
-- Cause: PL/pgSQL output column name conflicts with unqualified RETURNING idempotency_key.
-- Scope: replace only the supplier goods AP freeze RPC. No posting. No schema change.
