...
-- Preserve the existing OCR save contract and every existing guard. The only
-- change is that the legacy 20% VAT gross-up heuristic is not allowed to run
-- where delivery/discount facts already explain why goods lines differ from the
-- gross invoice header. Invoices without active adjustments retain the exact
-- established gross-up behaviour.
...
DECLARE
  ...
  v_has_active_adjustment boolean := false;
BEGIN
  SELECT s.id
  INTO v_staff_id
  FROM public.staff s
  ...
  IF p_lines IS NOT NULL AND jsonb_typeof(p_lines) = 'array' THEN
    ...
    -- Mindee can return line item prices excluding VAT while the OCR header total
    -- is gross. Keep the established low-risk adjustment only where no declared
    -- delivery/discount fact exists. Otherwise those facts, not an inferred VAT
    -- factor, explain the difference between goods and the gross header.
    IF NOT v_has_active_adjustment
       AND p_ocr_invoice_total_gbp IS NOT NULL
