...
-- Completion Loyalty Bulk Funding Pot Release v1
-- Adds an additive supervisor/admin wrapper for releasing multiple same-importer
-- completion-loyalty rewards from one reserved main-bank OUT and one DVA/card IN.
--
-- This migration does not call Sage, does not create VAT rows, does not change
-- apply-to-order logic, and does not change shipper AP or residual allocation flows.
-- The wrapper preserves the existing single-row validation by calling
-- staff_pair_loyalty_destination_in_and_release_v1(...) for each selected match.
