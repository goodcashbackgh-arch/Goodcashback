...
-- DVA supplier payment source-bank split v1.
-- Contract: docs/governing-pack/accounting/DVA_SUPPLIER_PAYMENT_SOURCE_SPLIT_CONTRACT_v1.md
--
-- Supplier invoice payment rows must credit the bank/wallet that actually funds
-- that payment leg. The prior cash workbench projection used DVA_CASH_BANK_ACCOUNT
-- for every supplier_invoice allocation row.
...
COMMENT ON COLUMN public.dva_statement_line_allocations.source_bank_account_mapping_code IS
'Sage mapping code for the bank/wallet account that funds this statement-line allocation leg. Supplier invoice payment cash rows must use this mapping instead of always DVA_CASH_BANK_ACCOUNT.';

COMMENT ON COLUMN public.dva_statement_line_allocations.source_wallet_code IS
'Optional platform wallet/source code for loyalty or virtual wallet funded statement-line allocation legs.';

INSERT INTO public.sage_mapping_settings (mapping_code, mapping_group, display_name, description, value_kind, required_for)
VALUES
  ('LOYALTY_DVA_GHS_BANK_ACCOUNT','cash_posting','Completion loyalty DVA GHS wallet bank account','Sage bank account id used when a supplier invoice payment leg is funded from completion-loyalty DVA/GHS wallet funds.','free_text',ARRAY['supplier_invoice_payment','completion_loyalty_wallet']::text[]),
  ('LOYALTY_VIRTUAL_GBP_BANK_ACCOUNT','cash_posting','Completion loyalty virtual GBP wallet bank account','Sage bank account id used when a supplier invoice payment leg is funded from completion-loyalty virtual GBP wallet funds.','free_text',ARRAY['supplier_invoice_payment','completion_loyalty_wallet']::text[])
  ...
  WITH cash_defaults AS (
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        'short_reference', left('GCB-IN-' || upper(left(replace(o.id::text, '-', ''), 8)) || '-' || upper(left(replace(COALESCE(to_jsonb(dsl)->>'auth_id_ref', to_jsonb(dsl)->>'payment_auth_id', dsl.id::text), '-', ''), 12)), 32),
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        'short_reference', ('GCB-FB-' || upper(left(replace(o.id::text, '-', ''), 8)) || '-' || upper(left(replace(adv.allocation_id::text, '-', ''), 8))),
        'endpoint', 'POST /contact_payments',
        'transaction_type_id', 'CUSTOMER_RECEIPT',
    ...
    SELECT
      ...
      pm.sage_contact_display_name::text,
      CASE
        WHEN COALESCE(NULLIF(trim(dla.source_bank_account_mapping_code), ''), 'DVA_CASH_BANK_ACCOUNT') = 'DVA_CASH_BANK_ACCOUNT'
          THEN cd.bank_account_id::text
        ELSE source_bank.sage_external_id::text
      END AS sage_bank_account_id,
      posted.sage_invoice_id::text AS target_sage_object_id,
      CASE
        WHEN adv.allocation_status <> 'confirmed' THEN 'blocked_allocation_not_confirmed'
        WHEN NULLIF(trim(COALESCE(CASE WHEN COALESCE(NULLIF(trim(dla.source_bank_account_mapping_code), ''), 'DVA_CASH_BANK_ACCOUNT') = 'DVA_CASH_BANK_ACCOUNT' THEN cd.bank_account_id ELSE source_bank.sage_external_id END, '')), '') IS NULL THEN 'blocked_missing_sage_bank_account'
      ...
      CASE
        ...
        WHEN NULLIF(trim(COALESCE(CASE WHEN COALESCE(NULLIF(trim(dla.source_bank_account_mapping_code), ''), 'DVA_CASH_BANK_ACCOUNT') = 'DVA_CASH_BANK_ACCOUNT' THEN cd.bank_account_id ELSE source_bank.sage_external_id END, '')), '') IS NULL THEN ('Sage bank account mapping missing for ' || COALESCE(NULLIF(trim(dla.source_bank_account_mapping_code), ''), 'DVA_CASH_BANK_ACCOUNT'))
      ...
      (
        adv.allocation_status = 'confirmed'
        AND NULLIF(trim(COALESCE(CASE WHEN COALESCE(NULLIF(trim(dla.source_bank_account_mapping_code), ''), 'DVA_CASH_BANK_ACCOUNT') = 'DVA_CASH_BANK_ACCOUNT' THEN cd.bank_account_id ELSE source_bank.sage_external_id END, '')), '') IS NOT NULL
        AND (
          (
      ...
      jsonb_build_object(
        ...
        'source_bank_account_mapping_code', COALESCE(NULLIF(trim(dla.source_bank_account_mapping_code), ''), 'DVA_CASH_BANK_ACCOUNT'),
        'source_wallet_code', dla.source_wallet_code,
        'short_reference', ('GCB-OUT-' || left(COALESCE(adv.supplier_invoice_ref::text, adv.order_ref::text, adv.allocation_id::text), 18)),
        'endpoint', CASE
          WHEN adv.allocation_type = 'supplier_invoice' THEN 'POST /contact_payments'
    ...
    FROM public.dva_statement_line_allocation_detail_vw adv
    LEFT JOIN public.dva_statement_line_allocations dla ON dla.id = adv.allocation_id
    LEFT JOIN public.sage_mapping_settings source_bank
      ON source_bank.mapping_code = COALESCE(NULLIF(trim(dla.source_bank_account_mapping_code), ''), 'DVA_CASH_BANK_ACCOUNT')
     AND source_bank.is_active = true
    CROSS JOIN cash_defaults cd
    ...
    WHERE (v_direction = 'all' OR lower(ar.direction) = v_direction)
      ...
      AND (v_search IS NULL OR lower(concat_ws(' ', ar.counterparty_name, ar.order_ref, ar.auth_ref, ar.reference_raw, ar.matched_target_ref, ar.category, ar.source_type, ar.blocker, ar.detail_json->>'source_bank_account_mapping_code', ar.detail_json->>'source_wallet_code')) LIKE '%' || v_search || '%')
  )
...
'Cash posting rows from DVA/card/wallet statement-line allocations. Supplier invoice payment rows resolve Sage bank account from source_bank_account_mapping_code, defaulting to DVA_CASH_BANK_ACCOUNT only when no source mapping is set.';
