...
-- Accounting Closure Control v4 wrapper.
-- Keeps existing v1/v3 closure logic but corrects stale Sage batch references.
-- For Sage posting rows, prefer the actual matched sage_posting_batch_rows.batch_id
-- over the snapshot's original batch_id. This avoids linking posted closure rows to
-- draft/stale local freeze batches.
...
BEGIN
  ...
  SELECT
    ...
    p.idempotency_key,
    CASE
      WHEN p.row_batch_id IS NOT NULL AND p.posting_batch_id IS NOT NULL AND p.row_batch_id <> p.posting_batch_id
        THEN COALESCE(p.duplicate_warning, 'Snapshot batch differs from actual posting-row batch; closure uses posting-row batch.')
      ELSE p.duplicate_warning
    ...
    CASE
      WHEN p.trace_json->>'source_kind' = 'sage_posting_snapshot' THEN
        jsonb_set(
          ...
          'closure_model_version', 'v4_prefer_posting_row_batch'
        )
      ELSE p.trace_json
    END AS trace_json,
    p.total_count
...
'Accounting Closure Control v4 wrapper: prefers actual posting-row batch over stale snapshot batch for Sage posting closure links and refs.';
