...
-- Fix completion-loyalty supplier wallet candidate matching.
-- The first bridge only matched released funding rows by credit_ledger_id.
-- Real released credits can also be traced by funding confirmation or by the
-- completed/source order stored on the released completion-loyalty credit lot.
-- Additive replacement only: no DVA cash, supplier AP, cash posting adapter, or
-- existing bank mapping behaviour is changed.
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      source_credit.source_table::text AS source_credit_source_table,
      source_credit.source_id AS source_credit_source_id,
      source_credit.source_entity_type::text AS source_credit_source_entity_type,
      source_credit.source_entity_id AS source_credit_source_entity_id,
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE m.importer_id = b.importer_id
        ...
        AND (
          m.credit_ledger_id = b.source_credit_ledger_id
          OR (
            b.source_credit_source_table = 'completion_loyalty_reward_funding_confirmations'
            AND m.funding_confirmation_id = b.source_credit_source_id
          )
          OR (
            b.source_credit_source_entity_type = 'order'
            AND m.completed_order_id = b.source_credit_source_entity_id
          )
        )
      ORDER BY
        CASE
          WHEN m.credit_ledger_id = b.source_credit_ledger_id THEN 1
          WHEN b.source_credit_source_table = 'completion_loyalty_reward_funding_confirmations' AND m.funding_confirmation_id = b.source_credit_source_id THEN 2
          ELSE 3
        END,
...
COMMENT ON FUNCTION public.internal_completion_loyalty_supplier_wallet_payment_candidates_v1(text, integer, integer) IS
'Read-only candidates for completion-loyalty supplier payments from Virtual GBP or DVA GHS wallet bank accounts. Resolves wallet from released funding match by credit ledger id, funding confirmation id, or completed reward source order. Does not touch DVA_CASH_BANK_ACCOUNT.';
