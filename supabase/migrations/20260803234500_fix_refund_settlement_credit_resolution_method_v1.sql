-- Correct live contract mismatch in settlement-credit refund closure.
-- The dispute_lines check permits refund/replacement/accept_as_is; the existing authority wrote credit.
...
BEGIN
  IF md5(pg_get_functiondef('public.staff_close_refund_exception_as_settlement_credit_v1(uuid,text,text)'::regprocedure))
     <> '0698d2ab2e7301881dac862a18284f52'
  THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE nsp.nspname='public'
      ...
      AND pg_get_constraintdef(con.oid,true) ILIKE '%refund%'
      AND pg_get_constraintdef(con.oid,true) NOT ILIKE '%credit%'
  ) THEN
    RAISE EXCEPTION 'Unexpected dispute_lines resolution-method contract.';
  END IF;
END
$preflight$;
...
BEGIN
  IF v_def NOT ILIKE '%resolution_method=''refund''%'
     OR v_def ILIKE '%resolution_method=''credit''%'
  THEN RAISE EXCEPTION 'Refund settlement-credit authority correction did not install.'; END IF;
END
$postflight$;
