...
-- Fix internal_sage_connection_status_v1: Postgres does not support max(uuid).
-- Scope: status/read-only RPC only. No OAuth change, no token change, no Sage API call.
...
BEGIN
  ...
  IF to_regclass('public.sage_oauth_tokens') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: public.sage_oauth_tokens';
  END IF;
