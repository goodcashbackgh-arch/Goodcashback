...
-- Patch E: deterministic one-active-account rule for future writes only.
-- Live preflight confirmed zero duplicate active retailer+shipper pairs before this guard.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'retailers', (
      ...
      FROM (
        SELECT r.id, r.name, r.website_url, r.global_enabled
        FROM public.retailers r
        WHERE r.global_enabled = true
      ) x
    ),
    'lanes', (
      ...
      FROM (
        SELECT
          ...
          min(ra.id) FILTER (WHERE ra.status = 'active') AS active_account_id,
          CASE
            WHEN count(ra.id) FILTER (WHERE ra.status = 'active') = 1 THEN 'ready'
            WHEN count(ra.id) FILTER (WHERE ra.status = 'active') = 0 THEN 'not_ready_missing_account'
            ELSE 'not_ready_ambiguous_accounts'
          END AS readiness
        FROM public.shipper_retailers sr
        JOIN public.shippers sh
          ON sh.id = sr.shipper_id
         AND sh.active = true
        JOIN public.retailers r
          ON r.id = sr.retailer_id
         AND r.global_enabled = true
        LEFT JOIN public.retailer_accounts ra
          ON ra.shipper_id = sr.shipper_id
         AND ra.retailer_id = sr.retailer_id
    ...
    'accounts', (
      ...
      FROM (
        SELECT
          ...
          ra.account_email,
          ra.account_username,
          ra.credentials_vault_ref,
          ra.credential_delivery_method,
          ra.delivery_address_locked_to_hub_id,
    ...
    'hubs', (
      ...
      FROM (
        ...
        FROM public.hubs h
        JOIN public.shippers sh
          ON sh.id = h.shipper_id
         AND sh.active = true
        WHERE h.active = true
          AND h.shipper_id IS NOT NULL
      ) x
    )
  );
END;
...
BEGIN
  ...
  IF p_retailer_id IS NULL THEN
    RAISE EXCEPTION 'retailer_required';
  END IF;

  IF v_email = '' OR v_email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' THEN
    RAISE EXCEPTION 'valid_account_email_required';
  END IF;

  IF v_delivery_method NOT IN ('vault_brokered', 'shared_direct', 'pending_vault_upgrade') THEN
    RAISE EXCEPTION 'invalid_credential_delivery_method';
  END IF;

  IF v_status NOT IN ('active', 'suspended', 'locked_out') THEN
    RAISE EXCEPTION 'invalid_retailer_account_status';
  END IF;

  IF v_card_last_4 IS NOT NULL AND v_card_last_4 !~ '^[0-9]{4}$' THEN
    RAISE EXCEPTION 'card_last_4_must_be_four_digits';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'retailer_not_found_or_disabled';
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM public.shipper_retailers sr
    WHERE sr.shipper_id = p_shipper_id
      AND sr.retailer_id = p_retailer_id
      AND sr.enabled = true
  ) THEN
    RAISE EXCEPTION 'shipper_retailer_lane_not_enabled';
  END IF;

  IF p_delivery_address_locked_to_hub_id IS NULL THEN
    RAISE EXCEPTION 'delivery_hub_required';
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM public.hubs h
    WHERE h.id = p_delivery_address_locked_to_hub_id
      AND h.shipper_id = p_shipper_id
      AND h.active = true
  ) THEN
    RAISE EXCEPTION 'delivery_hub_not_active_for_shipper';
  END IF;
  ...
  IF p_retailer_account_id IS NOT NULL THEN
    ...
    INTO v_existing
    FROM public.retailer_accounts ra
    WHERE ra.id = p_retailer_account_id
    FOR UPDATE;

    IF v_existing.id IS NULL THEN
      RAISE EXCEPTION 'retailer_account_not_found';
    END IF;

    IF v_existing.shipper_id IS NULL THEN
      RAISE EXCEPTION 'shared_retailer_account_not_editable_in_shipper_lane';
    END IF;

    IF v_existing.shipper_id IS DISTINCT FROM p_shipper_id
       OR v_existing.retailer_id IS DISTINCT FROM p_retailer_id THEN
      RAISE EXCEPTION 'retailer_account_lane_change_not_allowed';
    END IF;
  END IF;
  ...
  IF v_status = 'active' AND EXISTS (
    SELECT 1
    FROM public.retailer_accounts ra
    WHERE ra.shipper_id = p_shipper_id
      AND ra.retailer_id = p_retailer_id
      AND ra.status = 'active'
      AND (p_retailer_account_id IS NULL OR ra.id <> p_retailer_account_id)
  ) THEN
    RAISE EXCEPTION 'active_retailer_account_already_exists_for_lane';
  END IF;
  ...
  IF p_retailer_account_id IS NULL THEN
    INSERT INTO public.retailer_accounts (
      ...
      v_email,
      v_username,
      v_credentials_vault_ref,
      v_delivery_method,
      p_delivery_address_locked_to_hub_id,
      v_card_last_4,
      v_card_vault_ref,
      v_status
    )
  ...
  ELSE
    ...
    SET account_email = v_email,
        account_username = v_username,
        credentials_vault_ref = v_credentials_vault_ref,
        credential_delivery_method = v_delivery_method,
        delivery_address_locked_to_hub_id = p_delivery_address_locked_to_hub_id,
        card_last_4 = v_card_last_4,
        card_vault_ref = v_card_vault_ref,
        status = v_status
    WHERE id = p_retailer_account_id
    RETURNING id INTO v_account_id;
  END IF;

  RETURN jsonb_build_object(
    'ok', true,
    'retailer_account_id', v_account_id,
    'shipper_id', p_shipper_id,
    'retailer_id', p_retailer_id,
    'status', v_status,
    'ready', v_status = 'active'
  );
END;
$$;
