...
BEGIN
  ...
  IF to_regprocedure('public.staff_decide_physical_receipt_review_v2(uuid,text,jsonb,text,text)') IS NOT NULL THEN
    RAISE EXCEPTION 'Supervisor decision v2 authority already exists; inspect before replacing.';
  END IF;
END
$preflight$;
...
'Authenticated exact whole-unit supervisor decision gateway. Validates allocation payload before invoking the preserved atomic v1 decision authority.';
