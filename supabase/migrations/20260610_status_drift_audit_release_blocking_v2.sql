...
-- Contract correction:
-- internal_order_status_drift_audit_v1 is a release-blocking audit.
-- It must not return rows simply because the old hypothetical page-local formula
-- would have produced a different answer. It returns rows only where the live
-- canonical status or audience wrapper is still drifting from the expected
-- canonical settlement balance.
--
-- LOCAL_PAGE_BALANCE_DRIFT remains detectable as a release-blocking proxy only
-- where the audience wrapper/page-facing balance appears to have inherited the
-- legacy local formula.
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      WHEN ABS(a.audience_balance_due_gbp - a.expected_canonical_balance_due_gbp) > 0.01
