...
-- Fix PL/pgSQL ambiguity in shipper AP freeze RPC.
-- RETURNS TABLE has output column `idempotency_key`, so INSERT conflict/return references must be qualified/aliased.
...
BEGIN
  ...
  WITH requested AS (
    ...
    RETURNING
      ...
      public.sage_posting_snapshots.source_id AS inserted_shipping_document_id,
  ...
  SELECT
    ...
    i.inserted_shipping_document_id AS shipping_document_id,
