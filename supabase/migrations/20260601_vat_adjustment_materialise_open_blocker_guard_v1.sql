...
-- VAT adjustment journal materialisation open-blocker guard v1
-- Live-tested in Supabase before commit: VAT HMRC stress harness returned 28 passed / 0 failed.
-- Purpose: prevent creation of VAT adjustment journal queue while unresolved VAT return blockers exist.
...
DECLARE
  ...
  v_open_blocker_count integer := 0;
...
BEGIN
  ...
  -- Critical control guard:
  -- Preview blockers are generated from proposal logic, but VAT return blockers can already exist on the run.
  -- Do not create adjustment journal queue entries while unresolved blocker rows exist.
  ...
  IF v_open_blocker_count > 0 THEN
    RAISE EXCEPTION 'Open VAT return blocker(s) exist. Resolve blockers before creating VAT adjustment journals. Blockers: %',
      v_open_blocker_count;
  END IF;
