...
-- Compensating migration for the over-broad post-tracking projection.
--
-- Restores both canonical functions to the definitions introduced by
-- 20260726_supplier_rejection_canonical_status_final_v1.sql, with one narrow
-- presentation-only correction: after reconciliation is complete and tracking
-- exists, importer_status_label is "Tracking submitted" instead of falling back
-- to "Evidence attention".
--
-- This migration does not change importer_next_action, importer_complete_yn,
-- tracking allocation, buttons, balances, workflow ownership, customer output,
-- shipper output, or operational data.
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      WHEN b.reconciliation_state = 'complete' AND b.tracking_state IN ('allocation_incomplete', 'submitted') THEN 'Tracking submitted'
...
'Restored canonical supplier/reconciliation status from 20260726. Retired no-resubmission evidence remains audit-only.';

COMMENT ON FUNCTION public.order_audience_status_v1(uuid) IS
'Restored audience-safe projection with one status-label-only correction after tracking submission. Actions and completion facts pass through unchanged.';
