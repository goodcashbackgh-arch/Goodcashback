...
-- Corrective v3 over 20260606_platform_order_status_tracking_coverage_v2.
-- Universal rule: eligible_for_invoice_yn = 'N' is the default unresolved state.
-- A line is parked/non-physical only when it has an active supplier_invoice_line_resolutions
-- row. Positive/default N product lines must remain unresolved and must not be treated as
-- parked simply because eligible_for_invoice_yn = 'N'. This migration is read-model only.
...
BEGIN
  ...
  WITH order_scope AS (
    ...
    SELECT
      ...
      COUNT(*) FILTER (
        WHERE si.review_status IN ('pending_review', 'needs_action', 'duplicate_blocked')
           OR COALESCE(si.blocked_from_sage_yn, false) = true
           OR COALESCE(si.is_current_for_order, false) = false
      )::integer AS review_invoice_count
    FROM public.supplier_invoices si
    GROUP BY si.order_id
  ), supplier_line_state AS (
    SELECT
      si.order_id,
      sil.id AS supplier_invoice_line_id,
      lower(COALESCE(sil.eligible_for_invoice_yn::text, '')) IN ('y','yes','true','1') AS is_progressed_physical,
      sil.qty_confirmed IS NOT NULL AND sil.amount_confirmed IS NOT NULL AS has_confirmed_physical_values,
      EXISTS (
        SELECT 1
        ...
        WHERE r.supplier_invoice_line_id = sil.id
          ...
          AND r.active = true
      ) AS has_active_non_physical_resolution,
      EXISTS (
        SELECT 1
      ...
      ) AS has_active_dispute_link
    FROM public.supplier_invoices si
    JOIN public.supplier_invoice_lines sil ON sil.supplier_invoice_id = si.id
  ), supplier_lines AS (
    SELECT
      sls.order_id,
      COUNT(*)::integer AS total_supplier_lines,
      COUNT(*) FILTER (WHERE sls.is_progressed_physical)::integer AS physical_supplier_lines,
      COUNT(*) FILTER (WHERE sls.is_progressed_physical AND sls.has_confirmed_physical_values)::integer AS progressed_supplier_lines,
      COUNT(*) FILTER (
        WHERE NOT sls.is_progressed_physical
          AND NOT sls.has_active_non_physical_resolution
          AND NOT sls.has_active_dispute_link
      )::integer AS open_supplier_lines,
      COUNT(*) FILTER (WHERE sls.has_active_non_physical_resolution)::integer AS parked_supplier_lines,
      COUNT(*) FILTER (WHERE sls.has_active_dispute_link)::integer AS dispute_linked_supplier_lines,
      COUNT(*) FILTER (
        WHERE sls.is_progressed_physical
          AND NOT EXISTS (
            SELECT 1
            FROM public.order_tracking_line_allocations otla
            WHERE otla.supplier_invoice_line_id = sls.supplier_invoice_line_id
              AND COALESCE(otla.qty_allocated, 0) > 0
          )
      )::integer AS unallocated_physical_supplier_lines
    FROM supplier_line_state sls
    GROUP BY sls.order_id
  ), exceptions AS (
    SELECT
      d.order_id,
      COUNT(*) FILTER (
    ...
    SELECT
      ...
      COALESCE(sl.dispute_linked_supplier_lines, 0) AS dispute_linked_supplier_lines,
    ...
    SELECT
      ...
      CASE
        ...
        WHEN b.open_supplier_lines > 0 THEN 'supplier_reconciliation_incomplete'
  ...
  SELECT
    ...
    CASE s.current_stage
      ...
      WHEN 'tracking_allocation_incomplete' THEN 'Allocate all active tracking refs and progressed physical lines to shipment'
