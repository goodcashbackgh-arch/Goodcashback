...
-- Add supplier_credit_note to the existing Accounting Command Centre posting batch creator.
-- This is intentionally narrow: no evidence upload, no Sage API post, no customer credit note.
-- It only allows already-frozen supplier_credit_note snapshots to become normal posting batch rows.
...
BEGIN
  ...
  IF COALESCE(v_distinct_lane_count, 0) <> 1 THEN
    RAISE EXCEPTION 'Posting batches must be lane-specific. Select one lane only: customer_sales, supplier_goods_ap, supplier_credit_note, or shipper_ap.';
  END IF;

  IF v_batch_lane NOT IN ('customer_sales', 'supplier_goods_ap', 'supplier_credit_note', 'shipper_ap') THEN
