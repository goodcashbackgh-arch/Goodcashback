...
-- Shipping control overview must not show hardcoded export-evidence/master placeholders.
-- Final export evidence status comes from shipper_final_export_evidence_documents.
-- Master shipment is not an importer shipment-batch blocker in the current model.
...
BEGIN
  ...
  GROUP BY b.id, b.booking_ref, b.shipper_id, s.name, b.importer_id, i.trading_name, i.company_name,
           ...
           fee.has_accepted_current, fee.has_submitted_for_review, fee.has_rejected_resubmit_required, fee.document_count,
