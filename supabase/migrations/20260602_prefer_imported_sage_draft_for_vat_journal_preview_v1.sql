...
-- Keep all existing VAT source-line rules untouched.
-- Change only the Sage comparator selected by the existing journal preview/materialise path.
-- Imported Sage draft VAT totals are preferred over read-only diagnostic Sage reconstructions.
...
BEGIN
  ...
  ORDER BY
    CASE WHEN COALESCE(r.source_basis, '') LIKE 'sage_draft_vat_return_totals_import%' THEN 0 ELSE 1 END,
    r.created_at DESC
  LIMIT 1;

  IF v_recon.id IS NULL THEN
    RAISE EXCEPTION 'No Sage VAT comparator snapshot found for VAT return run. Import the Sage draft VAT return first.';
  END IF;
  ...
  LOOP
    ...
    v_gap_summary := v_gap_summary || jsonb_build_array(jsonb_build_object(
      ...
      'sage_comparator_snapshot_id', v_recon.id,
      'sage_comparator_source_basis', v_recon.source_basis
    ));
    ...
    v_proposals := v_proposals || jsonb_build_array(jsonb_build_object(
      ...
      'reason', COALESCE(v_line.adjustment_reason, 'Sage comparator does not match platform VAT position.'),
