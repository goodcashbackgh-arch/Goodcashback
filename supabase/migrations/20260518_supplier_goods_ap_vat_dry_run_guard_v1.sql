...
-- Runtime hardening for Contract v5 AP VAT-inclusive rule.
-- This replaces only the dry-run validation RPC so supplier_goods_ap cannot pass
-- validation unless the frozen payload preserves VAT-inclusive gross, net and VAT.
...
DO $$
BEGIN
  IF to_regprocedure('public.internal_validate_sage_posting_batch_payloads_v1(uuid)') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: public.internal_validate_sage_posting_batch_payloads_v1(uuid)';
  END IF;
  IF to_regprocedure('public.internal_sage_jsonb_num_v1(jsonb)') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: public.internal_sage_jsonb_num_v1(jsonb)';
  END IF;
END $$;
  ...
  WITH target AS (
    ...
    SET payload_validation_status = CASE
        ...
        response_payload_json = jsonb_build_object(
          ...
          'guard', 'supplier_goods_ap_vat_inclusive_guard_v1',
          'validated_at', now(),
