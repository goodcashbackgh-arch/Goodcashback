-- Correct the dispute status progression used by settlement-credit refund closure.
-- Settlement-credit calculation and ledger behavior remain unchanged.
...
DO $preflight$
BEGIN
  IF md5(pg_get_functiondef('public.staff_close_refund_exception_as_settlement_credit_v1(uuid,text,text)'::regprocedure))
     <> 'cdb3fc1a08bdd6edd56d4145dea23911'
  THEN
    RAISE EXCEPTION 'Unexpected refund settlement-credit authority definition; inspect before replacing.';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM public.status_transitions WHERE entity_type='dispute' AND from_status='raised' AND to_status='under_review' AND active)
     OR NOT EXISTS (SELECT 1 FROM public.status_transitions WHERE entity_type='dispute' AND from_status='under_review' AND to_status='approved_refund' AND active)
     OR NOT EXISTS (SELECT 1 FROM public.status_transitions WHERE entity_type='dispute' AND from_status='approved_refund' AND to_status='awaiting_refund_credit' AND active)
     OR NOT EXISTS (SELECT 1 FROM public.status_transitions WHERE entity_type='dispute' AND from_status='awaiting_refund_credit' AND to_status='refunded' AND active)
     OR NOT EXISTS (SELECT 1 FROM public.status_transitions WHERE entity_type='dispute' AND from_status='refunded' AND to_status='closed' AND active)
  THEN
    RAISE EXCEPTION 'Expected refund dispute transition path is not installed.';
  END IF;
END
$preflight$;
...
BEGIN
  IF v_def NOT ILIKE '%status=''under_review''%'
     ...
     OR v_def NOT ILIKE '%resolution_method=''refund''%'
  THEN RAISE EXCEPTION 'Refund settlement-credit status-path correction did not install.'; END IF;
END
$postflight$;
