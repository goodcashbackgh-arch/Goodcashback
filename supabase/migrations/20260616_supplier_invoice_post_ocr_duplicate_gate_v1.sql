-- =============================================================================
-- 20260616_supplier_invoice_post_ocr_duplicate_gate_v1.sql
-- Goodcashback — supplier invoice post-OCR duplicate gate
--
-- Purpose:
--   Ensure the deployed migration path enforces the duplicate rule that already
--   exists in the governing backend pack:
--     - same retailer_id OR matching OCR retailer name;
--     - same normalized OCR invoice ref;
--     - same OCR/header gross total within £0.01;
--     - ignore rejected/superseded prior rows so corrected resubmissions are allowed.
--
-- Behaviour:
--   If a duplicate is detected after OCR fields are saved, the new invoice is
--   marked duplicate_blocked, blocked from Sage, review-stamped, and a review flag
--   is raised for the operator where applicable.
-- =============================================================================
...
BEGIN
  ...
  NEW.reviewed_by_staff_id := COALESCE(NEW.reviewed_by_staff_id, v_staff_id);
...
DROP TRIGGER IF EXISTS trg_supplier_invoice_post_ocr_duplicate_gate ON public.supplier_invoices;
...
COMMENT ON FUNCTION public.trg_supplier_invoice_post_ocr_duplicate_gate() IS
'Post-OCR duplicate gate: blocks duplicate supplier invoices by normalized OCR invoice reference, OCR/header total and same retailer/matching OCR retailer name, while allowing rejected/superseded rows to be resubmitted.';
