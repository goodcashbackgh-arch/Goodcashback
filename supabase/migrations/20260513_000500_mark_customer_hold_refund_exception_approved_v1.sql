...
BEGIN
  ...
  IF v_existing_dispute_id IS NOT NULL THEN
    ...
    WHERE d.id = v_existing_dispute_id
      AND d.desired_outcome = 'refund';
...
UPDATE public.disputes d
SET refund_approved_by_staff_id = COALESCE(d.refund_approved_by_staff_id, h.reviewed_by_staff_id),
    refund_approved_at = COALESCE(d.refund_approved_at, h.reviewed_at, now())
