...
-- Phase 9/10 support: read-only posting batch history for Accounting Command Centre.
-- Scope: visibility only. No Sage API call, no dry-run, no posting.
...
BEGIN
  ...
  SELECT
    ...
    COUNT(r.id) FILTER (WHERE r.document_lane = 'shipper_ap' AND r.posting_status <> 'excluded') AS shipper_ap_count,
    b.created_at,
