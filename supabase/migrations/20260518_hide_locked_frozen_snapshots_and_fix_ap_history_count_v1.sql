...
-- Fix two Accounting Command Centre issues:
-- 1) A frozen snapshot already included in a non-cancelled posting batch must not
--    remain visible as frozen_ready_to_post in the workbench grid or batch creator.
-- 2) Batch history AP count must include supplier_goods_ap as well as shipper_ap.
--
...
BEGIN
  ...
  SELECT
    ...
    COUNT(r.id) FILTER (WHERE r.document_lane IN ('shipper_ap','supplier_goods_ap') AND r.posting_status <> 'excluded')::bigint AS shipper_ap_count,
...
BEGIN
  ...
  WITH live_ready AS (
    ...
    WHERE (v_lane = 'all' OR ar.out_document_lane = v_lane)
      ...
      AND (
        v_search IS NULL
        OR lower(concat_ws(' ', ar.out_order_ref, ar.out_reference_text, ar.out_counterparty_name, ar.out_document_lane, ar.out_document_type, ar.out_batch_ref, ar.out_idempotency_key, ar.out_source_id::text, ar.out_snapshot_id::text, ar.out_booking_ref)) LIKE ('%' || v_search || '%')
      )
  ), queued AS (
    SELECT s.*
    FROM searched s
  ...
  FROM queued q
  ORDER BY CASE q.out_candidate_kind WHEN 'freeze' THEN 0 WHEN 'revalidate' THEN 1 ELSE 2 END, q.out_document_lane, q.out_order_ref NULLS LAST, q.out_source_id NULLS LAST, q.out_snapshot_id NULLS LAST
  LIMIT v_max_rows;
END;
$$;
...
BEGIN
  ...
  WITH frozen_rows AS (
    ...
    WHERE (v_lane = 'all' OR ar.out_document_lane = v_lane)
      AND (v_posting_gate = 'all' OR ar.out_posting_gate = v_posting_gate)
      AND (v_search IS NULL OR lower(concat_ws(' ', ar.out_queue_row_id, ar.out_order_ref, ar.out_reference_text, ar.out_counterparty_name, ar.out_document_lane, ar.out_document_type, ar.out_batch_ref, ar.out_idempotency_key, ar.out_source_id::text)) LIKE ('%' || v_search || '%'))
  ), summary AS (
    ...
    SELECT c.*
    FROM counted c
    ORDER BY CASE c.out_work_queue WHEN 'live_ready_not_frozen' THEN 0 WHEN 'requires_revalidation' THEN 1 WHEN 'blocked_before_posting' THEN 2 WHEN 'posting_failed' THEN 3 WHEN 'frozen_ready_to_post' THEN 4 WHEN 'posted' THEN 5 ELSE 9 END, c.out_document_lane, c.out_order_ref NULLS LAST, c.out_reference_text NULLS LAST, c.out_source_id NULLS LAST
    LIMIT v_limit OFFSET v_offset
  )
  SELECT
    p.out_row_kind, p.out_work_queue, p.out_queue_row_id, p.out_snapshot_id, p.out_source_table, p.out_source_id, p.out_document_lane, p.out_document_type, p.out_order_id, p.out_order_ref, p.out_shipment_batch_id, p.out_booking_ref, p.out_counterparty_name, p.out_amount_gbp, p.out_currency_code, p.out_reference_text, p.out_mapping_state, p.out_payload_state, p.out_freeze_state, p.out_revalidation_state, p.out_posting_gate, p.out_sage_status, p.out_batch_ref, p.out_idempotency_key, p.out_row_created_at, p.out_row_age_hours, p.out_next_action, p.out_next_action_href, p.out_selectable, p.out_selection_group, p.out_blocker, p.out_warning, p.out_total_count, summary.summary_counts
  FROM paged p
  CROSS JOIN summary;
END;
$$;
