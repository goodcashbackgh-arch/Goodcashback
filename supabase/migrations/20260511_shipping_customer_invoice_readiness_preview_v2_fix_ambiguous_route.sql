...
BEGIN
  ...
  WITH src AS (
    ...
    SELECT
      ...
      END AS calc_goods_amount_gbp,
      COALESCE(s.allocated_shipping_amount, 0) AS calc_shipping_amount_gbp,
      CASE
      ...
      END AS calc_line_total_gbp
    FROM src s
    CROSS JOIN route r
  ), totals AS (
    SELECT
      COALESCE(SUM(lc.calc_goods_amount_gbp), 0) AS total_goods_amount_gbp,
      COALESCE(SUM(lc.calc_shipping_amount_gbp), 0) AS total_shipping_amount_gbp,
      COALESCE(SUM(lc.calc_line_total_gbp), 0) AS total_invoice_amount_gbp,
      jsonb_agg(
        jsonb_build_object(
          ...
          'description', lc.item_description,
          'goods_amount_gbp', lc.calc_goods_amount_gbp,
          'shipping_amount_gbp', lc.calc_shipping_amount_gbp,
          'total_line_amount_gbp', lc.calc_line_total_gbp,
  ...
  SELECT
    ...
    'T0'::text AS vat_code,
    t.total_invoice_amount_gbp AS proposed_amount_gbp,
    t.total_goods_amount_gbp AS proposed_goods_amount_gbp,
    t.total_shipping_amount_gbp AS proposed_shipping_amount_gbp,
    ...
    lc.calc_goods_amount_gbp AS goods_amount_gbp,
    lc.calc_shipping_amount_gbp AS shipping_amount_gbp,
    lc.calc_line_total_gbp AS total_line_amount_gbp,
    CASE
