...
-- Stop rather than overwrite definitions that were not reviewed for this build.
do $drift$
declare
  v_md5 text;
begin
  select md5(pg_get_functiondef('public.create_replacement_child_order(uuid,uuid,uuid,text)'::regprocedure))
    into v_md5;
  ...
  select md5(pg_get_functiondef('public.order_has_open_child_exceptions(uuid)'::regprocedure))
    into v_md5;
  ...
  where schemaname = 'public'
    and viewname = 'order_reconciliation_vw';
  ...
  -- Protected authorities: verify only; this migration must not replace them.
  select md5(pg_get_functiondef('public.approve_vat_release(uuid,uuid,jsonb)'::regprocedure)) into v_md5;
  if v_md5 is distinct from '13491a2d250a480ebb1ac607ce7acce5' then
    raise exception 'Build 4 drift stop: approve_vat_release changed (%).', v_md5;
  end if;

  select md5(pg_get_functiondef('public.mark_order_accounting_release_ready(uuid,uuid)'::regprocedure)) into v_md5;
  if v_md5 is distinct from 'dacaf00c6470a626cfc2d7e7aac2ccb8' then
    raise exception 'Build 4 drift stop: mark_order_accounting_release_ready changed (%).', v_md5;
  end if;

  select md5(pg_get_functiondef('public.recompute_order_status(uuid)'::regprocedure)) into v_md5;
  if v_md5 is distinct from '110d55541d4f729ff9331e23515fb563' then
    raise exception 'Build 4 drift stop: recompute_order_status changed (%).', v_md5;
  end if;

  select md5(pg_get_functiondef('public.physical_remedy_allocation_guard_v1()'::regprocedure)) into v_md5;
  if v_md5 is distinct from '32e1d3eb9161cdc3e09114edb8c0d3c0' then
    raise exception 'Build 4 drift stop: physical_remedy_allocation_guard_v1 changed (%).', v_md5;
  end if;
  ...
  if v_md5 is distinct from '3c5067f31d4f2112207e02d1f307e233' then
    raise exception 'Build 4 drift stop: physical_remedy_sequence_guard_v1 changed (%).', v_md5;
  end if;

  select md5(pg_get_functiondef('public.physical_remedy_terminal_immutability_guard_v1()'::regprocedure)) into v_md5;
  if v_md5 is distinct from 'a7aa361f066b454a6f9c4f9b81734834' then
    raise exception 'Build 4 drift stop: physical_remedy_terminal_immutability_guard_v1 changed (%).', v_md5;
  end if;

  select md5(pg_get_functiondef('public.enforce_status_transition()'::regprocedure)) into v_md5;
  if v_md5 is distinct from '5fc40897ac22a4adae838ecc6a3e1cb9' then
    raise exception 'Build 4 drift stop: enforce_status_transition changed (%).', v_md5;
  end if;
  ...
  if v_md5 is distinct from '497230d0cf04001f37c5e805cdd8da25' then
    raise exception 'Build 4 drift stop: enforce_order_locks changed (%).', v_md5;
  end if;
end
$drift$;
...
begin
  ...
  if v_physical_remedy_id is not null then
    ...
    if v_physical_remedy.id is null
       ...
       or v_physical_remedy.supplier_cost_mode not in (
         'free_replacement',
         'charged_repurchase',
...
comment on function public.create_replacement_child_order(uuid,uuid,uuid,text) is
  'Hardened replacement-child authority. Preserves legacy compatibility and, when present, exact approved physical-remedy provenance.';
...
comment on view public.order_reconciliation_vw is
  'Canonical order reconciliation using approved, unblocked, non-superseded supplier invoice identity. Public column contract preserved.';
...
canonical as (
  select * from public.order_reconciliation_vw
)
select
  c.order_id,
...
comment on view public.order_reconciliation_anomalies_v1 is
  'Read-only Build 4 anomaly model exposing authoritative over-progression, non-authoritative eligible evidence and raw evidence exceeding order baselines.';
...
-- The canonical view replacement must retain its exact public column order.
do $contract$
declare
  v_columns text[];
begin
