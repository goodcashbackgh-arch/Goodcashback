...
-- Second fix for supplier_goods_ap freeze ambiguity.
-- The previous fix qualified RETURNING columns, but ON CONFLICT (idempotency_key)
-- can still conflict with the PL/pgSQL output column named idempotency_key.
-- This replaces the conflict target with the named unique constraint.
