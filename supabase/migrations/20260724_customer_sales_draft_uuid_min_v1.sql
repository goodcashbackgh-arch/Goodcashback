...
-- =============================================================================
-- Customer sales draft UUID aggregate hotfix.
--
-- The current Mini-build 3 draft RPC uses MIN(shipment_batch_id) in two places.
-- PostgreSQL has no built-in MIN(uuid) aggregate. Reuse the established
-- UUID-safe repository pattern: ARRAY_AGG(DISTINCT uuid ORDER BY uuid)[1].
--
-- Only the two representative shipment-batch selections change. All source
-- membership, commercial-parent grouping, amounts, main/supplementary routing,
-- duplicate-draft controls, release-ledger writes, Sage and VAT behaviour remain
-- exactly on the existing Mini-build 3 route.
-- =============================================================================
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        jsonb_build_object(
          ...
          'shipment_batch_id', (ARRAY_AGG(DISTINCT rs.shipment_batch_id ORDER BY rs.shipment_batch_id))[1],
