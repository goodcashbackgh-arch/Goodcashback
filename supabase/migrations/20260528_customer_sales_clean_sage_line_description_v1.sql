...
-- Narrow fix only: clean the customer/Sage sales invoice line description.
-- Keep the same function signature, return shape, calculations, gates and source fields.
-- Remove order ref / booking ref from the Sage line description because those values
-- are already carried on the invoice header/reference/notes and source metadata.
...
BEGIN
  ...
  WITH src AS (
    ...
    SELECT
      ...
      COALESCE((SELECT s3.customer_recharge_route FROM src s3 WHERE s3.customer_recharge_route IS NOT NULL LIMIT 1), 'sales_invoice_route_not_resolved')::text AS resolved_customer_recharge_route,
      COALESCE((SELECT s4.sales_invoice_state FROM src s4 WHERE s4.sales_invoice_state IS NOT NULL LIMIT 1), 'sales_invoice_state_not_resolved')::text AS resolved_sales_invoice_state
  ), line_calc AS (
    SELECT
      s.*,
      ...
      CASE WHEN r.resolved_invoice_type = 'supplementary' THEN 0::numeric ELSE COALESCE(s.adjusted_goods_basis_gbp, 0) END AS calc_goods_evidence_gbp,
      ...
      COALESCE(NULLIF(BTRIM(s.item_description), ''), 'Goods')::text AS source_product_description,
      CASE
        WHEN r.resolved_invoice_type = 'supplementary' THEN 'Supplementary export shipping charge'
        ELSE COALESCE(NULLIF(BTRIM(s.item_description), ''), 'Goods')
      END::text AS customer_payload_description
    FROM src s
    CROSS JOIN route r
  ), totals AS (
    SELECT
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'source_product_description', lc.source_product_description,
          ...
          'presentation', 'principal_export_sale_line_from_allocated_supplier_goods',
        ...
        ) ORDER BY lc.order_ref NULLS LAST, lc.booking_ref NULLS LAST, lc.source_product_description NULLS LAST
