...
-- Extend the established supplier-invoice review lane only. This migration does
-- not update invoice totals, orders, adjustments, funding, banking, progression,
-- shipment, Sage, VAT or accounting source data.
DO $$
BEGIN
...
-- Fail closed: alter only the single existing flag-type check. Do not remove an
-- unknown future/composite check merely because it also mentions flag_type.
DO $$
DECLARE
...
BEGIN
  SELECT
    count(*)::integer,
    min(c.conname),
    min(pg_get_constraintdef(c.oid))
  ...
  WHERE c.conrelid = 'public.supplier_invoice_review_flags'::regclass
    AND c.contype = 'c'
    AND position('flag_type' in lower(pg_get_constraintdef(c.oid))) > 0;

  IF v_constraint_count <> 1 THEN
    RAISE EXCEPTION 'Expected exactly one supplier-invoice flag_type check; found %. No schema change applied.', v_constraint_count;
  END IF;

  IF v_constraint_definition NOT ILIKE '%order_bundle_limit_breach%' THEN
    IF v_constraint_definition NOT ILIKE '%invoice_total_mismatch%'
       OR v_constraint_definition NOT ILIKE '%ocr_unclear%'
       OR v_constraint_definition NOT ILIKE '%wrong_invoice%'
       OR v_constraint_definition NOT ILIKE '%delivery_discount_query%'
       OR v_constraint_definition NOT ILIKE '%manual_line_needed%'
       OR v_constraint_definition NOT ILIKE '%other%' THEN
      RAISE EXCEPTION 'Existing flag_type check is not the expected contract. No schema change applied.';
    END IF;
...
-- Preserve the existing view shape and routing equations. Only serious-flag
-- membership is extended to the existing delivery/discount and order-limit flags.
...
WITH active_summary AS (
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE f.status IN ('open','under_review'))::int AS open_review_flag_count,
    COUNT(*) FILTER (
...
-- The trigger covers every future operator upload. The one-time backfill is
-- intentionally restricted to the current test order so historic live orders are
-- not added to a new queue without a fresh upload action.
