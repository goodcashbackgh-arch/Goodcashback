...
-- One atomic, audited supervisor correction for submitted and OCR credit-note
-- header values. It deliberately leaves files, OCR raw JSON, extracted lines,
-- accounting coding, approval, Sage payloads and posting rows untouched.
...
BEGIN
  ...
  IF coalesce(v_submission.ocr_status, '') IS DISTINCT FROM 'completed' THEN
  ...
  IF v_credit_note_ref IS NULL THEN
    RAISE EXCEPTION 'Submitted credit-note reference is required.';
  END IF;
  IF p_credit_note_date IS NULL THEN
    RAISE EXCEPTION 'Submitted credit-note date is required.';
  END IF;
  IF v_expected_amount <= 0 THEN
    RAISE EXCEPTION 'Expected credit-note total must be above zero.';
  END IF;
  IF v_ocr_credit_note_ref IS NULL THEN
    RAISE EXCEPTION 'OCR credit-note reference is required after supervisor correction.';
  END IF;
  IF v_ocr_retailer_name IS NULL THEN
    RAISE EXCEPTION 'OCR retailer name is required after supervisor correction.';
  END IF;
  IF p_ocr_credit_note_date IS NULL THEN
    RAISE EXCEPTION 'OCR credit-note date is required after supervisor correction.';
  END IF;
  IF v_ocr_total <= 0 THEN
    RAISE EXCEPTION 'OCR credit-note total must be above zero.';
  END IF;

  SELECT d.id, d.order_id, r.name::text AS retailer_name
    INTO v_dispute
  ...
  v_expected_retailer_name := nullif(btrim(coalesce(v_dispute.retailer_name, '')), '');
  ...
  SELECT count(*)::integer,
         round(coalesce(sum(coalesce(l.amount_gbp, 0)), 0)::numeric, 2)
  ...
  IF v_submission.credit_note_ref IS DISTINCT FROM v_credit_note_ref THEN
    v_changed_fields := array_append(v_changed_fields, 'credit_note_ref');
  END IF;
  IF v_submission.credit_note_date IS DISTINCT FROM p_credit_note_date THEN
    v_changed_fields := array_append(v_changed_fields, 'credit_note_date');
  END IF;
  IF v_submission.expected_credit_note_total_gbp IS DISTINCT FROM v_expected_amount THEN
    v_changed_fields := array_append(v_changed_fields, 'expected_credit_note_total_gbp');
  END IF;
  IF v_submission.ocr_credit_note_ref IS DISTINCT FROM v_ocr_credit_note_ref THEN
    v_changed_fields := array_append(v_changed_fields, 'ocr_credit_note_ref');
  END IF;
  IF v_submission.ocr_retailer_name IS DISTINCT FROM v_ocr_retailer_name THEN
    v_changed_fields := array_append(v_changed_fields, 'ocr_retailer_name');
  END IF;
  IF v_submission.ocr_credit_note_date IS DISTINCT FROM p_ocr_credit_note_date THEN
    v_changed_fields := array_append(v_changed_fields, 'ocr_credit_note_date');
  END IF;
  IF v_submission.ocr_credit_note_total_gbp IS DISTINCT FROM v_ocr_total THEN
    v_changed_fields := array_append(v_changed_fields, 'ocr_credit_note_total_gbp');
  END IF;
  ...
  UPDATE public.dispute_refund_evidence_submissions s
     SET credit_note_ref = v_credit_note_ref,
         ...
         evidence_control_status = CASE
           WHEN v_match_status = 'matched_ready_to_release' THEN 'credit_note_ocr_matched_ready'
           ELSE 'credit_note_ocr_review_required'
         END,
         supplier_readiness_route = CASE
           WHEN v_match_status = 'matched_ready_to_release' THEN 'supplier_credit_note_ready_to_release'
           ELSE 'supplier_credit_note_review_required'
         END,
         supplier_control_status = CASE
           WHEN v_match_status = 'matched_ready_to_release' THEN 'not_released'
           ELSE 'blocked'
         END,
         supplier_approval_status = CASE
           WHEN v_match_status = 'matched_ready_to_release' THEN 'pending'
           ELSE 'blocked'
         END,
         supervisor_review_status = CASE
           WHEN v_match_status = 'matched_ready_to_release' THEN 'not_required'
           ELSE 'pending_review'
         END
...
'Atomically corrects submitted and OCR credit-note header fields before release/approval/Sage freeze, recalculates matching controls, and writes an immutable dispute audit message. Does not change files, lines, coding or Sage data.';
