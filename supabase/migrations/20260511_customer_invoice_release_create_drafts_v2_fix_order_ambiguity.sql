...
BEGIN
  ...
  WITH selected_batches AS (
    ...
    SELECT
      op.payload_shipment_batch_id AS result_shipment_batch_id,
      op.payload_order_id AS result_order_id,
      op.payload_order_ref AS result_order_ref,
      op.payload_booking_ref AS result_booking_ref,
      op.payload_invoice_type AS result_invoice_type,
      CASE
      ...
      COALESCE(ins.created_sales_invoice_id, op.payload_existing_draft_id, op.payload_existing_posted_id) AS result_sales_invoice_id,
      op.payload_amount_gbp AS result_amount_gbp,
      CASE
    ...
    SELECT
      q.shipment_batch_id AS result_shipment_batch_id,
      NULL::uuid AS result_order_id,
      q.first_order_ref AS result_order_ref,
      q.booking_ref AS result_booking_ref,
      q.proposed_invoice_type AS result_invoice_type,
      ('skipped_' || COALESCE(q.readiness_status, 'not_ready'))::text AS result_status,
      NULL::uuid AS result_sales_invoice_id,
      COALESCE(q.proposed_amount_gbp, 0) AS result_amount_gbp,
      ('Not created: ' || COALESCE(q.readiness_status, 'not ready'))::text AS result_message
    FROM selected_queue q
    WHERE q.readiness_status IS DISTINCT FROM 'ready_to_create_draft'
  )
  SELECT
    rr.result_shipment_batch_id,
    rr.result_order_id,
    rr.result_order_ref,
    rr.result_booking_ref,
    rr.result_invoice_type,
    rr.result_status,
    rr.result_sales_invoice_id,
    rr.result_amount_gbp,
    rr.result_message
  FROM ready_results rr
  UNION ALL
  SELECT
    nr.result_shipment_batch_id,
    nr.result_order_id,
    nr.result_order_ref,
    nr.result_booking_ref,
    nr.result_invoice_type,
    nr.result_status,
    nr.result_sales_invoice_id,
    nr.result_amount_gbp,
    nr.result_message
  FROM non_ready_results nr
