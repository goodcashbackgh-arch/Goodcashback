...
-- Projection-only correction.
-- Replaces only public.internal_customer_invoice_release_queue_v1().
-- No operational-row DML. No unrelated function fingerprint assumptions.

DO $preflight$
DECLARE
  v_queue_definition text;
  v_queue_compact text;
BEGIN
  ...
  v_queue_compact := regexp_replace(lower(v_queue_definition), '[[:space:]]+', ' ', 'g');

  IF md5(v_queue_definition) IS DISTINCT FROM '823d4488e24c335596d55351c3e752c3'
     OR strpos(v_queue_compact, 'left join public.sales_invoices invoice') = 0
     OR strpos(v_queue_compact, 'count(*)::integer as line_count') = 0
     OR strpos(v_queue_definition, 'internal_customer_sales_release_exact_clean_proof_v1') = 0
  THEN
    RAISE EXCEPTION 'Queue is not the diagnosed live definition; aborting without changes.';
  END IF;
END
$preflight$;
