...
-- Fix Accounting Command Centre lifecycle grid: batch-history rows should not show
-- Map = mapping_unknown when the local batch row has already been included or
-- dry-run/local validated. For historical batch rows, the meaningful state is
-- that the row is locked to the batch payload/mapping snapshot; target-fact
-- completeness is still checked on the batch detail page.
--
-- This patch keeps the previous cancelled-row re-freeze behaviour intact and
-- only tightens the batch_rows mapping_state rule.
...
BEGIN
  ...
  WITH frozen_rows AS (
    ...
    SELECT
      ...
      CASE
        WHEN br.payload_validation_status = 'dry_run_validated' THEN 'mapping_batched'
        WHEN br.posting_status IN ('included', 'validated', 'posted') THEN 'mapping_batched'
        WHEN br.request_payload_json ? 'resolved_lines' THEN 'mapping_batched'
      ...
      CASE
        WHEN (br.posting_status = 'cancelled' OR b.status = 'cancelled' OR b.batch_status = 'superseded')
         AND br.document_lane IN ('customer_sales','supplier_goods_ap','shipper_ap')
         AND NOT EXISTS (
          SELECT 1
         ...
         ) THEN 'Select then freeze visible to re-freeze source'
        ELSE 'Open posting batch'
      END::text AS out_next_action,
      ('/internal/accounting-command-centre/batches/' || b.id::text)::text AS out_next_action_href,
      (
        (br.posting_status = 'cancelled' OR b.status = 'cancelled' OR b.batch_status = 'superseded')
        AND br.document_lane IN ('customer_sales','supplier_goods_ap','shipper_ap')
        AND NOT EXISTS (
          SELECT 1
          FROM public.sage_posting_batch_rows br2
