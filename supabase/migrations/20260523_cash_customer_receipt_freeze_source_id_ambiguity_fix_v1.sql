...
-- column reference "source_id" is ambiguous.
-- Root cause: RETURNS TABLE defines an output variable named source_id, and the
-- inserted CTE used unqualified RETURNING id, source_id. This patch qualifies
-- the INSERT RETURNING columns only. No table/data/UI/Sage changes.
...
BEGIN
  ...
  WITH selected AS (
    ...
    RETURNING
      cash_posting_snapshots.id,
      cash_posting_snapshots.source_id,
      cash_posting_snapshots.validation_status,
      cash_posting_snapshots.short_reference,
      cash_posting_snapshots.amount_gbp
  )
  SELECT
