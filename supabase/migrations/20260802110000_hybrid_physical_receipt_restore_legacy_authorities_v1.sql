...
CREATE TEMP TABLE b4_correction_snapshot (
...
-- Exact reviewed Build-4 reconciliation reference. This temporary view is used
-- only to prove the current authority before any change.
CREATE TEMP VIEW b4_expected_reconciliation AS
...
dispute_line_totals AS (
  SELECT d.order_id,
         COALESCE(SUM(CASE
           ...
           THEN dl.qty_impact ELSE 0 END), 0::bigint) AS qty_resolved_noninvoiceable,
         COALESCE(SUM(CASE
           WHEN dl.line_status = 'resolved'
            AND asl.supplier_invoice_line_id IS NULL
           THEN dl.amount_impact_gbp ELSE 0::numeric END), 0::numeric) AS amount_resolved_dispute_gbp
...
resolved_nonphysical AS (
  SELECT r.order_id,
         COALESCE(SUM(CASE r.financial_type
           ...
           ELSE 0::numeric END), 0::numeric) AS signed_nonphysical_amount_gbp
...
reconciled AS (
  SELECT o.id AS order_id,
         ...
         EXISTS (
           SELECT 1 FROM authoritative_supplier_lines released
...
-- Exact reviewed current anomaly reference. It proves the existing anomaly body
-- before the single authorised dependency redirection.
CREATE TEMP VIEW b4_expected_anomalies AS
...
DECLARE
  ...
  v_trigger_oid oid;
  v_actual_view text;
  v_expected_view text;
  v_actual_anomaly text;
  v_expected_anomaly text;
  v_columns text[];
BEGIN
  ...
  END IF;
  IF to_regclass('public.order_reconciliation_vw') IS NULL
     OR to_regclass('public.order_reconciliation_anomalies_v1') IS NULL THEN
    RAISE EXCEPTION 'Expected Build-4 reconciliation objects are missing.';
  END IF;
  IF to_regclass('public.order_reconciliation_v2_vw') IS NOT NULL THEN
    RAISE EXCEPTION 'order_reconciliation_v2_vw already exists; inspect rather than replace.';
  END IF;
  ...
  IF v_guard_fingerprint IS DISTINCT FROM '32e1d3eb9161cdc3e09114edb8c0d3c0' THEN
    RAISE EXCEPTION 'Unexpected Build-2 guard baseline: %', v_guard_fingerprint;
  END IF;
  ...
  IF v_trigger_oid IS DISTINCT FROM v_guard_oid THEN
    RAISE EXCEPTION 'Hybrid remedy trigger is not bound to the reviewed Build-2 guard object.';
  END IF;

  SELECT pg_get_viewdef('public.order_reconciliation_vw'::regclass, false),
         pg_get_viewdef('b4_expected_reconciliation'::regclass, false)
  INTO v_actual_view, v_expected_view;

  IF md5(v_actual_view) IS DISTINCT FROM md5(v_expected_view) THEN
    RAISE EXCEPTION 'Current order_reconciliation_vw is not the reviewed Build-4 authority.';
  END IF;

  SELECT pg_get_viewdef('public.order_reconciliation_anomalies_v1'::regclass, false),
         pg_get_viewdef('b4_expected_anomalies'::regclass, false)
  INTO v_actual_anomaly, v_expected_anomaly;

  IF md5(v_actual_anomaly) IS DISTINCT FROM md5(v_expected_anomaly) THEN
    RAISE EXCEPTION 'Current anomaly view is not the reviewed Build-4 definition.';
  END IF;
  ...
  INSERT INTO b4_correction_snapshot(snapshot_key, snapshot_value) VALUES
    ('build2_guard_oid', v_guard_oid::text),
    ('build4_view_definition', v_actual_view),
    ('legacy_view_owner', (
...
DROP VIEW b4_expected_anomalies;
DROP VIEW b4_expected_reconciliation;
...
BEGIN
  ...
  IF NEW.dispute_line_id IS NOT NULL THEN
    SELECT dispute_line.supplier_invoice_line_id,
           dispute_row.order_id,
           dispute_row.id
    INTO v_dispute_line_supplier_id,
         v_dispute_order_id,
         v_dispute_id
...
DECLARE
  ...
  v_v2_owner text;
BEGIN
  IF md5((SELECT p.prosrc FROM pg_proc p
          WHERE p.oid = 'public.physical_remedy_allocation_guard_v1()'::regprocedure))
       IS DISTINCT FROM 'd51a2cddb589135e1611640033be6d83' THEN
    RAISE EXCEPTION 'Restored foundation v1 source fingerprint is wrong.';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Restored foundation v1 metadata is wrong.';
  END IF;
  ...
  IF v_v2_fingerprint IS DISTINCT FROM '32e1d3eb9161cdc3e09114edb8c0d3c0' THEN
    RAISE EXCEPTION 'Versioned Build-2 v2 changed: %', v_v2_fingerprint;
  END IF;
  ...
  IF v_trigger_oid IS DISTINCT FROM v_v2_oid THEN
    RAISE EXCEPTION 'Hybrid remedy trigger did not retain the renamed v2 object identity.';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Restored v1 was unexpectedly attached to a trigger.';
  END IF;
  ...
  SELECT COUNT(*)::integer INTO v_public_execute_count
  FROM pg_proc p
  ...
  IF v_public_execute_count <> 0
     ...
     OR has_function_privilege('authenticated','public.physical_remedy_allocation_guard_v2()','EXECUTE') THEN
    RAISE EXCEPTION 'One or more internal remedy guards remain directly executable.';
  END IF;
END
$guard_proof$;
...
COMMENT ON VIEW public.order_reconciliation_v2_vw IS
  'Versioned Build-4 authoritative-supplier reconciliation. Existing platform callers remain on order_reconciliation_vw.';
...
DECLARE
  ...
  v_actual text;
BEGIN
  SELECT snapshot_value INTO v_expected
  FROM b4_correction_snapshot WHERE snapshot_key = 'build4_view_definition';
  ...
  IF md5(v_actual) IS DISTINCT FROM md5(v_expected) THEN
    RAISE EXCEPTION 'Build-4 reconciliation semantics changed while moving to v2.';
  END IF;
END
$v2_view_proof$;

-- Exact pre-Build-4 definition from
-- 20260724_order_reconciliation_signed_nonphysical_v1.sql.
...
SELECT o.id AS order_id,
       ...
       o.total_qty_declared
         ...
         - COALESCE(SUM(CASE WHEN dl.line_status = 'resolved' THEN dl.qty_impact ELSE 0 END), 0) AS qty_unresolved,
       o.order_total_gbp_declared AS amount_target_gbp,
       ...
       COALESCE(SUM(CASE WHEN dl.line_status = 'resolved' THEN dl.amount_impact_gbp ELSE 0 END), 0)
         + COALESCE(MAX(rn.signed_nonphysical_amount_gbp), 0) AS amount_resolved_noninvoiceable_gbp,
       o.order_total_gbp_declared
         - COALESCE(SUM(CASE WHEN sil.eligible_for_invoice_yn = 'Y' THEN sil.amount_confirmed ELSE 0 END), 0)
         - COALESCE(SUM(CASE WHEN dl.line_status = 'resolved' THEN dl.amount_impact_gbp ELSE 0 END), 0)
         - COALESCE(MAX(rn.signed_nonphysical_amount_gbp), 0) AS amount_unresolved_gbp,
       CASE WHEN EXISTS (
         SELECT 1
...
COMMENT ON VIEW public.order_reconciliation_anomalies_v1 IS
  'Read-only Build 4 anomaly model using order_reconciliation_v2_vw.';
...
BEGIN
  ...
  IF v_columns IS DISTINCT FROM ARRAY[
    ...
    RAISE EXCEPTION 'Legacy reconciliation contract changed: %', v_columns;
  END IF;
  ...
  IF v_owner IS DISTINCT FROM (
       SELECT snapshot_value FROM b4_correction_snapshot
       WHERE snapshot_key = 'legacy_view_owner'
     )
     OR v_acl IS DISTINCT FROM (
       SELECT snapshot_value FROM b4_correction_snapshot
       WHERE snapshot_key = 'legacy_view_acl'
     ) THEN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Anomaly dependency was not redirected exclusively to v2.';
  END IF;
END
$final_proof$;
...
BEGIN
  IF EXISTS (
    SELECT dependent_class, dependent_oid, dependent_subid,
           dependency_type, dependent_identity, referenced_subid
    FROM b4_legacy_dependents_before
    WHERE dependent_identity NOT LIKE '%order_reconciliation_anomalies_v1%'
    EXCEPT
    SELECT d.classid::regclass::text, d.objid, d.objsubid,
           d.deptype, pg_describe_object(d.classid, d.objid, d.objsubid), d.refobjsubid
    FROM pg_depend d
    WHERE d.refobjid = 'public.order_reconciliation_vw'::regclass
      AND pg_describe_object(d.classid, d.objid, d.objsubid)
          NOT LIKE '%order_reconciliation_anomalies_v1%'
  ) OR EXISTS (
    SELECT d.classid::regclass::text, d.objid, d.objsubid,
           d.deptype, pg_describe_object(d.classid, d.objid, d.objsubid), d.refobjsubid
    FROM pg_depend d
