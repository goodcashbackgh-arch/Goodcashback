...
-- Make cancelled/superseded batch history rows operationally recoverable.
--
-- The Accounting Command Centre default remains current/actionable work only.
-- In Queue = Cancelled/Superseded or All documents, cancelled history rows for
-- supported source lanes become selectable ONLY when the same exact source row
-- is not locked by another active/non-cancelled posting batch.
--
-- Then the existing top buttons can be used:
--   - Freeze visible customer sales
--   - Freeze visible supplier goods AP
--   - Freeze visible shipper AP
--
-- This avoids batching audit rows directly. It re-freezes the original source
-- from the current resolver, then the user creates a fresh posting batch.
--
