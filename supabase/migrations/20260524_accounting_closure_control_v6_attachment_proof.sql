...
-- Accounting Closure Control v6.
-- Read-only refinement: prove attachment status from sage_posting_snapshots.sage_attachment_*.
-- No Sage API calls. No posting. No allocation. No endpoint expansion.

SET LOCAL lock_timeout = '15s';
SET LOCAL statement_timeout = '0';

CREATE OR REPLACE FUNCTION public.internal_accounting_closure_control_rows_v3(
...
BEGIN
  ...
  WITH base AS (
    SELECT *
    FROM public.internal_accounting_closure_control_rows_v2(p_lane, p_state, p_search, p_limit, p_offset)
  ), patched AS (
    SELECT
      b.*,
  ...
  SELECT
    ...
    p.trace_json || jsonb_build_object(
      'attachment_proof_version', 'v6_attachment_proof',
...
REVOKE ALL ON FUNCTION public.internal_accounting_closure_control_rows_v3(text, text, text, integer, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.internal_accounting_closure_control_rows_v3(text, text, text, integer, integer) TO authenticated;

COMMENT ON FUNCTION public.internal_accounting_closure_control_rows_v3(text, text, text, integer, integer) IS
'Accounting Closure Control v6 read-only wrapper: resolves AP/CN attachment_state from sage_posting_snapshots.sage_attachment_* fields.';
