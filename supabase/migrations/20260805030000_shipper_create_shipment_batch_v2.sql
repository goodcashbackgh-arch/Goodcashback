...
-- Additive exact-quantity shipment creation authority.
-- Reuses the existing shipment batch/package/membership tables.
-- Does not replace v1 and does not alter downstream COS/BOL/POD authorities.
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'Protected shipment authority changed during additive v2 creation installation.';
  END IF;
END
$postflight$;
