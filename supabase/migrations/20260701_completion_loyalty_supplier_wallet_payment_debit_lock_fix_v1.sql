...
-- Completion loyalty supplier wallet payment debit lock fix v1.
-- The candidate resolver must follow the same applied-credit trace used by the
-- release reversal controls: released match -> credit ledger -> debit ledger row
-- -> order_funding_events.credit_applied. That trace does not exclude locked
-- debit rows. Applied debit rows may be locked for audit/control reasons after
-- application; excluding them incorrectly hides valid supplier-wallet payment
-- candidates. This patch only replaces the candidate read-model function.
-- It does not touch DVA_CASH_BANK_ACCOUNT, cash posting batches, Sage posters,
-- supplier AP posting, or existing DVA reconciliation routes.
