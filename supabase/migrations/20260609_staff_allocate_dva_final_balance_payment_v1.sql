...
-- Staff RPC for allocating one IN DVA/card statement line to final balance.
-- Does not call staff_reconcile_dva_line_to_order.
-- Does not create importer credit, overfunding, Sage payload, or VAT records.
...
BEGIN
  ...
  FROM public.internal_order_final_sale_settlement_v1(p_order_id)
  LIMIT 1;
...
'Staff/supervisor RPC to allocate one IN DVA/card statement line to an order final-balance payment target. Reduces balance first; only excess after balance closure may become FX/card difference. Does not use accepted-estimate funding reconciliation, create importer credit, or post to Sage.';
...
-- select to_regprocedure('public.staff_allocate_statement_line_to_final_balance_payment_v1(uuid,uuid,boolean,text)') as final_balance_rpc;
