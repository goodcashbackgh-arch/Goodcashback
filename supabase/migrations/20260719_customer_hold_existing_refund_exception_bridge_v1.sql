...
AS $$
  WITH hold_row AS (
    ...
    SELECT
      ...
      CASE
        WHEN COALESCE(sil.qty_confirmed, sil.qty, 0) > 0 THEN
          ROUND(
...
BEGIN
  ...
  IF NEW.converted_dispute_id IS NOT NULL THEN
    ...
    SET refund_approved_by_staff_id = COALESCE(d.refund_approved_by_staff_id, NEW.reviewed_by_staff_id),
        refund_approved_at = COALESCE(d.refund_approved_at, NEW.reviewed_at, now())
  ...
  SELECT array_agg(t.supplier_invoice_line_id ORDER BY t.supplier_invoice_line_id),
         COUNT(*)::integer
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Approved hold cannot be converted because its exact affected quantity is invalid or fractional.';
  END IF;
  ...
  SELECT COUNT(DISTINCT supplier_invoice_line_id)::integer,
  ...
  IF v_linked_count > 0 THEN
    ...
    THEN
      ...
      WHERE dl.supplier_invoice_line_id = ANY(v_target_line_ids)
        AND dl.resolved_at IS NULL;
    ELSE
      RAISE EXCEPTION 'Approved hold overlaps an incompatible or partial open exception. Resolve or narrow it first.';
    END IF;
  END IF;
  ...
  IF v_operator_id IS NULL THEN
    RAISE EXCEPTION 'Order operator could not be resolved for hold exception conversion.';
  END IF;
...
BEGIN
  ...
  LOOP
    BEGIN
      UPDATE public.customer_pre_shipment_hold_requests h
      SET reviewed_at = COALESCE(h.reviewed_at, now()),
          updated_at = now()
