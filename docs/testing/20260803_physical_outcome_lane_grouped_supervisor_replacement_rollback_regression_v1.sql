...
-- are accepted atomically into two child-free same-order routes and one resolved lane.
...
BEGIN
  SELECT r.*,pr.order_id,dl.dispute_id,d.raised_by_operator_id,
  ...
  IF s.id IS NULL THEN RAISE EXCEPTION 'FAIL: no physical remedy structural fixture source'; END IF;
  ...
  v_unit_value:=GREATEST(COALESCE(s.customer_commercial_value_gbp,1),0.01);
  ...
  SET route_status='cancelled',updated_at=now()
  WHERE source_tracking_line_allocation_id=s.tracking_line_allocation_id
    AND route_status<>'cancelled';

  UPDATE public.order_tracking_line_allocations
  SET qty_allocated=2,
      base_value_gbp=round(v_unit_value*2,2),
      discount_share_gbp=0,
      retailer_delivery_share_gbp=0,
      adjusted_net_value_gbp=round(v_unit_value*2,2)
  WHERE id=s.tracking_line_allocation_id;

  UPDATE public.shipper_package_receipt_line_dispositions
  SET quantity=2,
      disposition_type=CASE WHEN disposition_type IN ('missing','damaged','wrong') THEN disposition_type ELSE 'missing' END,
      tracking_line_allocation_id=s.tracking_line_allocation_id,
      supplier_invoice_line_id=s.supplier_invoice_line_id
  WHERE id=s.receipt_line_disposition_id;
  ...
  SELECT s.physical_receipt_review_id,s.dispute_id,'replacement',s.staff_id
  WHERE NOT EXISTS (
    SELECT 1 FROM public.physical_receipt_review_dispute_links l
    WHERE l.physical_receipt_review_id=s.physical_receipt_review_id
      AND l.dispute_id=s.dispute_id
      AND l.desired_outcome='replacement'
  );

  UPDATE public.dispute_lines
  SET line_status='affected',qty_impact=1,amount_impact_gbp=v_unit_value,
      resolution_method=NULL,resolved_at=NULL,resolved_via_child_order_id=NULL,
      conversation_status='retailer_response_received',intended_remedy='replacement',
      physical_remedy_allocation_id=s.id
  WHERE id=s.dispute_line_id;

  UPDATE public.physical_exception_remedy_allocations
  SET proposed_remedy_type='replacement',proposed_remedy_qty=1,
      ...
      dispute_line_id=s.dispute_line_id,customer_commercial_value_gbp=v_unit_value,
  ...
  SELECT v_clone_dispute_id,order_id,now(),raised_by_operator_id,issue_type,'replacement',NULL,
         liable_party,stage_detected,v_unit_value,'Rollback grouped supervisor replacement fixture.',
  ...
  INSERT INTO public.physical_receipt_review_dispute_links(
    ...
    s.physical_receipt_review_id,v_clone_dispute_id,'replacement',s.staff_id
  );
  ...
  INSERT INTO public.dispute_lines(
    ...
    v_clone_line_id,v_clone_dispute_id,s.supplier_invoice_line_id,1,v_unit_value,
    'affected',NULL,NULL,NULL,now(),'retailer_response_received','replacement',v_clone_allocation_id
  );
  ...
  SELECT v_clone_allocation_id,physical_receipt_review_id,receipt_line_disposition_id,
         tracking_line_allocation_id,supplier_invoice_line_id,
         'replacement',1,proposed_by_operator_id,now(),
         'replacement',1,s.staff_id,now(),v_clone_line_id,
         supplier_claim_amount_gbp,v_unit_value,'free_replacement',NULL,NULL,
  ...
  INSERT INTO public.physical_receipt_outcome_lanes(
    ...
    v_lane_id,s.order_id,s.physical_receipt_review_id,'replacement','awaiting_supervisor_decision'
  );
...
BEGIN
  ...
  r1:=public.staff_decide_physical_outcome_lane_v1(
    l,st,payload,'Rollback grouped replacement acceptance.'
  );

  IF (r1->>'selected_items')::integer<>2
     OR (r1->>'resolved_items')::integer<>2
     OR r1->>'lane_status'<>'resolved'
  THEN RAISE EXCEPTION 'FAIL: grouped replacement decision result incorrect: %',r1; END IF;

  SELECT COUNT(*) INTO route_count
  FROM public.physical_replacement_same_order_routes
  WHERE physical_remedy_allocation_id IN (a1,a2)
    AND route_status<>'cancelled';
  IF route_count<>2 THEN
    RAISE EXCEPTION 'FAIL: expected two exact same-order routes, got %',route_count;
  END IF;

  IF (SELECT COALESCE(SUM(replacement_qty),0)
      FROM public.physical_replacement_same_order_routes
      WHERE physical_remedy_allocation_id IN (a1,a2)
        AND route_status<>'cancelled')<>2
  THEN RAISE EXCEPTION 'FAIL: replacement route quantity did not remain exactly two'; END IF;
  ...
  IF r2 IS DISTINCT FROM r1 THEN
    RAISE EXCEPTION 'FAIL: identical replay did not return stored result';
  END IF;
