-- Supabase SQL Editor regression v2.
...
BEGIN
  ...
  IF strpos(v_resolver_definition, 'settlement_resolution_status = ''fully_resolved''') = 0
  ...
  -- 1. Real partial case must resolve as DVA cash.
  ...
  -- 3. Partial but unresolved/over-resolved must fail closed.
  BEGIN
    UPDATE public.order_pending_funding_surplus
  ...
  -- 4. Direct-cash baseline executes unchanged.
  WITH candidates AS (
    SELECT
      o.id AS order_id,
      ROUND(COALESCE(SUM(ABS(e.amount_gbp)) FILTER (
        WHERE e.event_type = 'funding_contribution'
      ), 0)::numeric, 2) AS direct_gbp,
      ROUND(COALESCE((
  ...
  -- 5. Released-loyalty baseline executes unchanged.
  ...
  wallet_totals AS (
    SELECT order_id, wallet_code, ROUND(SUM(amount_gbp)::numeric, 2) AS applied_gbp
    FROM loyalty_applied
    GROUP BY order_id, wallet_code
  ),
  wallet_remaining AS (
    SELECT
      wt.order_id,
  ...
  -- 6. Existing loyalty/cash ambiguity remains fail-closed.
  IF strpos(
  ...
  -- 7. Real business facts remain unchanged.
  ...
  THEN
    RAISE EXCEPTION 'REGRESSION: rollback-only regression left persistent rows.';
  END IF;
