...
--
-- ROLLBACK-ONLY. No test write is allowed to survive.
-- Uses the already-established full-credit and partial-credit live fixtures.
...
BEGIN
  IF to_regprocedure('public.customer_correct_unprocessed_order_v1(uuid,integer,numeric,text[])') IS NULL THEN
    RAISE EXCEPTION 'FAIL: correction RPC is missing.';
  END IF;
  IF to_regprocedure('public.customer_order_correction_eligibility_v1(uuid)') IS NULL THEN
    RAISE EXCEPTION 'FAIL: eligibility wrapper is missing.';
  END IF;
  ...
  IF NOT COALESCE(v_correction_secdef, false)
     ...
     OR NOT COALESCE(v_wrapper_config, ARRAY[]::text[]) @> ARRAY['search_path=public, pg_temp']::text[] THEN
    RAISE EXCEPTION 'FAIL: SECURITY DEFINER/search_path boundary is wrong.';
  END IF;

  IF position('oi.importer_id = v_order.importer_id' IN v_correction_def) = 0
     OR position('v_operator.importer_id := v_order.importer_id' IN v_correction_def) = 0
     OR position('ORDER BY oi.id DESC' IN v_correction_def) > 0 THEN
    RAISE EXCEPTION 'FAIL: order-scoped access rewrite is not installed.';
  END IF;

  IF position('FOR UPDATE OF o' IN v_wrapper_def) = 0
     OR position('customer_correct_unprocessed_order_v1' IN v_wrapper_def) = 0
     OR position('NULL::text[]' IN v_wrapper_def) = 0 THEN
    RAISE EXCEPTION 'FAIL: eligibility wrapper is not the governed locked delegating wrapper.';
  END IF;
...
-- Fixture preflight
-- Full-credit Day3:
-- order 51c1a8bb-9f58-4f3b-8e50-9d9d935f0167
-- operator b5d0aa51-596f-41d1-ac88-b8d74206107d
-- auth user aa64af59-2244-4323-87c3-e9f32644af44
-- Partial-credit:
-- order 21dbf28d-f671-47dd-b85d-5c542dd61e2e
-- operator ba440d50-0162-4040-a771-87029f089d4b
-- auth user abf6dab8-2f12-49d4-ab71-af8ce0c989aa
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  v_full record;
  v_partial record;
BEGIN
  SELECT o.id, o.operator_id, o.importer_id, o.status, o.order_type,
         o.order_total_gbp_declared, o.funded_at,
         f.applied_credit_gbp, f.funded_total_gbp, f.gap_remaining_gbp,
         f.threshold_met_yn, f.confirmed_dva_funding_gbp
  INTO v_full
  ...
  IF v_full.id IS NULL
     ...
     OR NOT (v_full.funded_at IS NOT NULL OR v_full.threshold_met_yn OR v_full.gap_remaining_gbp <= 0.01) THEN
    RAISE EXCEPTION 'FAIL: full-credit fixture drifted; stop rather than testing the wrong state.';
  END IF;
  ...
  IF v_partial.id IS NULL
    ...
    RAISE EXCEPTION 'FAIL: partial-credit fixture drifted; stop rather than testing the wrong state.';
  END IF;
END $$;

-- -----------------------------------------------------------------------------
-- 2. Eligibility wrapper: locked current snapshot + no mutation
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  v_result jsonb;
...
BEGIN
  BEGIN
    ...
    INTO v_screenshot_count_before
    FROM public.order_screenshots
    WHERE order_id = v_order_before.id
      AND note = 'Original order screenshot';
    ...
    IF COALESCE((v_result->>'ok')::boolean, false) IS NOT TRUE
       ...
       OR ROUND((v_result->>'applied_credit_gbp')::numeric, 2) IS DISTINCT FROM ROUND(v_funding_before.applied_credit_gbp::numeric, 2)
       OR ROUND((v_result->>'funded_total_gbp')::numeric, 2) IS DISTINCT FROM ROUND(v_funding_before.funded_total_gbp::numeric, 2)
       OR ROUND((v_result->>'markup_applied_gbp')::numeric, 2) IS DISTINCT FROM ROUND(v_funding_before.markup_applied_gbp::numeric, 2) THEN
      RAISE EXCEPTION 'FAIL: eligibility wrapper snapshot does not match the locked current state.';
    END IF;

    SELECT id, importer_id, total_qty_declared, order_total_gbp_declared, quote_total_ghs, funded_at
    INTO v_order_after
    FROM public.orders
    WHERE id = v_order_before.id;
    ...
    SELECT COALESCE(ROUND(SUM(amount_gbp), 2), 0), COUNT(*)::integer
    INTO v_event_sum_after, v_event_count_after
    FROM public.order_funding_events
    WHERE order_id = v_order_before.id;

    SELECT COUNT(*)::integer
    INTO v_screenshot_count_after
    FROM public.order_screenshots
    WHERE order_id = v_order_before.id
      AND note = 'Original order screenshot';
    ...
    IF v_order_after.total_qty_declared IS DISTINCT FROM v_order_before.total_qty_declared
       ...
       OR ROUND(v_funding_after.applied_credit_gbp::numeric, 2) IS DISTINCT FROM ROUND(v_funding_before.applied_credit_gbp::numeric, 2)
       OR ROUND(v_funding_after.funded_total_gbp::numeric, 2) IS DISTINCT FROM ROUND(v_funding_before.funded_total_gbp::numeric, 2)
       OR ROUND(v_funding_after.gap_remaining_gbp::numeric, 2) IS DISTINCT FROM ROUND(v_funding_before.gap_remaining_gbp::numeric, 2)
       OR v_funding_after.threshold_met_yn IS DISTINCT FROM v_funding_before.threshold_met_yn
      ...
      RAISE EXCEPTION 'FAIL: eligibility wrapper mutated order/funding/screenshot state.';
    END IF;

    -- Force this case into a PL/pgSQL subtransaction rollback even though it is read-only.
    RAISE EXCEPTION '__PASS_ROLLBACK__';
  EXCEPTION WHEN OTHERS THEN
    IF SQLERRM IS DISTINCT FROM '__PASS_ROLLBACK__' THEN
      RAISE;
    END IF;
  END;
...
-- -----------------------------------------------------------------------------
-- 3. Multi-importer access: a different newer/high-id active assignment must not
--    steal scope from the importer belonging to the requested order.
-- -----------------------------------------------------------------------------
DO $$
DECLARE
...
BEGIN
  BEGIN
    ...
    IF v_relationship_type IS NULL OR v_other_importer IS NULL THEN
      RAISE EXCEPTION 'FAIL: cannot construct rollback-only multi-importer test fixture.';
    END IF;

    INSERT INTO public.operator_importers
      (id, operator_id, importer_id, relationship_type, granted_at, revoked_at)
    VALUES
      ('ffffffff-ffff-ffff-ffff-ffffffffffff'::uuid,
       'b5d0aa51-596f-41d1-ac88-b8d74206107d'::uuid,
       v_other_importer,
       v_relationship_type,
       clock_timestamp(),
       NULL);
...
-- -----------------------------------------------------------------------------
-- 4. Revoked assignment and wrong-owner access both fail closed.
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  v_dummy jsonb;
BEGIN
  -- Revoked exact importer assignment.
  BEGIN
    PERFORM set_config('request.jwt.claim.role', 'authenticated', true);
    PERFORM set_config('request.jwt.claim.sub', 'aa64af59-2244-4323-87c3-e9f32644af44', true);

    UPDATE public.operator_importers
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'FAIL: expected active Day3 operator/importer assignment was not found.';
    END IF;

    v_dummy := public.customer_order_correction_eligibility_v1('51c1a8bb-9f58-4f3b-8e50-9d9d935f0167'::uuid);
    RAISE EXCEPTION 'FAIL: revoked importer assignment unexpectedly retained correction access.';
  EXCEPTION WHEN OTHERS THEN
    IF position('does not belong to the active customer/operator assignment' IN lower(SQLERRM)) = 0 THEN
      RAISE;
    END IF;
  END;

  -- Wrong operator/customer identity against the other fixture.
  BEGIN
    PERFORM set_config('request.jwt.claim.role', 'authenticated', true);
    PERFORM set_config('request.jwt.claim.sub', 'aa64af59-2244-4323-87c3-e9f32644af44', true);

    v_dummy := public.customer_order_correction_eligibility_v1('21dbf28d-f671-47dd-b85d-5c542dd61e2e'::uuid);
    RAISE EXCEPTION 'FAIL: wrong operator unexpectedly gained correction access.';
  EXCEPTION WHEN OTHERS THEN
    IF position('does not belong to the active customer/operator assignment' IN lower(SQLERRM)) = 0 THEN
      RAISE;
    END IF;
...
-- -----------------------------------------------------------------------------
-- 5. Fully credit-funded decrease: must reject credit-release/repair semantics.
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  v_qty integer;
...
BEGIN
  BEGIN
    ...
    SELECT total_qty_declared, order_total_gbp_declared
    INTO v_qty, v_amount
    FROM public.orders
    WHERE id = '51c1a8bb-9f58-4f3b-8e50-9d9d935f0167'::uuid;
    ...
    v_dummy := public.customer_correct_unprocessed_order_v1(
      ...
      v_qty,
      ROUND(v_amount - 5.00, 2),
      NULL::text[]
    );

    RAISE EXCEPTION 'FAIL: fully credit-funded decrease unexpectedly succeeded.';
  EXCEPTION WHEN OTHERS THEN
    IF position('corrected value would require credit release or financial-state repair' IN lower(SQLERRM)) = 0 THEN
      RAISE;
    END IF;
  END;
...
-- -----------------------------------------------------------------------------
-- 6. Fully credit-funded quantity-only correction: funding/funded_at preserved.
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  v_before_order record;
  v_after_order record;
  v_before_funding record;
  v_after_funding record;
  v_event_sum_before numeric;
  v_event_sum_after numeric;
  v_event_count_before integer;
  v_event_count_after integer;
...
BEGIN
  BEGIN
    ...
    SELECT total_qty_declared, order_total_gbp_declared, quote_total_ghs, funded_at
    INTO v_before_order
    FROM public.orders
    WHERE id = '51c1a8bb-9f58-4f3b-8e50-9d9d935f0167'::uuid;

    SELECT applied_credit_gbp, funded_total_gbp, gap_remaining_gbp, threshold_met_yn
    INTO v_before_funding
    FROM public.order_funding_position_vw
    WHERE order_id = '51c1a8bb-9f58-4f3b-8e50-9d9d935f0167'::uuid;
    ...
    v_result := public.customer_correct_unprocessed_order_v1(
      '51c1a8bb-9f58-4f3b-8e50-9d9d935f0167'::uuid,
      v_before_order.total_qty_declared + 1,
      v_before_order.order_total_gbp_declared,
      NULL::text[]
    );
    ...
    IF COALESCE((v_result->>'ok')::boolean, false) IS NOT TRUE
       ...
       OR ROUND(v_after_funding.applied_credit_gbp::numeric, 2) IS DISTINCT FROM ROUND(v_before_funding.applied_credit_gbp::numeric, 2)
       OR ROUND(v_after_funding.funded_total_gbp::numeric, 2) IS DISTINCT FROM ROUND(v_before_funding.funded_total_gbp::numeric, 2)
       OR ROUND(v_after_funding.gap_remaining_gbp::numeric, 2) IS DISTINCT FROM ROUND(v_before_funding.gap_remaining_gbp::numeric, 2)
       OR v_after_funding.threshold_met_yn IS DISTINCT FROM v_before_funding.threshold_met_yn
       OR v_event_sum_after IS DISTINCT FROM v_event_sum_before
       OR v_event_count_after IS DISTINCT FROM v_event_count_before THEN
...
-- -----------------------------------------------------------------------------
-- 7. Fully credit-funded upward-value correction: credit stays fixed, new gap,
--    proportional quote, funded_at cleared through existing recompute authority.
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  v_before_order record;
  ...
  v_expected_quote numeric;
  v_expected_gap numeric;
  v_event_sum_before numeric;
  v_event_sum_after numeric;
  v_event_count_before integer;
  v_event_count_after integer;
...
BEGIN
  BEGIN
    ...
    v_expected_gap := ROUND(GREATEST(v_new_amount + v_before_funding.markup_applied_gbp - v_before_funding.funded_total_gbp, 0)::numeric, 2);

    v_result := public.customer_correct_unprocessed_order_v1(
      '51c1a8bb-9f58-4f3b-8e50-9d9d935f0167'::uuid,
      v_before_order.total_qty_declared,
      v_new_amount,
      NULL::text[]
    );
    ...
    IF COALESCE((v_result->>'ok')::boolean, false) IS NOT TRUE
       ...
       OR ROUND(v_after_funding.applied_credit_gbp::numeric, 2) IS DISTINCT FROM ROUND(v_before_funding.applied_credit_gbp::numeric, 2)
       OR ROUND(v_after_funding.funded_total_gbp::numeric, 2) IS DISTINCT FROM ROUND(v_before_funding.funded_total_gbp::numeric, 2)
       OR ROUND(v_after_funding.gap_remaining_gbp::numeric, 2) IS DISTINCT FROM v_expected_gap
       OR v_after_funding.threshold_met_yn
       OR v_event_sum_after IS DISTINCT FROM v_event_sum_before
       OR v_event_count_after IS DISTINCT FROM v_event_count_before THEN
...
-- -----------------------------------------------------------------------------
-- 8. Partial-credit upward-value correction: existing credit stays fixed and
--    canonical view derives the new gap without recompute-side effects.
-- -----------------------------------------------------------------------------
DO $$
DECLARE
  v_before_order record;
...
BEGIN
  BEGIN
    ...
    INTO v_before_order
    FROM public.orders
    WHERE id = '21dbf28d-f671-47dd-b85d-5c542dd61e2e'::uuid;

    SELECT applied_credit_gbp, funded_total_gbp, markup_applied_gbp, gap_remaining_gbp, threshold_met_yn
    INTO v_before_funding
    FROM public.order_funding_position_vw
    WHERE order_id = '21dbf28d-f671-47dd-b85d-5c542dd61e2e'::uuid;
