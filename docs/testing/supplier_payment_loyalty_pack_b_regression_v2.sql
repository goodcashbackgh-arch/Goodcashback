...
CREATE OR REPLACE FUNCTION pg_temp.create_loyalty_fixture(
  p_order_id uuid,
  p_credit_id uuid,
  p_debit_id uuid,
  p_approval_id uuid,
  p_match_id uuid,
  p_destination_in_line_id uuid,
  p_amount numeric,
  ...
  p_staff_id uuid,
  p_auth_user_id uuid,
  p_source_statement_line_id uuid,
  p_activation_route text,
  p_card_used_by text,
  p_order_template jsonb,
  p_credit_template jsonb,
  p_approval_template jsonb,
  p_line_template jsonb,
  p_label text
) RETURNS void
LANGUAGE plpgsql
AS $fn$
BEGIN
  PERFORM pg_temp.insert_clone('public.orders', p_order_template, jsonb_build_object(
    'id', p_order_id,
    'order_ref', 'REG-PACK-B-' || p_label || '-' || left(p_order_id::text,8),
    'payment_auth_id', 'AUTH-PACK-B-' || p_label || '-' || left(p_order_id::text,8),
    'order_type', 'original',
    'order_total_gbp_declared', p_order_total,
    'funded_at', now(),
  ...
  PERFORM pg_temp.insert_clone('public.dva_statement_lines', p_line_template, jsonb_build_object(
    'id', p_destination_in_line_id,
    'line_order', p_line_order,
    'statement_date', current_date,
    'direction', 'in',
    'amount_local_ccy', p_amount,
    'amount_gbp_equivalent', p_amount,
    'auth_id_ref', 'AUTH-PACK-B-' || p_label || '-' || left(p_order_id::text,8),
    'retailer_name_ref', null,
    'reference_raw', 'IN PACK B ' || p_label || ' LOYALTY',
    'match_status', 'confirmed',
    'created_at', now()
  ));

  PERFORM pg_temp.insert_clone('public.importer_credit_ledger', p_credit_template, jsonb_build_object(
    'id', p_credit_id,
    ...
    'effective_at', now(),
    'created_at', now(),
    'notes', 'Pack B v2 ' || p_label || ' loyalty source lot'
  ));

  PERFORM pg_temp.insert_clone(
    'public.completion_loyalty_reward_approvals',
    p_approval_template,
    jsonb_build_object(
      'id', p_approval_id,
      'order_id', p_order_id,
      ...
      'qualifying_signed_gross_basis_gbp', p_amount,
      'qualifying_net_spend_gbp', p_amount,
      'suggested_reward_gbp', p_amount,
      'approved_amount_gbp', p_amount,
      'credit_ledger_id', p_credit_id,
      'approval_status', 'released_available_dashboard_credit',
      'funding_confirmation_id', null,
      'created_at', now(),
      'updated_at', now(),
      'notes', 'Pack B v2 ' || p_label || ' approval'
    )
  );
  ...
  INSERT INTO public.main_bank_completion_loyalty_funding_matches (
    ...
    paired_at,
    paired_by_staff_id,
    paired_by_auth_user_id,
    variance_gbp
  ) VALUES (
    p_match_id,
    p_source_statement_line_id,
    p_order_id,
    p_importer_id,
    p_approval_id,
    null,
    ...
    'Pack B v2 ' || p_label || ' paired release',
    p_staff_id,
    p_auth_user_id,
    now(),
    p_destination_in_line_id,
    coalesce(p_activation_route,'dva_account_top_up'),
    coalesce(p_card_used_by,'staff'),
    'paired_released',
    now(),
    p_staff_id,
    p_auth_user_id,
  ...
  PERFORM pg_temp.insert_clone('public.importer_credit_ledger', p_credit_template, jsonb_build_object(
    ...
    'source_entity_id', p_credit_id,
    'applied_to_order_id', p_order_id,
    'lock_reason', null,
    'created_by_staff_id', p_staff_id,
    'effective_at', now(),
    'created_at', now(),
    'notes', 'Pack B v2 ' || p_label || ' exact loyalty application debit'
  ));
  ...
  SELECT
    ...
    'credit_applied',
    p_amount,
    'importer_credit_ledger:' || p_debit_id,
    'importer_credit_ledger',
    p_debit_id,
    p_staff_id,
    now(),
    'Pack B v2 ' || p_label || ' fallback loyalty event'
  WHERE NOT EXISTS (
    SELECT 1
    FROM public.order_funding_events
    WHERE event_type = 'credit_applied'
      AND source_entity_type = 'importer_credit_ledger'
      AND source_entity_id = p_debit_id
  );
END
$fn$;
...
BEGIN
  ...
  WHERE si.order_id = v_template_match.completed_order_id
    ...
    AND coalesce(si.ocr_invoice_total_gbp, si.reconciliation_gbp_total) > 0
  ORDER BY
    si.reviewed_at DESC NULLS LAST,
    si.uploaded_at DESC NULLS LAST,
    si.id DESC
  LIMIT 1;
  ...
  IF v_order_template IS NULL
     OR v_credit_template IS NULL
     OR v_approval_template IS NULL
     OR v_line_template IS NULL
     OR v_invoice_template IS NULL
     OR v_mapping_code IS NULL THEN
    RAISE EXCEPTION 'Incomplete Pack B template';
  END IF;

  SELECT coalesce(max(line_order),0) + 1000
  INTO v_next_line_order
  FROM public.dva_statement_lines
  WHERE dva_statement_id = (v_line_template->>'dva_statement_id')::uuid;

  PERFORM pg_temp.create_loyalty_fixture(
    s1_order,
    s1_credit,
    s1_debit,
    s1_approval,
    ...
    100.00,
    v_next_line_order,
    v_template_match.importer_id,
    v_staff_id,
    v_auth_user_id,
    v_template_match.dva_statement_line_id,
    v_template_match.activation_route,
    v_template_match.card_used_by,
    v_order_template,
    v_credit_template,
    v_approval_template,
    v_line_template,
  ...
  PERFORM pg_temp.insert_clone('public.supplier_invoices', v_invoice_template, jsonb_build_object(
    ...
    'reviewed_at', now(),
    'uploaded_at', now(),
    'review_notes', 'Pack B v2 scenario 1 invoice'
  ));
  ...
  PERFORM pg_temp.insert_clone('public.dva_statement_lines', v_line_template, jsonb_build_object(
    ...
    'statement_date', current_date,
    'direction', 'out',
    'amount_local_ccy', 100.00,
    'amount_gbp_equivalent', 100.00,
  ...
  SELECT selectable_yn, blocker
  INTO v_selectable, v_candidate_blocker
  FROM public.supplier_payment_candidate_status_vw
  WHERE supplier_invoice_id = s1_invoice;
  ...
  WHERE dva_statement_line_id = s1_out
    AND suggested_match_type = 'supplier_invoice'
  ...
  IF v_count <> 1 OR v_line_status IS DISTINCT FROM 'suggested' THEN
    RAISE EXCEPTION
      'Scenario 1 suggestion failed: count %, status %, rpc %',
      v_count,
      v_line_status,
      v_suggestion;
  END IF;

  SELECT supplier_payment_ready_yn, blocker
  ...
  v_allocation := public.staff_allocate_statement_line_to_supplier_invoice(
    ...
    s1_invoice,
    100.00,
    'Pack B v2 scenario 1 full physical OUT'
  );
  ...
  VALUES (
    1,
    'Released loyalty retailer match',
    'PASS',
  ...
  PERFORM pg_temp.insert_clone('public.dva_statement_lines', v_line_template, jsonb_build_object(
    ...
    'statement_date', current_date,
    'direction', 'in',
    'amount_local_ccy', 300.00,
    'amount_gbp_equivalent', 300.00,
    'auth_id_ref', 'AUTH-PACK-B-MIX-' || left(s2_order::text,8),
    'retailer_name_ref', null,
    'reference_raw', 'IN PACK B MIXED CASH',
    'match_status', 'confirmed',
    'created_at', now()
  ));
  ...
  INSERT INTO public.dva_reconciliation (
    ...
    v_staff_id,
    now(),
    'Pack B v2 scenario 2 proven £300 cash leg'
  );
  ...
  SELECT
    ...
    300.00,
    'dva_reconciliation:' || s2_cash_recon,
    'dva_reconciliation',
    s2_cash_recon,
    v_staff_id,
    now(),
    'Pack B v2 fallback cash event'
  WHERE NOT EXISTS (
    SELECT 1
    FROM public.order_funding_events
  ...
  PERFORM pg_temp.insert_clone('public.supplier_invoices', v_invoice_template, jsonb_build_object(
    ...
    'ocr_invoice_total_gbp', 400.00,
    'reconciliation_gbp_total', 400.00,
    'review_status', 'approved_current',
    'blocked_from_sage_yn', false,
    'is_current_for_order', true,
    'reviewed_by_staff_id', v_staff_id,
  ...
  PERFORM pg_temp.insert_clone('public.dva_statement_lines', v_line_template, jsonb_build_object(
    ...
    'statement_date', current_date,
    'direction', 'out',
    'amount_local_ccy', 400.00,
    'amount_gbp_equivalent', 400.00,
    'auth_id_ref', 'AUTH-PACK-B-MIX-' || left(s2_order::text,8),
    'retailer_name_ref', v_retailer_name,
    'reference_raw', v_retailer_name || ' CARD PURCHASE PACK B MIXED',
    'match_status', 'unmatched',
    'created_at', now()
  ));
  ...
  INTO v_selectable, v_candidate_blocker
  FROM public.supplier_payment_candidate_status_vw
  WHERE supplier_invoice_id = s2_invoice;
  ...
  INTO v_count
  FROM public.match_suggestions
  WHERE dva_statement_line_id = s2_out
    AND suggested_match_type = 'supplier_invoice'
  ...
  IF v_count <> 1 OR v_line_status IS DISTINCT FROM 'suggested' THEN
    RAISE EXCEPTION
      'Scenario 2 suggestion failed: count %, status %, rpc %',
      v_count,
      v_line_status,
      v_suggestion;
  END IF;
  ...
  BEGIN
    PERFORM public.staff_allocate_statement_line_to_supplier_invoice(
      ...
      s2_invoice,
      400.00,
      'Pack B v2 one £400 OUT must not be split or guessed'
    );
  ...
  VALUES (
    2,
    '£100 loyalty + £300 cash, one £400 retailer OUT',
    'PASS',
    ...
    jsonb_build_object(
      ...
      'active_allocations_after_failure', v_count
    )
  );
END
$pack$;
