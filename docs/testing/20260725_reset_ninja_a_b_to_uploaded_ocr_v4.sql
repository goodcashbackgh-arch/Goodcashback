...
-- Exact one-time reset for NIN-240726-A and NIN-240726-B only.
-- The normal OCR-line delete prohibition remains unchanged in production.
-- This script takes an ACCESS EXCLUSIVE lock, temporarily suspends only the
-- named delete-prevention trigger inside this transaction, deletes only the
-- proven OCR-derived rows for A/B, restores and verifies the trigger before
-- resetting the invoice OCR state, then commits. Any failure rolls back both
-- the data changes and the trigger state.
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'OCR-line delete protection trigger is missing or not enabled; refusing reset.';
  END IF;

  IF to_regprocedure('public.prevent_ocr_supplier_invoice_line_delete()') IS NULL THEN
    RAISE EXCEPTION 'OCR-line delete protection function is missing; refusing reset.';
  END IF;
END $$;
...
-- Prevent another session changing invoice lines while the single trigger is
-- suspended. The lock is transactional and released automatically at COMMIT.
LOCK TABLE public.supplier_invoice_lines IN ACCESS EXCLUSIVE MODE;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'A/B no longer contain OCR-extracted rows; refusing an unexpected reset state.';
  END IF;
...
-- One-time, exact transactional suspension. ACCESS EXCLUSIVE lock above prevents
-- any concurrent session using the table while the trigger is disabled.
ALTER TABLE public.supplier_invoice_lines
  DISABLE TRIGGER trg_prevent_ocr_supplier_invoice_line_delete;
...
ALTER TABLE public.supplier_invoice_lines
  ENABLE TRIGGER trg_prevent_ocr_supplier_invoice_line_delete;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'OCR-line delete protection trigger was not restored; transaction rolled back.';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'OCR-line deletion did not complete for A/B; transaction rolled back.';
  END IF;
END $$;
...
CROSS JOIN LATERAL (
  SELECT EXISTS (
    SELECT 1
    FROM pg_trigger pt
    WHERE pt.tgrelid = 'public.supplier_invoice_lines'::regclass
      AND pt.tgname = 'trg_prevent_ocr_supplier_invoice_line_delete'
