...
BEGIN
  -- -------------------------------------------------------------------------
  -- Exact frozen-authority check before any behavioural exercise.
  -- -------------------------------------------------------------------------
  ...
  -- -------------------------------------------------------------------------
  -- Establish the same SQL-editor staff auth context used by August treasury
  -- regressions.
  -- -------------------------------------------------------------------------
  ...
  -- -------------------------------------------------------------------------
  -- Controlled live prerequisite state.
  -- -------------------------------------------------------------------------
  ...
  IF cardinality(v_block_reasons) > 0 THEN
    ...
    VALUES (
      ...
      'No behavioural writer exercise was run: ' || array_to_string(v_block_reasons, '; ')
    );
    RETURN;
  END IF;

  -- -------------------------------------------------------------------------
  -- 1. Exact controlled £20.19 / £19.99 / £0.20 success path, fully rolled back.
  -- -------------------------------------------------------------------------
  BEGIN
    SELECT public.staff_classify_final_balance_in_fx_residual_v1(
      v_target_line,
      0.20,
      '[REGRESSION_ONLY] exact controlled final-balance IN FX residual'
    ) INTO v_result;
    ...
    FROM public.internal_cash_posting_workbench_rows_v1(
      'in', 'fx_card_difference', 'all', v_target_order_ref, 300, 0
    ) w
    WHERE w.source_id = v_new_fx_alloc
      AND w.direction = 'in'
      AND w.category = 'fx_card_difference'
      AND ROUND(COALESCE(w.amount_gbp, 0)::numeric, 2) = 0.20;

    IF v_cash_count <> 1 THEN
      RAISE EXCEPTION 'FAIL: new IN FX allocation did not appear exactly once in existing cash workbench';
    END IF;

    SELECT COUNT(*)::integer INTO v_fb_cash_count
    FROM public.internal_cash_posting_workbench_rows_v1(
      'in', 'customer_receipt_on_account', 'all', v_target_order_ref, 300, 0
    ) w
    WHERE w.source_id = v_target_final_alloc
      AND w.source_type = 'dva_final_balance_allocation'
      AND ROUND(COALESCE(w.amount_gbp, 0)::numeric, 2) = 19.99;

    IF v_fb_cash_count <> 1 THEN
      RAISE EXCEPTION 'FAIL: existing final-balance cash bridge no longer exposes exactly £19.99';
    END IF;

    RAISE EXCEPTION 'REGRESSION_ROLLBACK_CONTROLLED_SUCCESS';
  EXCEPTION WHEN OTHERS THEN
    IF SQLERRM IS DISTINCT FROM 'REGRESSION_ROLLBACK_CONTROLLED_SUCCESS' THEN
      RAISE;
    END IF;
  END;

  -- Exact live row must be back to its original state after subtransaction rollback.
  SELECT p.* INTO v_target_position
  FROM public.statement_line_control_position_v1 p
  WHERE p.statement_line_id = v_target_line;

  SELECT COUNT(*)::integer INTO v_count_after
  FROM public.dva_statement_line_allocations a
  WHERE a.dva_statement_line_id = v_target_line
    AND a.allocation_type = 'fx_card_difference'
    AND a.allocation_status <> 'reversed';
  ...
  IF ROUND(COALESCE(v_target_position.active_consumed_gbp, 0)::numeric, 2) <> 19.99
     OR ROUND(COALESCE(v_target_position.remaining_unconsumed_gbp, 0)::numeric, 2) <> 0.20
     OR v_count_after <> 0 THEN
    RAISE EXCEPTION 'FAIL: controlled success rollback left residue on live target';
  END IF;

  -- -------------------------------------------------------------------------
  -- 2. Expected amount mismatch must reject with zero residue.
  -- -------------------------------------------------------------------------
  v_rejected := false;
  BEGIN
    PERFORM public.staff_classify_final_balance_in_fx_residual_v1(
      v_target_line, 0.19, '[REGRESSION_ONLY] amount mismatch rejection'
    );
  EXCEPTION WHEN OTHERS THEN
    v_rejected := position('does not equal canonical remaining' in lower(SQLERRM)) > 0;
  END;
  IF NOT v_rejected THEN
    RAISE EXCEPTION 'FAIL: amount mismatch was not rejected by canonical residual guard';
  END IF;

  -- -------------------------------------------------------------------------
  -- 3. Wrong direction must reject before mutation.
  -- -------------------------------------------------------------------------
  ...
  BEGIN
    PERFORM public.staff_classify_final_balance_in_fx_residual_v1(
      v_out_line, 0.01, '[REGRESSION_ONLY] wrong direction rejection'
    );
  EXCEPTION WHEN OTHERS THEN
  ...
  IF NOT v_rejected OR v_count_after <> v_count_before THEN
    RAISE EXCEPTION 'FAIL: wrong-direction rejection failed or changed allocation count';
  END IF;

  -- -------------------------------------------------------------------------
  -- 4. No final-balance allocation / no principal lane must reject.
  -- -------------------------------------------------------------------------
  ...
  BEGIN
    INSERT INTO public.dva_statements(
      ...
      v_statement_id, v_target_importer, 'importer_dva_card_account', v_target_importer::text,
      'Regression no-final-balance line', 'gcb', v_staff_id,
      'regression://no-final-balance-' || v_statement_id::text,
    ...
    INSERT INTO public.dva_statement_lines(
      ...
      v_test_line_id, v_statement_id, 1, CURRENT_DATE,
      'REGRESSION no final-balance allocation', 'in',
      0.20, 'GBP', 1, 0, 0.20,
      'REG-NO-FB-' || left(v_test_line_id::text, 8), 'Regression only', 'unmatched'
    );

    v_rejected := false;
    BEGIN
      PERFORM public.staff_classify_final_balance_in_fx_residual_v1(
        v_test_line_id, 0.20, '[REGRESSION_ONLY] no final balance rejection'
      );
    EXCEPTION WHEN OTHERS THEN
      v_rejected := position('principal lane' in lower(SQLERRM)) > 0
        OR position('final_balance_payment' in lower(SQLERRM)) > 0;
    END;

    IF NOT v_rejected THEN
      RAISE EXCEPTION 'FAIL: line without final-balance allocation was not rejected';
    END IF;
  ...
  IF EXISTS (SELECT 1 FROM public.dva_statement_lines WHERE id = v_test_line_id)
    ...
    RAISE EXCEPTION 'FAIL: no-final-balance fixture left residue';
  END IF;

  -- -------------------------------------------------------------------------
  -- 5. Defensive open-final-balance state must reject. A synthetic allocation
  --    is used only inside this rollback subtransaction to exercise the guard.
  -- -------------------------------------------------------------------------
  v_statement_id := gen_random_uuid();
  v_test_line_id := gen_random_uuid();
  BEGIN
    INSERT INTO public.dva_statements(
      ...
      v_statement_id, v_open_importer, 'importer_dva_card_account', v_open_importer::text,
      'Regression open-final-balance line', 'gcb', v_staff_id,
      'regression://open-final-balance-' || v_statement_id::text,
      CURRENT_DATE, CURRENT_DATE, 'parsed'
    );
    ...
    INSERT INTO public.dva_statement_lines(
      ...
      v_test_line_id, v_statement_id, 1, CURRENT_DATE,
      'REGRESSION open final balance', 'in',
      0.20, 'GBP', 1, 0, 0.20,
      'REG-OPEN-FB-' || left(v_test_line_id::text, 8), 'Regression only', 'unmatched'
    );
    ...
    INSERT INTO public.dva_statement_line_allocations(
      ...
      v_test_line_id, 'final_balance_payment', NULL, NULL, v_open_order,
      0.10, 'confirmed', 1, 0,
      '[REGRESSION_ONLY] synthetic partial final-balance evidence',
      v_staff_id, now(), v_staff_id, now()
    );

    SELECT ROUND(COALESCE(s.final_balance_due_gbp, 0)::numeric, 2)
      INTO v_open_after
    FROM public.internal_order_final_sale_settlement_v2(v_open_order) s
    LIMIT 1;

    IF v_open_after <= 0.005 THEN
      RAISE EXCEPTION 'FAIL: open-final-balance fixture unexpectedly closed the balance';
    END IF;

    v_rejected := false;
    BEGIN
      PERFORM public.staff_classify_final_balance_in_fx_residual_v1(
        v_test_line_id, 0.10, '[REGRESSION_ONLY] open balance rejection'
      );
    EXCEPTION WHEN OTHERS THEN
    ...
    IF NOT v_rejected THEN
      RAISE EXCEPTION 'FAIL: open final balance was not rejected';
    END IF;
  ...
  IF EXISTS (SELECT 1 FROM public.dva_statement_lines WHERE id = v_test_line_id)
     OR EXISTS (SELECT 1 FROM public.dva_statements WHERE id = v_statement_id) THEN
    RAISE EXCEPTION 'FAIL: open-final-balance fixture left residue';
  END IF;

  -- -------------------------------------------------------------------------
  -- 6. Duplicate/reuse attempt after one valid FX classification must reject.
  -- -------------------------------------------------------------------------
  BEGIN
    SELECT public.staff_classify_final_balance_in_fx_residual_v1(
      v_target_line, 0.20, '[REGRESSION_ONLY] first duplicate exercise allocation'
    ) INTO v_result;
    ...
    IF v_count_before <> 1 THEN
      RAISE EXCEPTION 'FAIL: duplicate exercise first classification did not create exactly one FX row';
    END IF;

    v_rejected := false;
    BEGIN
      PERFORM public.staff_classify_final_balance_in_fx_residual_v1(
        v_target_line, 0.20, '[REGRESSION_ONLY] duplicate rejection'
      );
    EXCEPTION WHEN OTHERS THEN
      v_rejected := position('no canonical residual remaining' in lower(SQLERRM)) > 0
        OR position('already has an active fx/card difference' in lower(SQLERRM)) > 0;
    END;
    ...
    IF NOT v_rejected OR v_count_after <> 1 THEN
      RAISE EXCEPTION 'FAIL: duplicate classification was not rejected cleanly';
    END IF;
  ...
  IF v_count_after <> 0 THEN
    RAISE EXCEPTION 'FAIL: duplicate exercise rollback left FX residue on controlled line';
  END IF;

  -- -------------------------------------------------------------------------
  -- Exact frozen-authority check again after all behavioural exercises.
  -- -------------------------------------------------------------------------
  FOR r IN SELECT * FROM _final_balance_in_fx_frozen ORDER BY object_name
  LOOP
    IF r.object_type = 'view' THEN
    ...
    END IF;
    IF v_actual IS DISTINCT FROM r.expected_md5 THEN
      RAISE EXCEPTION 'FAIL: frozen % changed after regression. expected %, actual %',
        r.object_name, r.expected_md5, v_actual;
    END IF;
  END LOOP;
