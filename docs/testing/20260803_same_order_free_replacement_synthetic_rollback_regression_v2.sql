-- Supabase-compatible synthetic rollback-only end-to-end regression v2.
-- No data-modifying CTEs. No writes survive.
...
SELECT
  gen_random_uuid() AS source_tracking_submission_id,
  gen_random_uuid() AS successor_tracking_submission_id,
  gen_random_uuid() AS source_allocation_id,
  gen_random_uuid() AS receipt_submission_id;
...
INSERT INTO public.order_tracking_submissions (
  id,order_id,courier_id,tracking_ref,tracking_date,note,
  submitted_by_operator_id,is_final_delivery_yn
)
SELECT
  i.source_tracking_submission_id,p.order_id,p.courier_id,
...
SELECT
  i.successor_tracking_submission_id,p.order_id,p.courier_id,
...
INSERT INTO public.order_tracking_line_allocations (
  ...
  base_value_gbp,discount_share_gbp,retailer_delivery_share_gbp,
  adjusted_net_value_gbp,allocation_status,allocation_basis,notes,
  allocated_by_operator_id
)
SELECT
  i.source_allocation_id,p.order_id,p.supplier_invoice_line_id,
  ...
  'allocated','operator_declaration','Synthetic rollback source allocation',
  p.operator_id
FROM fixture_parent p CROSS JOIN fixture_ids i;
...
DO $$ BEGIN
  ...
  THEN
    RAISE EXCEPTION 'Synthetic source allocation was not created with positive value.';
  END IF;
END $$;
...
SELECT public.shipper_record_package_receipt_v2(
  i.source_tracking_submission_id,
  i.receipt_submission_id,
...
JOIN public.physical_receipt_reviews pr
  ON pr.tracking_submission_id=i.source_tracking_submission_id
...
SELECT public.operator_allocate_same_order_replacement_tracking_v1(
  p.order_id,
  i.successor_tracking_submission_id,
  ARRAY[a.route_id],
  'Synthetic rollback successor allocation'
) result
FROM fixture_parent p
CROSS JOIN fixture_ids i
CROSS JOIN accepted_route a;
...
DO $$ BEGIN
  IF EXISTS (
    ...
    JOIN public.order_tracking_line_allocations s
      ON s.id=r.successor_tracking_line_allocation_id
