...
-- Corrected rollback-only authenticated post-install regression.
--
-- v1 compared queue counts directly with the readiness preview. That was not
-- equivalent to the existing queue because the queue first LEFT JOINs every
-- preview row to matching main/supplementary sales_invoices. This v1.1 proof
-- reproduces that unchanged join and grouping exactly before comparing all
-- non-classification outputs.
--
-- No operational-row INSERT, UPDATE or DELETE.
...
BEGIN
  ...
  WHERE queue_row.shipment_batch_id = v_batch;
  ...
  IF v_status IS DISTINCT FROM 'draft_exists'
     ...
     OR v_amount IS DISTINCT FROM COALESCE(v_expected_amount, 0)
     OR v_goods IS DISTINCT FROM COALESCE(v_expected_goods, 0)
     OR v_shipping IS DISTINCT FROM COALESCE(v_expected_shipping, 0)
  ...
  THEN
    RAISE EXCEPTION
      'FAIL: J040826 queue output differs from the unchanged queue aggregation';
  END IF;

  -- Prove J040826v1.
  v_batch := v_j040826v1_batch;
  ...
  THEN
    RAISE EXCEPTION
      'FAIL: J040826v1 queue output differs from the unchanged queue aggregation';
  END IF;
...
SELECT jsonb_build_object(
  'regression', 'exact_shipment_batch_draft_status_v1_1',
  'status', 'passed',
  ...
  'queue_amounts_and_counts_match_unchanged_join', true,
  'protected_mini_builds_1_to_3', true,
  'operational_rows_changed', false,
  'note',
    'Authenticated browser refresh remains the final visual acceptance.'
) AS result;
