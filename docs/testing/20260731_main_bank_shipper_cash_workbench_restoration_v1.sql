...
-- Target: confirm the historical shipper cash row family is restored without
-- changing existing workbench/freeze/batch/posting implementations.
...
-- ---------------------------------------------------------------------------
-- 2. Preservation proof: the pre-restoration function is the prior canonical
--    retailer-refund wrapper, not a reconstructed copy of its row families.
-- ---------------------------------------------------------------------------
SELECT
  position(
  ...
  ) > 0 AS canonical_adds_shipper_only_through_helper,
  position(
...
-- ---------------------------------------------------------------------------
-- 3. The restored helper retains the historical identifiers and dynamic
--    mapping contract. No Sage bank UUID is embedded in the function body.
-- ---------------------------------------------------------------------------
SELECT
  position(
...
-- ---------------------------------------------------------------------------
-- 4. Current confirmed/unfrozen shipper backlog, derived directly from source
--    tables. The restoration migration must not mutate these rows/snapshots.
-- ---------------------------------------------------------------------------
SELECT
...
-- Live baseline before restoration: five rows, GBP 102.00 total.
-- The exact count may legitimately change if accounting acts on the backlog
-- between runs; every remaining row must have Sage contact/bank/target ids.
...
-- ---------------------------------------------------------------------------
-- 5. Reversed allocations remain history only and must never qualify for the
--    restored helper's source predicate.
-- ---------------------------------------------------------------------------
SELECT
  id,
  allocation_status,
...
-- Expected helper_is_confirmed_only = true.

-- ---------------------------------------------------------------------------
-- 6. AUTHENTICATED runtime proof.
--    This UUID was the active accounting-admin test identity used for the
--    target-database verification on 31 July 2026. If that staff row changes,
--    replace only this UUID with another active accounting-admin auth_user_id.
-- ---------------------------------------------------------------------------
BEGIN;
...
-- The restored category should now return the historical row contract.
SELECT
  queue_row_id,
  source_type,
  source_id,
...
-- Contract checks over the same authenticated result.
...
-- Expected: contract_violations = 0; missing_sage_prerequisites = 0 for the
-- current Jobyco rows. Historical/other blocked data, if later introduced,
-- should be reviewed rather than silently excluded.

-- Confirm every currently confirmed/unfrozen source allocation appears in the
-- canonical shipper-category result.
WITH expected AS (
  SELECT a.id
  FROM public.main_bank_shipper_ap_allocations a
  WHERE a.allocation_status = 'confirmed'
...
--    Do not run this section unless you intend to exercise the real freeze RPC.
--    The transaction rolls back all snapshot mutation.
--
--    Choose one currently confirmed/unfrozen allocation id returned by section 4
--    and replace <ALLOCATION_UUID> below. Keep the queue shape exactly as shown.
-- ---------------------------------------------------------------------------
-- BEGIN;
...
-- -- Existing payload normalisation should leave the snapshot on the current
-- -- POST /contact_payments + VENDOR_PAYMENT + allocated_artefacts contract.
--
-- ROLLBACK;
