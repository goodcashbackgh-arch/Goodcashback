...
// No replacement of existing RPCs/authorities.
...
// Existing exceptions queue remains intact; card is flag-gated only.
...
// Dedicated server action accepts provenance, not amount.
...
// Breach protector: Save survives, approval rolls back, same bundle lock serialises race.
assert(migration.includes("protect_order_bundle_limit_breach_resolution_v1"), "breach protector missing");
assert(migration.includes("BEFORE UPDATE OF status, resolved_by_staff_id, resolved_at, resolution_notes"), "breach protector trigger too broad/narrow");
...
// UPDATE companion is exact and provenance-safe.
...
// Dedicated RPC boundaries.
assert(migration.includes("staff_approve_order_supplier_price_increase_v1(\n  p_order_id uuid,\n  p_supplier_invoice_id uuid,"), "price RPC signature wrong");
...
// Do not create a DVA lock-order dependency inside this feature.
...
// Governing scope explicitly excludes discarded architecture/no-op migration debris.
