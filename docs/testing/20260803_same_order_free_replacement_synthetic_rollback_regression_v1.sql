...
WHERE o.shipper_id IS NOT NULL
  ...
  AND NOT EXISTS (
    ...
    WHERE dl.supplier_invoice_line_id=src.supplier_invoice_line_id AND dl.resolved_at IS NULL
  )
  AND NOT EXISTS (
...
WITH source_tracking AS (
  ...
  SELECT order_id,courier_id,'ROLLBACK-SOURCE-'||substr(gen_random_uuid()::text,1,8),
  ...
  SELECT order_id,courier_id,'ROLLBACK-SUCCESSOR-'||substr(gen_random_uuid()::text,1,8),
         current_date,'Synthetic rollback successor tracking',operator_id,false
  FROM fixture_parent
  RETURNING id,order_id
)
SELECT s.id AS source_tracking_submission_id,x.id AS successor_tracking_submission_id
FROM source_tracking s JOIN successor_tracking x ON x.order_id=s.order_id;

CREATE TEMP TABLE fixture_source_allocation ON COMMIT DROP AS
SELECT
  NULL::uuid AS id,
  NULL::uuid AS order_id,
  NULL::uuid AS supplier_invoice_line_id,
  NULL::uuid AS tracking_submission_id,
  NULL::numeric AS qty_allocated,
  NULL::numeric AS base_value_gbp,
  NULL::numeric AS discount_share_gbp,
  NULL::numeric AS retailer_delivery_share_gbp,
  NULL::numeric AS adjusted_net_value_gbp
WHERE false;
...
INSERT INTO fixture_source_allocation
...
FROM fixture_parent p CROSS JOIN fixture_tracking t CROSS JOIN fixture_source_allocation a;

CREATE TEMP TABLE fixture_review ON COMMIT DROP AS
SELECT pr.id review_id,d.id disposition_id
FROM fixture_tracking t
JOIN public.physical_receipt_reviews pr ON pr.tracking_submission_id=t.source_tracking_submission_id
...
DO $$ BEGIN
  ...
  ) THEN RAISE EXCEPTION 'Same-order acceptance created child linkage or wrong remedy state.'; END IF;
END $$;

SELECT set_config('request.jwt.claim.sub',(SELECT operator_auth_user_id::text FROM fixture_parent),true);
CREATE TEMP TABLE allocation_result ON COMMIT DROP AS
SELECT public.operator_allocate_same_order_replacement_tracking_v1(
  p.order_id,t.successor_tracking_submission_id,ARRAY[a.route_id],
