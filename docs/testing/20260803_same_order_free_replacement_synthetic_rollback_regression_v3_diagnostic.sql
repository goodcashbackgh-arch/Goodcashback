...
BEGIN
  ...
  THEN RAISE EXCEPTION 'Required existing or same-order authorities are missing.'; END IF;
END
$preflight$;

CREATE TEMP TABLE fixture_parent ON COMMIT DROP AS
SELECT o.id order_id,o.importer_id,o.shipper_id,o.sop_version,
       op.id operator_id,op.auth_user_id operator_auth_user_id,
       s.id staff_id,s.auth_user_id staff_auth_user_id,
       su.auth_user_id shipper_auth_user_id,
       src.supplier_invoice_line_id,src.qty_allocated template_qty,
       src.base_value_gbp template_base,src.discount_share_gbp template_discount,
       src.retailer_delivery_share_gbp template_delivery,
       src.adjusted_net_value_gbp template_adjusted,ots.courier_id
...
JOIN public.staff s ON COALESCE(s.active,true) AND s.role_type IN ('admin','supervisor')
JOIN public.shipper_users su ON su.shipper_id=o.shipper_id AND su.active=true
JOIN public.order_tracking_line_allocations src
  ON src.order_id=o.id AND src.qty_allocated>=1 AND src.adjusted_net_value_gbp>0
JOIN public.order_tracking_submissions ots ON ots.id=src.tracking_submission_id
WHERE o.shipper_id IS NOT NULL AND ots.courier_id IS NOT NULL
  AND NOT EXISTS (
...
SELECT gen_random_uuid() source_tracking_submission_id,
       gen_random_uuid() successor_tracking_submission_id,
       gen_random_uuid() source_allocation_id,
       gen_random_uuid() receipt_submission_id;

CREATE TEMP TABLE protection_before ON COMMIT DROP AS
SELECT md5(pg_get_functiondef('public.physical_remedy_allocation_guard_v2()'::regprocedure)) remedy_guard,
...
INSERT INTO public.order_tracking_submissions(
  id,order_id,courier_id,tracking_ref,tracking_date,note,submitted_by_operator_id,is_final_delivery_yn
)
SELECT i.source_tracking_submission_id,p.order_id,p.courier_id,
       'ROLLBACK-SOURCE-'||substr(i.source_tracking_submission_id::text,1,8),
       current_date,'Synthetic rollback source tracking',p.operator_id,false
FROM fixture_parent p CROSS JOIN fixture_ids i;

INSERT INTO public.order_tracking_submissions(
  id,order_id,courier_id,tracking_ref,tracking_date,note,submitted_by_operator_id,is_final_delivery_yn
)
SELECT i.successor_tracking_submission_id,p.order_id,p.courier_id,
       'ROLLBACK-SUCCESSOR-'||substr(i.successor_tracking_submission_id::text,1,8),
       current_date,'Synthetic rollback successor tracking',p.operator_id,false
FROM fixture_parent p CROSS JOIN fixture_ids i;

INSERT INTO public.order_tracking_line_allocations(
  id,order_id,supplier_invoice_line_id,tracking_submission_id,qty_allocated,
...
SELECT i.source_allocation_id,p.order_id,p.supplier_invoice_line_id,
       i.source_tracking_submission_id,1,
...
SELECT a.* FROM public.order_tracking_line_allocations a
JOIN fixture_ids i ON i.source_allocation_id=a.id;

DO $$ BEGIN
  IF (SELECT COUNT(*) FROM fixture_source_allocation)<>1
     OR EXISTS (SELECT 1 FROM fixture_source_allocation WHERE adjusted_net_value_gbp<=0)
  THEN RAISE EXCEPTION 'Synthetic source allocation was not created with positive value.'; END IF;
END $$;
...
SELECT public.shipper_record_package_receipt_v2(
  i.source_tracking_submission_id,i.receipt_submission_id,
  jsonb_build_array(jsonb_build_object(
    'tracking_line_allocation_id',a.id,
    'supplier_invoice_line_id',a.supplier_invoice_line_id,
    'disposition_type','damaged','quantity',1,
    'condition_note','Synthetic rollback damaged receipt'
  )),
  jsonb_build_array(jsonb_build_object(
    'storage_object_path','shipper-receipts/'||p.shipper_id::text||'/'||i.source_tracking_submission_id::text||'/synthetic-'||gen_random_uuid()::text||'.jpg',
    'original_filename','synthetic.jpg','content_type','image/jpeg','display_order',0,
    'tracking_line_allocation_id',a.id,'disposition_type','damaged'
  )),NULL,NULL
) result
FROM fixture_parent p CROSS JOIN fixture_ids i CROSS JOIN fixture_source_allocation a;

CREATE TEMP TABLE fixture_review ON COMMIT DROP AS
SELECT pr.id review_id,d.id disposition_id
FROM fixture_ids i
JOIN public.physical_receipt_reviews pr ON pr.tracking_submission_id=i.source_tracking_submission_id
JOIN public.shipper_package_receipt_line_dispositions d ON d.receipt_id=pr.receipt_id
JOIN fixture_source_allocation a ON a.id=d.tracking_line_allocation_id
ORDER BY pr.id,d.id LIMIT 1;
...
DO $$ BEGIN
  IF (SELECT COUNT(*) FROM fixture_remedy)<>1 THEN
    RAISE EXCEPTION 'Importer proposal did not create exactly one replacement remedy.';
  END IF;
END $$;
...
SELECT public.staff_decide_physical_receipt_review_v2(
  r.review_id,'approve_existing_exception',
  jsonb_build_array(jsonb_build_object(
    'remedy_allocation_id',m.remedy_id,
    'approved_remedy_type','replacement','approved_remedy_qty',1,
    'supplier_cost_mode','free_replacement'
  )),
  'retailer','Synthetic rollback supervisor approval'
) FROM fixture_review r CROSS JOIN fixture_remedy m;

CREATE TEMP TABLE fixture_dispute ON COMMIT DROP AS
SELECT d.id dispute_id,dl.id dispute_line_id
...
DO $$ BEGIN
  IF (SELECT COUNT(*) FROM fixture_dispute)<>1 THEN
    RAISE EXCEPTION 'Supervisor bridge did not create exactly one replacement dispute.';
  END IF;
END $$;
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'dispute_child_order_id',d.replacement_child_order_id,
    'dispute_line_id',dl.id,
    'line_conversation_status',dl.conversation_status,
    'line_resolved_at',dl.resolved_at,
    'line_child_order_id',dl.resolved_via_child_order_id,
    'remedy_id',r.id,
    'remedy_status',r.status,
    'remedy_supplier_cost_mode',r.supplier_cost_mode,
    'remedy_child_order_id',r.replacement_child_order_id,
    'remedy_child_tracking_allocation_id',r.replacement_child_tracking_allocation_id,
    'route_id',sr.id,
    'route_status',sr.route_status,
    'route_successor_tracking_submission_id',sr.successor_tracking_submission_id,
    'route_successor_allocation_id',sr.successor_tracking_line_allocation_id
  ),
  array_remove(ARRAY[
    CASE WHEN d.replacement_child_order_id IS NOT NULL THEN 'dispute_child_order_not_null' END,
  ...
  LEFT JOIN public.physical_replacement_same_order_routes sr ON sr.id=ar.route_id;

  IF COALESCE(array_length(v_blockers,1),0)>0 THEN
    RAISE EXCEPTION 'Same-order acceptance diagnostic failed. Blockers: %. State: %',v_blockers,v_state;
  END IF;
END
$diagnostic$;
...
SELECT public.operator_allocate_same_order_replacement_tracking_v1(
  p.order_id,i.successor_tracking_submission_id,ARRAY[a.route_id],
  'Synthetic rollback successor allocation'
) result
FROM fixture_parent p CROSS JOIN fixture_ids i CROSS JOIN accepted_route a;
...
DO $$ BEGIN
  IF EXISTS (
    ...
    WHERE r.route_status<>'tracking_allocated'
       OR r.successor_tracking_submission_id IS DISTINCT FROM i.successor_tracking_submission_id
       OR s.tracking_submission_id IS DISTINCT FROM i.successor_tracking_submission_id
       OR s.tracking_submission_id IS NOT DISTINCT FROM i.source_tracking_submission_id
  ...
  ) THEN RAISE EXCEPTION 'Successor route/allocation identity or value mismatch.'; END IF;
END $$;
...
DO $$ BEGIN
  ...
  ) THEN RAISE EXCEPTION 'Raw history did not increase or effective entitlement changed.'; END IF;
