...
DECLARE
  ...
  v_candidate_importer_id uuid;
  v_candidate_dispute_id uuid;
  v_candidate_amount numeric;
  v_candidate_gap numeric;
  v_statement_id uuid;
  v_test_line_id uuid;
  v_result jsonb;
...
BEGIN
  -- -------------------------------------------------------------------------
  -- 1. Frozen authorities must remain exactly as audited.
  -- -------------------------------------------------------------------------
  FOR r IN
    SELECT *
    FROM (VALUES
  ...
  -- -------------------------------------------------------------------------
  -- 2. Exact compatibility-view contract: same 33 deployed columns, same order.
  -- -------------------------------------------------------------------------
  ...
  IF v_order_id IS NULL OR v_line_importer_id IS NULL THEN
    RAISE EXCEPTION 'FAIL: known £20.18 line lost its funding order/importer linkage.';
  END IF;

  -- -------------------------------------------------------------------------
  -- 4. Changed writers must retain their established contracts plus local guards.
  -- -------------------------------------------------------------------------
  ...
  IF position('statement_line_control_position_v1' IN v_function_def) = 0
     ...
     OR position('confirmed_unallocated_after_gbp' IN v_function_def) = 0 THEN
    RAISE EXCEPTION 'FAIL: dispute/hold writer lost an existing branch/result field or canonical retailer-refund guard.';
  END IF;
  ...
  -- -------------------------------------------------------------------------
  -- 5. Establish real staff auth context for writer exercises.
  -- -------------------------------------------------------------------------
  ...
  -- -------------------------------------------------------------------------
  -- 6. Known fully-consumed line: final-balance and customer-FX reuse must fail
  --    before any new economic-use row survives.
  -- -------------------------------------------------------------------------
  ...
  -- Retailer-refund rejection needs a genuine dispute for the same importer,
  -- because the existing RPC validates the dispute before calculating remaining.
  SELECT d.id
    INTO v_candidate_dispute_id
  ...
  IF v_candidate_dispute_id IS NULL THEN
    RAISE EXCEPTION 'FAIL: no existing dispute for the known line importer is available to exercise canonical retailer-refund rejection.';
  END IF;
  ...
  -- -------------------------------------------------------------------------
  -- 7. VALID FINAL-BALANCE PATH. Fresh statement evidence only; target order is
  --    selected from the existing read model. Entire exercise is subtransaction-
  --    rolled back immediately after assertions.
  -- -------------------------------------------------------------------------
  SELECT s.order_id, s.importer_id,
         ROUND(LEAST(s.final_balance_due_gbp, 1.00)::numeric, 2)
    INTO v_candidate_order_id, v_candidate_importer_id, v_candidate_amount
  ...
  IF v_candidate_order_id IS NULL OR v_candidate_amount <= 0 THEN
    RAISE EXCEPTION 'FAIL: no eligible final-balance-due order exists for rollback success-path regression.';
  END IF;
  ...
  BEGIN
    INSERT INTO public.dva_statements(
      ...
      v_statement_id, v_candidate_importer_id, 'importer_dva_card_account',
      v_candidate_importer_id::text, 'Rollback final-balance success path', 'regression',
    ...
    SELECT public.staff_allocate_statement_line_to_final_balance_payment_v1(
      v_test_line_id,
      v_candidate_order_id,
      false,
      '[REGRESSION_ONLY] valid final-balance path'
    ) INTO v_result;

    IF COALESCE((v_result ->> 'ok')::boolean, false) IS NOT TRUE
       OR ROUND(COALESCE((v_result ->> 'amount_to_final_balance_gbp')::numeric, 0), 2) <> v_candidate_amount
       OR ROUND(COALESCE((v_result ->> 'statement_remaining_before_gbp')::numeric, 0), 2) <> v_candidate_amount
  ...
  -- -------------------------------------------------------------------------
  -- 8. VALID RETAILER-REFUND PATH. Fresh statement evidence, existing dispute,
  --    immediate subtransaction rollback after success assertions.
  -- -------------------------------------------------------------------------
  SELECT d.id, o.importer_id
    INTO v_candidate_dispute_id, v_candidate_importer_id
  FROM public.disputes d
  JOIN public.orders o ON o.id = d.order_id
  ...
  IF v_candidate_dispute_id IS NULL THEN
    RAISE EXCEPTION 'FAIL: no eligible dispute exists for rollback retailer-refund success-path regression.';
  END IF;
  ...
  BEGIN
    INSERT INTO public.dva_statements(
      ...
      v_statement_id, v_candidate_importer_id, 'importer_dva_card_account',
      v_candidate_importer_id::text, 'Rollback retailer-refund success path', 'regression',
  ...
  SELECT o.id, o.importer_id, ROUND(public.order_funding_gap_gbp(o.id)::numeric, 2)
    INTO v_candidate_order_id, v_candidate_importer_id, v_candidate_gap
  ...
  IF v_candidate_order_id IS NULL OR v_candidate_gap <= 0.01 THEN
    RAISE EXCEPTION 'FAIL: no eligible original order with a funding gap exists for rollback customer-FX success-path regression.';
  END IF;

  v_candidate_amount := ROUND(v_candidate_gap + 0.10, 2);
  v_statement_id := gen_random_uuid();
  v_test_line_id := gen_random_uuid();
  ...
  WHERE icl.source_entity_type = 'order'
    AND icl.source_entity_id = v_candidate_order_id;
  ...
  BEGIN
    INSERT INTO public.dva_statements(
      ...
      v_statement_id, v_candidate_importer_id, 'importer_dva_card_account',
      v_candidate_importer_id::text, 'Rollback customer-FX success path', 'regression',
    ...
    IF COALESCE((v_result ->> 'customer_fx_gain_routed_yn')::boolean, false) IS NOT TRUE
       OR ROUND(COALESCE((v_result ->> 'funding_amount_gbp')::numeric, 0), 2) <> v_candidate_gap
    ...
    IF (SELECT COUNT(*) FROM public.dva_reconciliation dr
        WHERE dr.dva_statement_line_id = v_test_line_id
          AND dr.reconciliation_type::text = 'order_funding'
          AND ABS(dr.reconciled_gbp_amount - v_candidate_gap) <= 0.005) <> 1 THEN
  ...
  -- -------------------------------------------------------------------------
  -- 10. Frozen authorities must still be unchanged after every success exercise.
  -- -------------------------------------------------------------------------
  FOR r IN
    SELECT *
    FROM (VALUES
  ...
  LOOP
    ...
    IF v_actual IS DISTINCT FROM r.expected_md5 THEN
      RAISE EXCEPTION 'FAIL: frozen % changed after behavioral exercises. expected %, actual %.', r.object_name, r.expected_md5, v_actual;
    END IF;
  END LOOP;
END
$regression$;
...
SELECT
  ...
  'Frozen authorities unchanged; exact 33-column compatibility contract preserved; both live funding+FX lines are fully consumed; invalid reuse is rejected; valid final-balance, retailer-refund and customer funding+FX paths execute successfully and roll back without residue.'::text AS details;
