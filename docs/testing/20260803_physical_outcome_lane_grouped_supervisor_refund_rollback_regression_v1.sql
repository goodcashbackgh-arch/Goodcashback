-- Rollback-only behavioral regression for grouped supervisor refund settlement-credit decisions.
-- Seeds a natural GBP 60 credit_due position using order_funding_events plus one posted invoice,
-- exercises the real grouped supervisor RPC, verifies delegated financial/audit effects and replay,
-- then rolls the entire transaction back.
...
BEGIN
  ...
  SELECT COALESCE(public.order_funding_total_gbp(s.order_id),0)
  INTO v_existing_funding;

  SELECT COALESCE(SUM(si.amount_gbp),0)
  ...
  PERFORM set_config('app.test.refund_funding_event_id',v_funding_event_id::text,true);
  PERFORM set_config('app.test.refund_invoice_id',v_invoice_id::text,true);
  ...
  WHERE order_id=s.order_id AND id<>s.dispute_id
  ...
  WHERE physical_receipt_review_id=s.physical_receipt_review_id
    AND dispute_id=s.dispute_id;
  ...
  INSERT INTO public.physical_receipt_review_dispute_links(
    ...
    s.physical_receipt_review_id,s.dispute_id,'refund',s.staff_id
  );
  ...
  SET proposed_remedy_type='refund',proposed_remedy_qty=1,
      ...
      customer_commercial_value_gbp=v_credit_due,
      supplier_cost_mode='not_applicable',
      replacement_child_order_id=NULL,replacement_child_tracking_allocation_id=NULL,
  ...
  INSERT INTO public.order_funding_events(
    ...
    'physical_outcome_lane_refund_regression',v_lane_id,
    NULL,false,s.staff_id,now(),'Rollback-only GBP 60 refund settlement-credit fixture.',
    'rollback:physical_outcome_lane_refund','physical_outcome_lane',v_lane_id,NULL
  );
  ...
  INSERT INTO public.sales_invoices(
    ...
    NULL,v_invoice_amount,'T0',jsonb_build_array(jsonb_build_object(
      'rollback_fixture',true,'amount_gbp',v_invoice_amount
    )),
  ...
  INSERT INTO public.physical_receipt_outcome_lanes(
    ...
    v_lane_id,s.order_id,s.physical_receipt_review_id,'refund','awaiting_supervisor_decision'
  );
...
BEGIN
  ...
  IF p.settlement_status<>'credit_due'
     OR ABS(p.funding_less_posted_invoice_gbp-60)>0.01
  THEN
    RAISE EXCEPTION 'FAIL: rollback fixture did not produce exact GBP 60 credit_due position: %',to_jsonb(p);
  END IF;

  IF (SELECT amount_impact_gbp FROM public.disputes WHERE id=d)<>60 THEN
    RAISE EXCEPTION 'FAIL: refund dispute amount is not GBP 60';
  END IF;
END
$fixture_assertions$;
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'FAIL: grouped refund decision result incorrect: %',r1;
  END IF;
  ...
  IF r2 IS DISTINCT FROM r1 THEN
    RAISE EXCEPTION 'FAIL: identical refund replay did not return stored result';
  END IF;
...
SELECT jsonb_build_object(
  'result','PASS',
  'regression','physical_outcome_lane_grouped_supervisor_refund_rollback_v1',
