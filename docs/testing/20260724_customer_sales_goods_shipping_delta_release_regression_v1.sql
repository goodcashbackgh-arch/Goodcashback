...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'FAIL: source resolver lacks independent exact goods/shipping deltas or protected gates';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'FAIL: old shipping suppression remains or protected progression/status scope was crossed';
  END IF;
  ...
  THEN
    RAISE EXCEPTION 'FAIL: shipment-batch shipping guard or protected financial/hold controls are missing';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'FAIL: active release draft concurrency control is missing';
  END IF;
  ...
  WHERE table_schema = 'public'
    ...
    AND column_name IN (
      'sales_invoice_id','sales_invoice_type','commercial_parent_order_id','source_shipment_batch_id',
      ...
      'customer_charge_amount_gbp','release_status','membership_fingerprint','reversed_at',
      'reversed_by_staff_id','reversal_reason'
    );
...
BEGIN
  WITH scenarios AS (
    SELECT * FROM (VALUES
      ('main_goods_shipping', false, 100.00::numeric, 10.00::numeric, 'main'::text, 110.00::numeric, NULL::text),
      ('main_goods_only', false, 100.00::numeric, 0.00::numeric, 'main'::text, 100.00::numeric, NULL::text),
      ('first_shipping_only', false, 0.00::numeric, 10.00::numeric, 'main'::text, 0.00::numeric, 'shipping_only_main_not_permitted'::text),
      ...
      ('supp_goods_shipping', true, 100.00::numeric, 10.00::numeric, 'supplementary'::text, 110.00::numeric, NULL::text)
    ) AS s(name,has_main,goods_delta,shipping_delta,expected_type,expected_charge,expected_blocker)
  ), actual AS (
    SELECT s.*,
      CASE WHEN has_main THEN 'supplementary' ELSE 'main' END::text AS actual_type,
      ROUND(goods_delta + CASE WHEN has_main OR goods_delta > 0 THEN shipping_delta ELSE 0 END,2)::numeric AS actual_charge,
      CASE
        WHEN NOT has_main AND goods_delta <= 0 AND shipping_delta > 0 THEN 'shipping_only_main_not_permitted'
        WHEN ROUND(goods_delta + CASE WHEN has_main OR goods_delta > 0 THEN shipping_delta ELSE 0 END,2) <= 0 THEN 'source_fully_released'
        ELSE NULL
  ...
  IF v_bad <> 0 THEN
    RAISE EXCEPTION 'FAIL: goods/shipping delta route matrix mismatch (%)',v_bad;
  END IF;
END $$;
...
BEGIN
  ...
  IF v_count>0 THEN RAISE EXCEPTION 'FAIL: duplicate active release drafts exist'; END IF;

  SELECT COUNT(*) INTO v_count
  FROM public.customer_sales_release_lines release_line
  JOIN public.order_tracking_line_allocations allocation_row ON allocation_row.id=release_line.tracking_line_allocation_id
  JOIN public.supplier_invoice_lines supplier_line ON supplier_line.id=allocation_row.supplier_invoice_line_id
  ...
  IF v_count>0 THEN RAISE EXCEPTION 'FAIL: release ledger exact source identity mismatch'; END IF;

  SELECT COUNT(*) INTO v_count FROM (
    SELECT release_line.tracking_line_allocation_id,
      SUM(release_line.released_qty) released_qty,MAX(allocation_row.qty_allocated) allocated_qty,
      SUM(release_line.goods_amount_gbp) released_goods,MAX(allocation_row.adjusted_net_value_gbp) allocated_goods
    FROM public.customer_sales_release_lines release_line
    JOIN public.order_tracking_line_allocations allocation_row ON allocation_row.id=release_line.tracking_line_allocation_id
  ...
  IF v_count>0 THEN RAISE EXCEPTION 'FAIL: active quantity/goods exceeds exact allocation'; END IF;

  SELECT COUNT(*) INTO v_count
  FROM public.sales_invoices customer_invoice
  LEFT JOIN LATERAL (
    SELECT COUNT(*) line_count,ROUND(COALESCE(SUM(release_line.customer_charge_amount_gbp),0),2) ledger_total
    FROM public.customer_sales_release_lines release_line
    WHERE release_line.sales_invoice_id=customer_invoice.id AND release_line.release_status='active'
  ) release_total ON true
  WHERE customer_invoice.invoice_type IN ('main','supplementary')
    AND customer_invoice.sage_status<>'void'
    AND NOT EXISTS (
      SELECT 1 FROM public.customer_sales_release_legacy_issues legacy_issue
      WHERE legacy_issue.sales_invoice_id=customer_invoice.id AND legacy_issue.resolved_at IS NULL
    )
    AND (release_total.line_count=0 OR ABS(release_total.ledger_total-customer_invoice.amount_gbp)>0.02);
  IF v_count>0 THEN RAISE EXCEPTION 'FAIL: non-void customer invoice does not match active durable membership'; END IF;
END $$;
...
BEGIN
  ...
  FROM (
    ...
    FROM public.sales_invoices customer_invoice
    JOIN public.orders parent_order ON parent_order.id=customer_invoice.order_id
    JOIN public.customer_sales_release_lines release_line
      ON release_line.sales_invoice_id=customer_invoice.id AND release_line.release_status='active'
    LEFT JOIN LATERAL (
      ...
      ORDER BY shipping_allocation.approved_at DESC NULLS LAST,shipping_allocation.created_at DESC,shipping_allocation.id DESC
      LIMIT 1
  ...
  IF v_count>0 THEN
    RAISE EXCEPTION 'FAIL: known goods-only draft remains active despite exact ready shipping';
  END IF;
  ...
  IF EXISTS (
    ...
    FROM public.sales_invoices customer_invoice
    JOIN public.orders parent_order ON parent_order.id=customer_invoice.order_id
    JOIN public.customer_sales_release_lines release_line ON release_line.sales_invoice_id=customer_invoice.id
    WHERE parent_order.order_ref='ORD-1784498556959'
      AND customer_invoice.invoice_type='main'
      AND customer_invoice.sage_status='void'
      AND ABS(customer_invoice.amount_gbp-699.97)<=0.01
      AND release_line.reversal_reason='automated_migration_correction_v1_original_draft_creator_retained_as_mandatory_actor'
    GROUP BY customer_invoice.id
    HAVING COUNT(*)<>3
    ...
    RAISE EXCEPTION 'FAIL: known void correction lacks complete audited reversed membership';
  END IF;
END $$;

ROLLBACK;

SELECT 'PASS: independent exact goods and shipment-batch shipping deltas are installed; main/repeated supplementary, immutable draft, Mini-build 1-3 provenance and planned Mini-build 4 boundaries remain protected.' AS regression_result;
