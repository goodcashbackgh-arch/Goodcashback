...
## 6. Customer correction UI: one authoritative eligibility contract and snapshot
...
A direct browser no-op call using values read earlier is not authorised because a concurrent order change could make those values stale and turn the apparent probe into a real correction. Eligibility and the form seed snapshot must therefore use the feature-owned wrapper:
...
4. read the order's current `importer_id`, `total_qty_declared`, `order_total_gbp_declared` and `funded_at` under that lock;
5. call `public.customer_correct_unprocessed_order_v1(...)` with those exact current quantity/value values and `NULL` replacement screenshots;
6. require that delegated result to be the existing successful no-op result (`ok = true`, `changed = false`); and
7. after that success, return the locked order seed values together with read-only presentation/advisory values needed by the existing UI: original screenshot count, applied credit, funded total, markup and whether the order was previously fully/effectively funded.

Because the row lock is already held and the values come from the same transaction, the delegated correction call reaches its existing no-op path after running the authoritative blockers and cannot become a stale-value mutation. The wrapper may read `order_funding_position_vw` and count existing `Original order screenshot` rows only after delegated eligibility succeeds so it can restore the existing customer credit message, immediate fully-funded amount guidance and attachment-count guidance. Those returned values are advisory/presentational only. The wrapper must not reproduce blocker decisions from them and must not insert, update or delete any business row.

The browser must seed the correction form only from the successful wrapper response. It must not first read `orders`, `operator_importers`, funding tables/views, tracking, invoice or other blocker tables to establish the form snapshot or eligibility. This ensures the values presented after a successful eligibility check are the same locked values that were actually validated.

The UI state contract is:

- `loading`: eligibility has not yet resolved;
- `eligible`: the wrapper succeeds and returns a valid snapshot, so the existing `Correct order` control is available;
...
- `check_failed`: authentication, transport, unexpected RPC error, malformed snapshot or other technical failure prevents a reliable eligibility decision, so the UI says correction availability could not be checked and offers refresh/retry behaviour.
...
Client-side form validation should preserve the already-working immediate guidance for quantity, amount, attachment constraints and the fully-funded upward-value rule using the wrapper's advisory snapshot, but it remains advisory only. The RPC remains final authority for ownership, downstream state, funding consistency and all postconditions.

The existing applied-credit message and conditional original-attachment replacement guidance are part of the non-regression UI baseline and should be preserved when the wrapper reports the corresponding values. The existing sky/blue collapsed `Correct order` treatment remains the visual baseline. This amendment changes availability resolution and failure visibility, not the surrounding customer order journey, payment cards or lifecycle presentation.
...
RLS policies are not authorised to change merely to make the correction UI work. The integration must use the feature-owned security-definer eligibility wrapper and correction RPC boundaries rather than expanding browser read authority.
...
- create or replace only `public.customer_order_correction_eligibility_v1(uuid)` as the race-safe delegating/snapshot wrapper described in section 6;
- allow that wrapper, after delegated no-op eligibility succeeds, to read only the locked order snapshot, `public.order_funding_position_vw`, and the count of `Original order screenshot` rows for the requested order to return advisory/presentation values;
...
It must not add or alter tables, views, triggers, RLS policies, credit/funding functions or adjacent workflow objects. The eligibility wrapper must not write any business row and must delegate every business eligibility decision to the correction RPC.
...
3. one new forward migration dedicated to the section 5 access-resolution correction and section 6 race-safe eligibility snapshot wrapper; and
...
4. an eligibility-wrapper simulation proves the wrapper uses the locked current quantity/value, returns delegated no-op success plus the same locked form seed values, and leaves the order/funding state unchanged;
5. the wrapper snapshot correctly returns applied-credit/funding advisory values and original screenshot count without changing those records;
6. the customer UI on an authorised clean order resolves to `eligible`, seeds the form only from the wrapper snapshot, restores the applied-credit/fully-funded guidance and exposes `Correct order` without direct browser reads of the internal blocker tables;
7. a recognised downstream/business blocker produces a visible customer-safe `blocked` state rather than silently hiding the component;
8. an injected or reproducible technical eligibility failure produces `check_failed`, not a false business blocker;
9. the existing static regression retains the earlier v1/v1.1/v1.2/v1.3/v1.4 non-regression assertions and adds the new order-scoped access/wrapper assertions rather than replacing those protections; and
10. repository/build deployment checks remain green.
...
Order access is resolved against the importer belonging to the requested order, not an arbitrary latest assignment. The correction RPC remains the single business eligibility and mutation authority. The eligibility wrapper obtains the race-safe locked current snapshot, delegates to that authority, then returns only the validated form seed and advisory presentation values. The customer UI must use that same validated snapshot, preserve the already-working correction guidance, make eligible correction usable, make genuine blockers understandable, make technical failures distinguishable, and must not duplicate or weaken the backend's financial and lifecycle protections.
