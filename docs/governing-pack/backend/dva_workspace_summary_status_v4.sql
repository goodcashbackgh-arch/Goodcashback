-- dva_workspace_summary_status_v4.sql
-- Status: EXECUTABLE SQL PATCH - RUN IN SUPABASE SQL EDITOR.
--
-- Fixes v2/v3 failures by preserving the exact live view signature of
-- public.dva_statement_line_allocation_summary_vw.
--
-- Live signature confirmed before this patch:
...
--   truth from `dva_statement_line_allocation_status_vw` and excluding voided import
--   links from active workspace results.
...
'DB-backed compatibility read model for /internal/dva-reconciliation/workspace. Exact existing column signature preserved. Uses dva_statement_line_allocation_status_vw for statement-line status, used amount, open amount, and balanced/part/unmatched truth.';
