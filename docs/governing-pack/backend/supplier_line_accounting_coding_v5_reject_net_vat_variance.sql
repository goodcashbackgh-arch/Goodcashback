-- =============================================================================
-- supplier_line_accounting_coding_v5_reject_net_vat_variance.sql
-- Multi Tenant Platform Build — require net/VAT/gross reconciliation on save-all
--
-- Run after supplier_line_accounting_coding_v4_invoice_vat_totals.sql.
-- =============================================================================
...
BEGIN
  ...
  IF NOT COALESCE(v_totals.gross_reconciled_to_invoice_yn, false) THEN
    RAISE EXCEPTION 'Gross does not reconcile to invoice. Variance %.', v_totals.gross_variance_gbp;
  END IF;

  IF v_totals.accepted_invoice_net_gbp IS NOT NULL
    AND NOT COALESCE(v_totals.net_reconciled_to_invoice_yn, false) THEN
    RAISE EXCEPTION 'Net does not reconcile to invoice. Variance %.', v_totals.net_variance_gbp;
  END IF;

  IF v_totals.accepted_invoice_vat_gbp IS NOT NULL
    AND NOT COALESCE(v_totals.vat_reconciled_to_invoice_yn, false) THEN
    RAISE EXCEPTION 'VAT does not reconcile to invoice. Variance %.', v_totals.vat_variance_gbp;
  END IF;
