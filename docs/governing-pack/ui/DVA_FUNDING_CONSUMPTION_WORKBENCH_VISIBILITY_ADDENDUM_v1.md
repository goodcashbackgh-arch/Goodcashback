...
10. before any success-path mutation, all three eligible success-path targets must be discovered read-only. If any required target is unavailable, the regression must terminate with a distinct `BLOCKED_PREREQUISITE` result and must not emit `PASS` or label runtime behaviour defective;
...
The implementation must be carried on a fresh branch based directly on audited base commit `a603382d3cb4f5030bf1e2094b5ba3c3caa359b3`. Do not cherry-pick the earlier PR #245 commits.
...
The resulting PR must contain exactly these three changed files. The earlier PR #245 must be closed unmerged and must not be merged or rebased into the clean branch.
