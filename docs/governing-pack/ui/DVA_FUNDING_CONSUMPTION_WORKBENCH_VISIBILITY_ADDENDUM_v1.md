...
The earlier read-model-only draft is superseded. This addendum is the sole governing authority for this intervention. The migration/regression must not be applied or merged unless they conform exactly to this addendum and the rollback regression completes successfully.
...
- use canonical remaining as the available amount;
...
- fail closed if canonical remaining is `<= 0.005`;
- use canonical remaining as `v_unallocated_before` for the existing retailer-refund calculation;
- **preserve the existing `v_amount > v_unallocated_before + 0.01` tolerance exactly; do not tighten, replace or otherwise alter that tolerance**;
...
- the existing retailer-refund `+0.01` amount tolerance.
...
2. the compatibility view adds exact `order_funding` consumption only: take the post-migration `pg_get_viewdef(...)`, reverse only the three authorised `order_funding` expression additions using exact unique anchors, and require the reconstructed definition MD5 to equal the audited pre-change view fingerprint `1219ed77fd0db05f59624e508fc64357`; retain the exact 33-column/order assertion. This is the preservation proof for allocation families, loyalty source/destination behaviour, `control_match_reason`, voided-import filtering and all other untouched view semantics;
...
5. a fully consumed funding line is rejected by the retailer-refund writer without committed residue while preserving the pre-existing `+0.01` tolerance;
...
10. before any success-path mutation, discover an active admin/supervisor read-only. If none exists, terminate with `BLOCKED_PREREQUISITE`. Immediately after successful staff discovery, establish transaction-local JWT/auth context for that staff user and verify `auth.uid()` before calling any auth-gated read model, including `internal_order_final_sale_settlement_v2(NULL)`. Then discover all three eligible success-path targets read-only. If any required target is unavailable, terminate with a distinct `BLOCKED_PREREQUISITE` result. Establishing transaction-local auth claims is test-session context only and is not a business-data mutation;
11. after all success-path exercises, every frozen function/view fingerprint **and all four frozen trigger fingerprints** must be rechecked;
12. every generated rollback-only statement/line must be proven absent after its subtransaction;
13. no browser amount or client-side calculation becomes monetary authority.
...
The implementation must be carried on a fresh branch based directly on audited base commit `a603382d3cb4f5030bf1e2094b5ba3c3caa359b3`. Do not cherry-pick the earlier PR #245 or #246 commits.

The clean branch must contain exactly three commits in this order:

```text
1. final governing DVA addendum
2. governed migration
3. governed rollback regression
```

The resulting PR must contain exactly these three changed files. The earlier PRs #245 and #246 must be closed unmerged and must not be merged or rebased into the clean branch.
