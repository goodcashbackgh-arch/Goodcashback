...
## Purpose
Add admin-created Shipper logins to the existing multi-tenant onboarding centre without changing the proven Customer/Importer onboarding path.
...
- Shipper-login creation is Admin-only in this extension. Supervisor user-management/scoping remains governed by the existing supervisor-scope contract and is not expanded here.
...
- create one disposable Shipper login from Internal Onboarding as Admin;
...
- existing admin, supervisor, shipper, importer/customer access is not regressed.
