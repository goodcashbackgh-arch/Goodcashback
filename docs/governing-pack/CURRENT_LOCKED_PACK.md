...
19. Shipper Shipment Batch Bulk Selection UI Addendum v1 (`docs/governing-pack/ui/SHIPPER_SHIPMENT_BATCH_BULK_SELECTION_UI_ADDENDUM_v1.md`) — locks Select all / Clear selection / selected-count behaviour on `/shipper/shipments/new`, preserves one logical selection across mobile and desktop presentations, and forbids backend, shipment-eligibility, membership, accounting, VAT or downstream scope changes.
