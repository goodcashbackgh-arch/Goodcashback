...
-- VAT HMRC rule stress test harness v2
...
--   2. Run it.
--   3. Review the result sets near the end.
--   4. The final ROLLBACK removes synthetic test data and rollback-only mapping overrides.
--
-- Accounting note confirmed 2026-06-01:
--   Bank fees are VAT exempt for this model. VAT18 asserts no Box 4 input VAT reclaim.
--   Box 7 treatment for exempt bank charges remains policy-controlled separately if required.
...
BEGIN
  ...
  INSERT INTO public.sage_mapping_settings (
    mapping_code, mapping_group, display_name, description, value_kind, required_for,
    sage_external_id, sage_display_name, is_active, configured_at, configured_by_staff_id, notes
  )
  VALUES
...
BEGIN
  ...
  INSERT INTO public.vat_return_runs (
    return_period_label, period_start_date, period_end_date, status,
    generated_by_staff_id, generated_by_auth_user_id,
    expected_box1_gbp, expected_box2_gbp, expected_box3_gbp,
    expected_box4_gbp, expected_box5_gbp,
    expected_box6_gbp, expected_box7_gbp, expected_box8_gbp, expected_box9_gbp,
    source_counts_json, blockers_summary_json, notes
  ) VALUES (
    'VAT STRESS ' || p_label,
    DATE '2036-01-01', DATE '2036-03-31', 'draft',
    v_staff_id, current_setting('request.jwt.claim.sub', true)::uuid,
    p_box1, 0, p_box1,
    p_box4, p_box1 - p_box4,
    p_box6, p_box7, 0, 0,
    jsonb_build_object('stress_harness', 'VAT_HMRC_RULE_STRESS_TEST_HARNESS_v2'),
  ...
  INSERT INTO public.vat_return_sage_reconstruction_snapshots (
    vat_return_run_id, period_start_date, period_end_date, status, source_basis,
    box1_gbp, box2_gbp, box3_gbp, box4_gbp, box5_gbp, box6_gbp, box7_gbp, box8_gbp, box9_gbp,
    source_counts, source_summary, warning_notes, created_by_staff_id
  ) VALUES (
    v_run_id, DATE '2036-01-01', DATE '2036-03-31', 'reconstructed', 'stress_harness_sage_natural_snapshot',
    p_sage_box1, 0, p_sage_box1, p_sage_box4, p_sage_box1 - p_sage_box4, p_sage_box6, p_sage_box7, 0, 0,
...
BEGIN
  INSERT INTO public.vat_return_run_lines (
    vat_return_run_id, line_kind, source_table, source_id, source_ref,
    source_json, source_lineage_json,
    ...
    p_run_id, p_line_kind, 'stress_harness', gen_random_uuid(), p_line_kind || ':' || gen_random_uuid()::text,
    jsonb_build_object('source', 'VAT_HMRC_RULE_STRESS_TEST_HARNESS_v2'),
    jsonb_build_object('contract_test', true, 'reason', p_reason),
    p_box_number, p_direction, p_amount,
    CASE WHEN p_box_number IN (1,4) THEN p_amount ELSE 0 END,
    'stress_harness_expected_vat_box_amount', DATE '2036-01-15', 'VAT STRESS',
    p_natural_sage_covered, p_adjustment_required, p_reason, 'active'
...
BEGIN
  ...
  -- Dry-run validate one created Box 6 journal to prove /journals mechanics and mapping route.
  ...
  -- VAT04: New period starts, but no Sage invoice/correction exists: no automatic reversal.
  ...
  -- VAT06: Sales invoice marked posted without Sage id should be blocked/excluded by refresh guard, covered by live-tested migration 20260601_sales_invoice_integrity_and_vat_refresh_guard_v1.sql.
  PERFORM pg_temp.gcb_assert('VAT06', 'Posted-without-Sage-id sales invoice is guarded by live DB migration', 'covered by live-tested DB integrity guard', 'see migration 20260601_sales_invoice_integrity_and_vat_refresh_guard_v1.sql', true, jsonb_build_object('guard', 'sales_invoices_posted_requires_sage_confirmation_chk'));
  ...
  -- VAT08: Non-void credit note without linked invoice is guarded by DB constraint.
  PERFORM pg_temp.gcb_assert('VAT08', 'Credit note without linked original invoice is guarded by DB constraint', 'covered by live-tested DB integrity guard', 'see migration 20260601_sales_invoice_integrity_and_vat_refresh_guard_v1.sql', true, jsonb_build_object('guard', 'sales_invoices_credit_note_requires_link_chk'));
  ...
  -- VAT15: Retailer refund without credit note after VAT claimed remains admin-review controlled; do not auto-create VAT reclaim/increase.
  PERFORM pg_temp.gcb_assert('VAT15', 'Retailer refund without credit note remains admin-review controlled', 'manual/admin VAT review before Box 4/7 decrease', 'covered by contract; source-engine scenario pending refund evidence objects', true, jsonb_build_object('treatment', 'admin_review_required_if_original_input_vat_claimed'));
  ...
  FOR v_cfg IN
    SELECT jsonb_build_object(
      ...
      'direction', direction,
      'platform_box1', platform_box1,
      'platform_box4', platform_box4,
      'platform_box6', platform_box6,
      'platform_box7', platform_box7,
      'sage_box1', sage_box1,
      'sage_box4', sage_box4,
      'sage_box6', sage_box6,
      'sage_box7', sage_box7,
  ...
  LOOP
    v_target_box := (v_cfg->>'target_box')::integer;
    v_direction := v_cfg->>'direction';
    v_expected_debit := (v_cfg->>'expected_debit')::numeric;
    v_expected_credit := (v_cfg->>'expected_credit')::numeric;

    v_run := pg_temp.gcb_make_run(
      'VAT19 movement ' || v_target_box || ' ' || v_direction,
      (v_cfg->>'platform_box1')::numeric,
      (v_cfg->>'platform_box4')::numeric,
      (v_cfg->>'platform_box6')::numeric,
      (v_cfg->>'platform_box7')::numeric,
      (v_cfg->>'sage_box1')::numeric,
      (v_cfg->>'sage_box4')::numeric,
      (v_cfg->>'sage_box6')::numeric,
      (v_cfg->>'sage_box7')::numeric
    );

    v_line := pg_temp.gcb_add_vat_line(
      v_run,
      'vat19_journal_movement_contract',
      v_target_box,
      CASE WHEN v_cfg->>'source_direction' = 'decrease' THEN 'decrease' ELSE 'increase' END,
      10,
      true,
      false,
      'journal_movement_contract_stress'
    );

    v_preview := public.staff_preview_vat_adjustment_journal_proposals_v1(v_run, 0.01);
    v_actual_debit := COALESCE((v_preview #>> '{proposals,0,proposed_vat_box_journal_line,debit_amount_gbp}')::numeric, -999999);
    v_actual_credit := COALESCE((v_preview #>> '{proposals,0,proposed_vat_box_journal_line,credit_amount_gbp}')::numeric, -999999);

    PERFORM pg_temp.gcb_assert(
      'VAT19-B' || v_target_box || '-' || v_direction,
      'VAT journal movement matches debit/credit contract',
      format('vat_box_line debit=%s credit=%s include_on_tax_return=true', v_expected_debit, v_expected_credit),
      v_preview::text,
      COALESCE((v_preview->>'proposal_count')::int, -1) = 1
        AND v_preview #>> '{proposals,0,target_box}' = v_target_box::text
        AND v_preview #>> '{proposals,0,direction}' = v_direction
        AND v_actual_debit = v_expected_debit
        AND v_actual_credit = v_expected_credit
        AND (v_preview #>> '{proposals,0,proposed_vat_box_journal_line,include_on_tax_return}') = 'true'
        AND (v_preview #>> '{proposals,0,proposed_balancing_journal_line,include_on_tax_return}') = 'false',
      jsonb_build_object('config', v_cfg, 'preview', v_preview)
    );
  END LOOP;
  ...
  INSERT INTO public.vat_return_blockers (
    vat_return_run_id, blocker_code, severity, owner_role, source_table, source_id, source_ref, message, required_action, status
  ) VALUES (
    v_run, 'stress_open_blocker_should_prevent_materialise', 'blocker', 'admin', 'stress_harness', v_line, 'VAT20:' || v_line::text,
    'Stress harness open blocker.', 'Materialisation/posting must wait until blocker is resolved.', 'open'
  );
...
SELECT scenario_code, scenario_name, passed, expected, actual, detail
