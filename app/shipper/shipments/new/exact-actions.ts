...
export async function createExactShipmentBatchAction(formData: FormData) {
  ...
  const selected = formData
    .getAll("tracking_submission_ids")
    .filter((value): value is string =>
      typeof value === "string" && value.trim().length > 0
    );

  if (!importerId) {
    redirect("/shipper/shipments/new?error=Choose%20an%20importer.");
  }
  if (!bookingRef) {
    redirect(
      `/shipper/shipments/new?importer=${encodeURIComponent(importerId)}&error=Booking%20reference%20is%20required.`
    );
  }
  if (selected.length === 0) {
    redirect(
      `/shipper/shipments/new?importer=${encodeURIComponent(importerId)}&error=Select%20at%20least%20one%20received-clean%20package.`
    );
  }
  ...
  if (error) {
    redirect(
      `/shipper/shipments/new?importer=${encodeURIComponent(importerId)}&error=${encodeURIComponent(error.message)}`
    );
  }
  ...
  redirect(
    `/shipper/shipments/${data}?success=${encodeURIComponent("Shipment batch created")}`
  );
}
