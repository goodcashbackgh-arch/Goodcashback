      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <p className="mt-3 max-w-4xl text-sm leading-6 text-slate-600">
            View shipment batches created from received-clean packages. This is package/shipment truth only: no COS generation, Sage posting, VAT clearance, or shipping apportionment.
          </p>
              ...
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-100 text-xs uppercase tracking-wide text-slate-500">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left">Container/BOL</th>
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {rows.map((batch) => (
                    <tr key={batch.id}>
                      ...
                      <td className="px-3 py-2">{batch.container_ref ?? "—"} / {batch.bol_ref ?? "—"}</td>
                      ...
                      <td className="px-3 py-2"><Link href={`/shipper/shipments/${batch.id}`} className="font-semibold text-sky-700 underline">View detail</Link></td>
                    </tr>
                  ))}
                </tbody>
              </table>
