...
function borderStatusClass(status: string | null | undefined) {
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-2 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            ...
            <span className={`w-fit rounded-full px-3 py-1 text-sm font-semibold ${statusClass(first?.groupage_status)}`}>{movementLabel(first?.groupage_status)}</span>
          </div>
        ...
        {first ? <>
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between"><div><h2 className="text-xl font-semibold">Movement facts</h2><p className="mt-2 text-sm leading-6 text-slate-600">Source profile facts are snapshotted from admin/onboarding. Transport facts below are entered for this movement and applied to included batches.</p></div>{first.groupage_status === "movement_facts_ready" ? <span className="w-fit rounded-full bg-emerald-100 px-3 py-1 text-sm font-semibold text-emerald-800">Facts ready</span> : <span className="w-fit rounded-full bg-amber-100 px-3 py-1 text-sm font-semibold text-amber-800">Facts incomplete</span>}</div>
            ...
            {editable ? <details className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4" open={first.groupage_status !== "movement_facts_ready"}>
