    ...
    <main className="min-h-screen space-y-5 bg-slate-50 p-4 md:p-6">
      <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
          <div><h1 className="text-2xl font-semibold text-slate-950">Physical Receipt Exceptions</h1><p className="mt-1 text-sm text-slate-600">Affected receipt quantities awaiting importer routing.</p></div>
      ...
      <section className="grid gap-3">
        {reviews.length === 0 ? <div className="rounded-2xl border border-slate-200 bg-white p-6 text-slate-600">No physical receipt reviews are visible.</div> : reviews.map((review) => {
          const needsAction = ["awaiting_importer_proposal", "returned_for_information"].includes(review.status);
          return <Link key={review.id} href={`/importer/physical-receipts/${review.id}`} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:border-sky-300">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <span className={`rounded-full px-3 py-1 text-xs font-semibold ${needsAction ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-700"}`}>{review.status.replaceAll("_", " ")}</span>
            </div>
