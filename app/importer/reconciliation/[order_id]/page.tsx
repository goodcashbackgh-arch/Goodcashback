...
export default async function Page({params,searchParams}:{params:Promise<{order_id:string}>;searchParams?:Promise<Search>}) {
  ...
  const selectable=lines.filter(l=>!progressed(l)&&!disputes.has(l.id)&&!resolutions.has(l.id)&&Number(l.amount_inc_vat_gbp)>=0&&Number(l.qty)<=remainingQty&&Number(l.amount_inc_vat_gbp)<=remainingValue+0.01);
  const unresolved=lines.filter(l=>!progressed(l)&&!disputes.has(l.id)&&!resolutions.has(l.id));
  const exceptionEligible=unresolved.filter(l=>Number(l.amount_inc_vat_gbp)>=0);
