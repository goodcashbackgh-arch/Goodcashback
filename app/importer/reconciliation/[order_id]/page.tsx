            ...
            {invoiceError ? (
              ...
              <div className="mt-4 space-y-4">
                ...
                <div className="flex flex-wrap gap-3">
                  <a href={invoice.invoice_pdf_url} target="_blank" rel="noreferrer" className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">
                    Open invoice
                  </a>
                  <a href={invoice.invoice_pdf_url} target="_blank" rel="noreferrer" download className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-800">
                    Download invoice
                  </a>
                </div>
              </div>
          ...
          <>
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              ...
              <form action={addManualSupplierInvoiceLineAction} className="mt-4 grid gap-3 md:grid-cols-6">
                ...
                <div className="flex items-end md:col-span-6">
                  <button type="submit" className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Add manual line</button>
                </div>
              </form>
            </section>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <p className="text-sm text-slate-600">Progressed status updates only the clean invoiceable subset. It does not complete the order.</p>
              </div>
                    ...
                    return (
                      <article key={line.id} className={`rounded-2xl border p-4 ${progressed ? "border-emerald-300 bg-emerald-50/60" : "border-slate-200 bg-white"}`}>
                        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                          <div className="text-sm text-slate-600">Line {line.line_order} · {line.line_source}</div>
                        ...
                        <form action={updateSupplierInvoiceLineAction} className="grid gap-3 md:grid-cols-12">
                          ...
                          <label className="space-y-1 text-sm md:col-span-6">
                            <span className="text-xs uppercase tracking-wide text-slate-500">description</span>
                            <input name="description" defaultValue={line.description} required readOnly={isOcrLine} title={isOcrLine ? "OCR description is source evidence and cannot be changed." : undefined} className={`w-full rounded-xl border border-slate-300 px-3 py-2 ${isOcrLine ? "bg-slate-100 text-slate-600" : ""}`} />
                            {isOcrLine ? <span className="text-xs text-slate-500">OCR source description is preserved for audit.</span> : null}
                          </label>
                          ...
                          <div className="md:col-span-12 pt-1">
                            <button type="submit" className="rounded-xl bg-sky-600 px-3 py-2 text-sm font-semibold text-white hover:bg-sky-500">Save line</button>
                          </div>
                        </form>
