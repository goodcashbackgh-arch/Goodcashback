...
type Adjustment = { supplier_invoice_id:string; adjustment_type:string; amount_gbp:number; approval_status:string|null };
...
export default async function Page({params,searchParams}:{params:Promise<{order_id:string}>;searchParams?:Promise<Search>}) {
  ...
  const [{data:allLineRows,error:linesError},{data:adjustmentRows}] = await Promise.all([
    invoiceIds.length?supabase.from("supplier_invoice_lines").select("id, supplier_invoice_id, line_order, line_source, retailer_sku, description, qty, size, amount_inc_vat_gbp, qty_confirmed, amount_confirmed, eligible_for_invoice_yn").in("supplier_invoice_id",invoiceIds).order("line_order"):Promise.resolve({data:[] as Line[],error:null}),
    invoiceIds.length?supabase.from("order_value_adjustments").select("supplier_invoice_id, adjustment_type, amount_gbp, approval_status").in("supplier_invoice_id",invoiceIds).neq("approval_status","rejected"):Promise.resolve({data:[] as Adjustment[],error:null}),
  ]);
  ...
  const selectedAdjustments=((adjustmentRows??[]) as Adjustment[]).filter(a=>a.supplier_invoice_id===invoice?.id);
  const declaredDelivery=round2(selectedAdjustments.filter(a=>a.adjustment_type==="retailer_delivery").reduce((sum,a)=>sum+Number(a.amount_gbp??0),0));
  const declaredDiscount=round2(selectedAdjustments.filter(a=>a.adjustment_type==="retailer_discount").reduce((sum,a)=>sum+Number(a.amount_gbp??0),0));
  const deliveryCandidates=lines.filter(l=>Number(l.amount_inc_vat_gbp)>0&&isDeliveryDescription(l.description));
  const discountCandidates=lines.filter(l=>Number(l.amount_inc_vat_gbp)<0&&isDiscountDescription(l.description));
  const extractedDelivery=round2(deliveryCandidates.reduce((sum,l)=>sum+Number(l.amount_inc_vat_gbp??0),0));
  const extractedDiscount=round2(Math.abs(discountCandidates.reduce((sum,l)=>sum+Number(l.amount_inc_vat_gbp??0),0)));
  const deliveryMatched=declaredDelivery>0&&extractedDelivery>0&&Math.abs(extractedDelivery-declaredDelivery)<=0.01;
  const discountMatched=declaredDiscount>0&&extractedDiscount>0&&Math.abs(extractedDiscount-declaredDiscount)<=0.01;
  const matchedDeliveryIds=new Set(deliveryMatched?deliveryCandidates.map(l=>l.id):[]);
  const matchedDiscountIds=new Set(discountMatched?discountCandidates.map(l=>l.id):[]);
  const nonPhysicalLineIds=new Set(lines.filter(l=>Number(l.amount_inc_vat_gbp)<0).map(l=>l.id));
  for(const lineId of matchedDeliveryIds) nonPhysicalLineIds.add(lineId);
  for(const lineId of matchedDiscountIds) nonPhysicalLineIds.add(lineId);

  const unresolved=lines.filter(l=>!progressed(l)&&!disputes.has(l.id)&&!resolutions.has(l.id));
  const unresolvedMatchedFinancialOffset=round2(unresolved.filter(l=>matchedDeliveryIds.has(l.id)||matchedDiscountIds.has(l.id)).reduce((sum,l)=>sum+Number(l.amount_inc_vat_gbp??0),0));
  const physicalRemainingValue=Math.max(0,round2(remainingValue-unresolvedMatchedFinancialOffset));
  const selectable=unresolved.filter(l=>!nonPhysicalLineIds.has(l.id)&&Number(l.amount_inc_vat_gbp)>=0&&Number(l.qty)<=remainingQty&&Number(l.amount_inc_vat_gbp)<=physicalRemainingValue+0.01);
  const exceptionEligible=unresolved.filter(l=>!nonPhysicalLineIds.has(l.id));
