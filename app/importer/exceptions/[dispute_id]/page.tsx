...
function renderExpandableMessage(body: string | null | undefined) {
  ...
  if (text.length <= RETAILER_MESSAGE_PREVIEW_LENGTH) {
    return <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{text}</p>;
  }

  const preview = `${text.slice(0, RETAILER_MESSAGE_PREVIEW_LENGTH).trimEnd()}…`;

  return (
    <div className="mt-2 text-sm leading-6 text-slate-700">
      <p className="whitespace-pre-wrap">{preview}</p>
      <details className="mt-2 rounded-xl border border-slate-200 bg-white p-3">
        <summary className="cursor-pointer text-sm font-semibold text-sky-700">Show full retailer response</summary>
        <p className="mt-3 whitespace-pre-wrap text-slate-700">{text}</p>
      </details>
    </div>
  );
}
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 border-t border-slate-200 pt-5">
            ...
            {retailerReplyMessages.length > 0 ? (
              <div className="mt-4 space-y-3">
                {[...retailerReplyMessages].reverse().map((message) => (
              ...
              <p className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">No retailer response text has been saved yet.</p>
            )}
          </div>
        </section>
