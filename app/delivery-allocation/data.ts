...
export async function loadDeliveryAllocationData(
  ...
  if (orderError || !order) return { data: null, error: orderError?.message ?? "Order not found." };
  ...
  const invoices: DeliveryAllocationInvoice[] = ((invoiceRows ?? []) as any[])
    ...
    .map((row) => ({ id: String(row.id), invoice_ref: row.invoice_ref ?? null, uploaded_at: row.uploaded_at ?? null, review_status: row.review_status ?? null }));
  ...
  if (trackingError) return { data: null, error: trackingError.message };
  ...
  if (allocationError) return { data: null, error: allocationError.message };

  const { data: replacementRoutes, error: replacementRoutesError } = await db
    .from("physical_replacement_same_order_routes")
    .select("successor_tracking_line_allocation_id")
    .eq("order_id", orderId)
    .eq("route_status", "tracking_allocated")
    .not("successor_tracking_line_allocation_id", "is", null);
  if (replacementRoutesError) return { data: null, error: replacementRoutesError.message };

  const successorAllocationIds = new Set(
    ((replacementRoutes ?? []) as any[])
      .map((row) => String(row.successor_tracking_line_allocation_id ?? ""))
      .filter(Boolean)
  );
  ...
  const { data: adjustmentRows, error: adjustmentError } = await db
    ...
    .eq("order_id", orderId);
  if (adjustmentError) return { data: null, error: adjustmentError.message };
  ...
  return {
    data: {
      ...
      allocations: ((allocationRows ?? []) as any[]).map((row) => ({
        ...
        counts_toward_ordinary_remaining: !successorAllocationIds.has(String(row.id)),
      })),
      adjustments: {
