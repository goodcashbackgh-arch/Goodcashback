...
function blockerLabel(blocker: string | null) {
  switch (blocker) {
    case "exact_physical_receipt":
      return "contents frozen by exact receipt";
    case "exact_shipment_snapshot":
      return "contents frozen by shipment snapshot";
    default:
...
export default function DeliveryAllocationBulkControls({
  ...
  return (
    <>
      <form
        id={FORM_ID}
        action={saveBulkDeliveryAllocationAction}
        ...
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          <label className="space-y-1 text-sm">
            <span className="text-xs font-semibold uppercase tracking-wide text-sky-800">
              Tracking ref / package for selected items
            </span>
            <select
            ...
            >
              ...
              {tracking.map((row) => (
                <option key={row.id} value={row.id} disabled={!row.acceptsNewAllocations}>
                  {row.label}{row.acceptsNewAllocations ? "" : ` — ${blockerLabel(row.blocker)}`}
                </option>
              ))}
            </select>
          </label>
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="rounded-xl border border-sky-300 bg-white px-3 py-2 text-sm font-semibold text-sky-900 hover:bg-sky-100 disabled:cursor-not-allowed disabled:opacity-50"
            >
            ...
            <button
              ...
              className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50"
            >
              Clear
            </button>
          </div>
        </div>
        <p className="mt-3 text-sm font-medium text-sky-950">
          {selectedCount} of {selectableCount} available item{selectableCount === 1 ? "" : "s"} selected
        </p>
        <p className="mt-1 text-xs leading-5 text-sky-800">
          Bulk allocation uses the full current remaining quantity of every selected item. Use the existing individual item form for a partial quantity.
        </p>
      </form>
        ...
        >
          <div className="min-w-0 text-sm">
            <p className="font-semibold text-slate-950">
              {selectedCount} item{selectedCount === 1 ? "" : "s"} · {selectedQty} unit{Math.abs(selectedQty - 1) < 0.0001 ? "" : "s"}
            </p>
            <p className="truncate text-xs text-slate-600">
              {selectedTracking?.label ?? "Select a tracking ref/package"}
            </p>
          </div>
          ...
          <button
            form={FORM_ID}
            type="submit"
            disabled={!trackingId || !confirmed || selectedCount === 0}
