  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-slate-50 p-4 pb-24 text-slate-950 xl:p-6 xl:pb-10">
      ...
      <section className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        {thresholdMet && finalBalanceDueGbp > 0.01 ? <div className="rounded-[1.5rem] border border-amber-100 bg-amber-50/70 p-4 shadow-sm"><p className="text-xs font-black uppercase tracking-wide text-amber-700">Balance due</p><p className="mt-2 text-2xl font-black text-amber-950">{money(finalBalanceDueGbp)}</p><p className="mt-1 text-xs font-bold text-amber-800">{effectiveRate ? localAmount(finalBalanceDueLocal, currencyCode) : "Local guide unavailable"}</p></div> : null}
