...
function saleDocumentLabel(value: string | null | undefined) {
  if (value === "supplementary") return "Final order adjustment";
  if (value === "credit_note") return "Order credit";
  return "Final order document";
}
...
export default async function CustomerOrderOperationsPage({ params, searchParams }: { params: Promise<{ order_id: string }>; searchParams?: Promise<{ success?: string; error?: string }> }) {
  ...
  if (!audienceStatusRes.data) {
    return <main className="min-h-screen bg-slate-50 p-6 text-slate-950"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><p className="font-semibold">Customer status unavailable.</p><p className="mt-1">This page is blocked to avoid showing stale order information.</p></div></main>;
  }
  ...
  const statusLabel = reviewHref ? "Ready for your review" : cleanUiText(canonicalStatus.customer_status_label ?? "In progress");
  ...
  const nextActionTitle = reviewHref ? "Review items before shipment" : cleanUiText(canonicalStatus.customer_next_action ?? "No action needed right now");
  const nextActionBody = reviewHref
    ...
    : !thresholdMet && appliedCreditGbp > 0.01
      ? `Account credit of ${money(appliedCreditGbp)} has been applied. ${money(initialCashDueGbp)} remains due before this order can continue${effectiveRate ? ` (${localAmount(visibleCashDueLocal, currencyCode)}).` : "."}`
      : !thresholdMet
        ? "The accepted estimate needs to be paid before this order can continue."
        : finalBalanceDueGbp > 0.01
          ? `The final order value is now confirmed. Remaining order balance: ${money(finalBalanceDueGbp)}${effectiveRate ? ` (${localAmount(finalBalanceDueLocal, currencyCode)}).` : "."}`
          : orderComplete
  ...
  const journey = [
    ...
    { label: "Order value confirmed", done: finalSaleValueConfirmed },
    ...
    <main className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-slate-50 p-4 pb-24 text-slate-950 xl:p-6 xl:pb-10">
      ...
      <header className="mt-4 overflow-hidden rounded-[2rem] border border-sky-100 bg-white shadow-sm">
        ...
        <div className="p-5 xl:flex xl:items-start xl:justify-between xl:gap-6 xl:p-7">
          <div className="min-w-0"><p className="text-xs font-black uppercase tracking-[0.28em] text-sky-600">Customer order</p><h1 className="mt-2 text-4xl font-black tracking-tight xl:text-5xl">{orderTitle}</h1><p className="mt-2 text-sm font-semibold text-slate-600">{itemLabel} · Ref: {order.order_ref ?? orderId}</p><p className="mt-1 text-xs font-black uppercase tracking-wide text-slate-500">Payment reference: {order.payment_auth_id ?? "Not assigned"}</p></div>
          <div className="mt-5 xl:mt-0 xl:min-w-72"><div className={`rounded-2xl border p-4 ${toneCardClass(tone)}`}><p className="text-xs font-black uppercase tracking-wide opacity-70">Current status</p><p className="mt-1 text-2xl font-black">{statusLabel}</p><p className="mt-3 text-xs font-black uppercase tracking-wide opacity-70">Next step</p><p className="mt-1 text-sm font-black">{nextActionTitle}</p>{appliedCreditGbp > 0.01 && !thresholdMet ? <p className="mt-3 text-xs font-bold opacity-80">Account credit applied: {money(appliedCreditGbp)} · Amount to pay: {money(initialCashDueGbp)}</p> : null}</div></div>
          {qp.success ? <p className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm font-semibold text-emerald-800 xl:col-span-2">{cleanUiText(qp.success)}</p> : null}
          {qp.error ? <p className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm font-semibold text-rose-800 xl:col-span-2">{cleanUiText(qp.error)}</p> : null}
        </div>
      </header>
      ...
      <section className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Accepted estimate</p><p className="mt-2 text-2xl font-black">{money(acceptedEstimateGbp)}</p><p className="mt-1 text-xs font-bold text-slate-500">Payment reference: {order.payment_auth_id ?? "Not assigned"}</p></div>
        {hasAmountReceived ? <div className="rounded-[1.5rem] border border-cyan-100 bg-cyan-50/70 p-4 shadow-sm"><p className="text-xs font-black uppercase tracking-wide text-cyan-700">Amount received</p><p className="mt-2 text-2xl font-black text-cyan-950">{money(amountReceivedGbp)}</p>{appliedCreditGbp > 0.01 ? <p className="mt-1 text-xs font-bold text-cyan-800">Account credit applied: {money(appliedCreditGbp)}</p> : null}{finalBalancePaymentGbp > 0.01 ? <p className="mt-1 text-xs font-bold text-cyan-800">Remaining balance payment: {money(finalBalancePaymentGbp)}</p> : null}</div> : null}
        {!thresholdMet && visibleCashDueGbp > 0.01 ? <div className="rounded-[1.5rem] border border-amber-100 bg-amber-50/70 p-4 shadow-sm"><p className="text-xs font-black uppercase tracking-wide text-amber-700">Amount to pay</p><p className="mt-2 text-2xl font-black text-amber-950">{money(visibleCashDueGbp)}</p><p className="mt-1 text-xs font-bold text-amber-800">{effectiveRate ? localAmount(visibleCashDueLocal, currencyCode) : "Local guide unavailable"}</p></div> : null}
        {finalSaleValueConfirmed ? <div className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Final order value</p><p className="mt-2 text-2xl font-black">{money(finalSaleValueGbp)}</p><p className="mt-1 text-xs font-bold text-slate-500">After final order documents and credits</p></div> : null}
        {thresholdMet && finalBalanceDueGbp > 0.01 ? <div className="rounded-[1.5rem] border border-amber-100 bg-amber-50/70 p-4 shadow-sm"><p className="text-xs font-black uppercase tracking-wide text-amber-700">Remaining order balance</p><p className="mt-2 text-2xl font-black text-amber-950">{money(finalBalanceDueGbp)}</p><p className="mt-1 text-xs font-bold text-amber-800">{effectiveRate ? localAmount(finalBalanceDueLocal, currencyCode) : "Local guide unavailable"}</p></div> : null}
      ...
      <section className="mt-5 grid gap-4 xl:grid-cols-3">
        ...
        <article className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-wide text-slate-500">Final order documents</p><h3 className="mt-2 text-xl font-black">{finalSaleValueConfirmed ? `${invoices.length} ${invoices.length === 1 ? "document" : "documents"} issued` : "Not available yet"}</h3><div className="mt-3 text-sm leading-6 text-slate-700">{finalSaleValueConfirmed ? <><p>Final order value: <span className="font-black text-slate-950">{money(finalSaleValueGbp)}</span></p><p className="text-xs font-semibold text-slate-500">Payment reference: {order.payment_auth_id ?? "Not assigned"}</p><div className="mt-3 space-y-2">{invoices.map((invoice, index) => <div key={invoice.id} className="rounded-2xl bg-slate-50 p-3 ring-1 ring-slate-100"><p className="font-black text-slate-950">{saleDocumentLabel(invoice.invoice_type)} · {signedMoney(saleDocumentSignedAmount(invoice))}</p><p className="text-xs font-semibold text-slate-500">Issued {formatDate(invoice.sage_invoice_date ?? invoice.sage_posted_at)} · Document {index + 1}</p><a href={`/customer/orders/${orderId}/invoice-pdf/${invoice.id}`} className="mt-3 inline-flex rounded-xl bg-slate-950 px-3 py-2 text-xs font-black text-white">Download order document</a></div>)}</div></> : <p>Your final order documents will appear once issued.</p>}</div></article>
      ...
      {visibleCashDueGbp > 0.01 || pendingCreditGbp > 0.01 || appliedCreditGbp > 0.01 || availableCreditGbp > 0.01 ? <details className="mt-5 rounded-[1.75rem] border border-cyan-100 bg-cyan-50/70 p-5 shadow-sm" open={visibleCashDueGbp > 0.01}><summary className="cursor-pointer list-none text-xl font-black text-cyan-950">Credit and payment details</summary><p className="mt-2 text-sm leading-6 text-slate-700">The order closes in GBP. Local figures are payment-stage guidance.</p><div className="mt-4 grid gap-3 xl:grid-cols-3">{appliedCreditGbp > 0.01 ? <div className="rounded-2xl bg-white p-4 ring-1 ring-cyan-100"><p className="text-xs font-black uppercase text-cyan-700">Account credit applied</p><p className="mt-1 text-xl font-black">{money(appliedCreditGbp)}</p></div> : null}{visibleCashDueGbp > 0.01 ? <div className="rounded-2xl bg-white p-4 ring-1 ring-amber-100"><p className="text-xs font-black uppercase text-amber-700">Amount to pay</p><p className="mt-1 text-xl font-black">{money(visibleCashDueGbp)}</p><p className="mt-1 text-xs font-semibold text-slate-500">{effectiveRate ? localAmount(visibleCashDueLocal, currencyCode) : "Local guide unavailable"}</p></div> : null}{availableCreditGbp > 0.01 ? <div className="rounded-2xl bg-white p-4 ring-1 ring-cyan-100"><p className="text-xs font-black uppercase text-cyan-700">Available account credit</p><p className="mt-1 text-xl font-black">{money(availableCreditGbp)}</p><p className="mt-1 text-xs font-semibold text-slate-500">{effectiveRate ? localAmount(availableCreditLocal, currencyCode) : "Local guide unavailable"}</p></div> : null}</div></details> : null}
