...
export async function createCustomerOrderAction(formData: FormData) {
  ...
  if (!importer?.shipper_id) redirect("/customer/orders/new?error=Customer+record+missing+shipper.");
  if (!importer?.country_id) redirect("/customer/orders/new?error=Customer+record+missing+country.");
  ...
  if (!retailerId) redirect("/customer/orders/new?error=Retailer+is+required.");
  if (!confirmed) redirect("/customer/orders/new?error=Product+confirmation+is+required.");
  if (screenshots.length < 1) redirect("/customer/orders/new?error=At+least+one+screenshot+is+required.");
  if (!destinationHubId) redirect("/customer/orders/new?error=Destination+hub+is+required.");
  if (!Number.isInteger(totalQty) || totalQty <= 0) redirect("/customer/orders/new?error=Total+qty+declared+must+be+a+positive+integer.");
  if (!Number.isFinite(totalAmount) || totalAmount <= 0) redirect("/customer/orders/new?error=Order+total+GBP+declared+must+be+greater+than+0.");
  ...
  const { data: fxRate } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (!fxRate?.quote_rate) redirect("/customer/orders/new?error=No+dated+FX+rate+configured+for+customer+country.");
  ...
  const { data: validRetailer } = await supabase
    ...
    .eq("enabled", true)
    .maybeSingle();
  if (!validRetailer?.retailer_id) redirect("/customer/orders/new?error=Retailer+is+not+enabled+for+your+shipper.");
  ...
  const { data: validHub } = await supabase
    ...
    .eq("active", true)
    .maybeSingle();
  if (!validHub?.id) redirect("/customer/orders/new?error=Destination+hub+is+not+valid+for+your+shipper.");
  ...
  if (orderError || !createdOrder?.id) redirect(`/customer/orders/new?error=${encodeURIComponent(orderError?.message ?? "Failed to create order")}`);
  ...
  for (let i = 0; i < screenshots.length; i += 1) {
    ...
    if (uploadError) redirect(`/customer/orders/new?error=${encodeURIComponent(`Screenshot upload failed. ${uploadError.message}`)}`);
