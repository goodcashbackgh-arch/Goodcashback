...
function statusLabel(status: string | null | undefined) {
  ...
  if (status === "ready_for_sage_ap_readiness_review") return "Ready for Sage/AP readiness review";
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-950 sm:px-6 sm:py-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid gap-4 md:grid-cols-5">
          ...
          <div className={`rounded-3xl border p-4 shadow-sm ${podPending > 0 ? "border-amber-200 bg-amber-50" : "border-slate-200 bg-white"}`}><p className="text-xs uppercase tracking-wide text-slate-500">POD pending</p><p className="mt-1 text-2xl font-semibold">{podPending}</p><p className="mt-1 text-xs text-slate-500">AP ready: {apReady}</p></div>
          <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm"><p className="text-xs uppercase tracking-wide text-slate-500">Complete</p><p className="mt-1 text-2xl font-semibold">{completed}</p><p className="mt-1 text-xs text-slate-500">Missing shipper invoice: {missingShipperInvoice}</p></div>
        </section>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div><h2 className="text-xl font-semibold">Shipment batch worklist</h2><p className="mt-2 text-sm leading-6 text-slate-600">Summary-first control view. Use child queues for repeated actions; use row buttons for drill-down review.</p></div>
            <form action="/internal/shipping-control" className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:grid-cols-[1fr_1fr_auto]">
              <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">Status<select name="status" defaultValue={selectedStatus} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-950"><option value="all">All</option><option value="allocation_missing">Allocation missing</option><option value="receipt_issue">Receipt issue</option><option value="missing_shipper_invoice">Missing shipper invoice</option><option value="apportionment_pending">Apportionment pending</option><option value="sage_ap_ready">AP/Sage approved</option><option value="pod_pending">POD pending</option><option value="complete">Complete</option><option value="voided">Voided</option></select></label>
          ...
          <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-slate-500">Showing {rows.length} row(s) · AP/Sage approved: {apReady} · POD pending: {podPending} · Complete: {completed}</p>
