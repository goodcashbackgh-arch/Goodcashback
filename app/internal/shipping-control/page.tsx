      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          ...
          <Link href="/internal/shipping-control/shipper-documents" className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm hover:bg-slate-50">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">Accept, reject or request resubmission for shipper charge/receipt documents.</p>
            ...
            <div className="mt-4 grid gap-2 text-sm text-slate-950 sm:grid-cols-3">
              ...
              <div className="rounded-2xl bg-slate-50 p-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Document processing</p><p className="mt-1 font-bold">{shipperDocumentProcessingRows.length}</p></div>
          ...
          <Link href={draftCosCandidateRows[0]?.shipment_batch_id ? `/internal/export-evidence/draft/${draftCosCandidateRows[0].shipment_batch_id}` : "/internal/shipping-control?status=allocation_missing"} className="rounded-3xl border border-indigo-200 bg-indigo-50 p-5 shadow-sm hover:bg-indigo-100">
            ...
            <h2 className="mt-2 text-xl font-semibold text-indigo-950">Draft shipment certificate / export pack</h2>
            <p className="mt-2 text-sm leading-6 text-indigo-900">Review draft shipment-certificate basis, export/packing-list rows and final evidence blockers before shipper completion.</p>
            <p className="mt-3 text-2xl font-semibold text-indigo-950">{draftCosCandidateRows.length}</p>
            <p className="text-xs font-semibold uppercase tracking-wide text-indigo-700">content-matched candidate batches</p>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <form action="/internal/shipping-control" className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:grid-cols-[1fr_1fr_auto]">
              ...
              <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">Search<input name="q" defaultValue={qp.q ?? ""} placeholder="Booking, importer, shipper, tracking, groupage" className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-normal text-slate-950" /></label>
