    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm leading-5 text-slate-600">Read-only history of batch locks. No posting action is exposed here.</p>
        </div>
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {rows.length === 0 ? (
              ...
              <tr key={text(row.batch_id)} className="hover:bg-slate-50">
                <td className="px-2 py-2"><p className="truncate font-mono text-[11px] font-bold text-slate-950">{text(row.batch_ref)}</p><p className="truncate text-[10px] text-slate-500">{pretty(row.batch_kind)}</p></td>
                ...
                <td className="px-2 py-2"><Link href={text(row.detail_href)} className="rounded-lg border border-slate-300 bg-white px-2 py-1 text-[11px] font-bold text-slate-800 hover:bg-slate-50">Open</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
