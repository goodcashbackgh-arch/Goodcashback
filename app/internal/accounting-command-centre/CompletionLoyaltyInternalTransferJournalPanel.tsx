...
function badgeTone(status: string) {
  ...
  if (["partially_posted_needs_review", "warning_only", "already_materialised", "already_batched"].includes(status)) return "border-amber-200 bg-amber-50 text-amber-700";
...
function batchReadyToApprove(batch: Row) { return text(batch.status) === "validated" && text(batch.approval_status) !== "approved" && num(batch.row_count) > 0; }
function batchReadyToPost(batch: Row) { return text(batch.approval_status) === "approved" && ["approved", "failed_retryable", "partially_posted_needs_review"].includes(text(batch.status)) && num(batch.row_count) > 0; }
function batchMatchesStatus(batch: Row, statusFilter: string) {
  ...
  if (statusFilter === "blocked") return ["blocked", "failed_terminal"].includes(text(batch.status)) || num(batch.failed_count) > 0;
  return false;
}
          ...
          {showSourceSections ? (
            <div className="mt-5 grid gap-4 xl:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="mt-4 space-y-2">{candidates.length === 0 ? <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-500">No paired released internal-transfer candidates match the current filters.</div> : candidates.map((row) => {
