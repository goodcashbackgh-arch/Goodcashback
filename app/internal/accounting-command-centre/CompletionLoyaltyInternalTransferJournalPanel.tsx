...
function walletLabel(value: unknown) { const raw = text(value); if (raw === "virtual_gbp_wallet") return "Virtual GBP wallet"; if (raw === "dva_ghs_wallet") return "DVA GHS wallet"; if (raw === "main_gbp_bank") return "Main GBP bank"; return pretty(raw); }
...
export default async function CompletionLoyaltyInternalTransferJournalPanel({ searchQuery = "" }: Props) {
  ...
  const [{ data: candidateData, error: candidateError }, { data: groupData, error: groupError }, { data: batchData, error: batchError }] = await Promise.all([
  ...
  return (
    <section id="step-3-internal-transfer" className="rounded-3xl border border-cyan-200 bg-white p-5 shadow-sm scroll-mt-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><p className="text-sm font-medium uppercase tracking-[0.18em] text-cyan-700">Step 3 · Internal transfer journal lane</p><h2 className="mt-2 text-xl font-semibold text-slate-950">Completion-loyalty bank movement</h2><p className="mt-2 max-w-5xl text-sm leading-6 text-slate-600">Paired released main-bank OUT and DVA/card IN rows become local direct journal groups first. Validated groups are batched, approved, then posted as Sage journals.</p></div>
        <div className="rounded-2xl bg-cyan-50 px-4 py-3 text-sm font-semibold text-cyan-950 ring-1 ring-cyan-200">{candidates.length} candidate(s) · {gbp(totalCandidateAmount)}<br />{groups.length} group(s) · {gbp(totalGroupAmount)}<br />{batchReadyGroups.length} ready · {gbp(batchReadyAmount)}</div>
      </div>

      {[candidateError, groupError, batchError].filter(Boolean).map((error, index) => <div key={index} className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-900">{error.message}</div>)}

      {!hasRows && !hasRpcError ? <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-600">No paired released internal-transfer candidates, materialised transfer groups, or transfer batches match the current filters.</div> : (
        <>
          <div className="mt-5 grid gap-4 xl:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-bold text-slate-950">Paired transfer candidates</h3><p className="mt-1 text-xs leading-5 text-slate-500">Bulk or single materialise/freeze creates local journal groups only. No Sage API call.</p></div><div className="rounded-xl border border-cyan-200 bg-cyan-50 px-3 py-2 text-right text-xs font-bold text-cyan-900">{materialisableCandidates.length} selectable<br />{gbp(totalMaterialisableAmount)}</div></div>
              {materialisableCandidates.length > 0 ? <form id="bulk-internal-transfer-materialise-form" action={materialiseSelectedInternalTransferJournalsAction} className="mt-3 rounded-2xl border border-cyan-200 bg-white p-3"><div className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-end"><label className="grid gap-1 text-xs font-bold uppercase tracking-wide text-slate-500">Materialisation note<input name="notes" placeholder="Optional bulk materialisation note" className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-normal normal-case tracking-normal text-slate-950" /></label><button type="submit" className="rounded-xl bg-cyan-700 px-4 py-2 text-xs font-bold text-white hover:bg-cyan-800">Materialise / freeze selected</button></div><div className="mt-3"><SelectionControls /></div></form> : null}
              <div className="mt-4 space-y-2">
                {candidates.length === 0 ? <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-500">No paired released internal-transfer candidates match the current filters.</div> : candidates.map((row) => {
                  const blocker = text(row.blocker); const alreadyMaterialised = Boolean(text(row.existing_posting_group_id)); const canMaterialise = !blocker && !alreadyMaterialised && text(row.source_out_statement_line_id) && text(row.destination_in_statement_line_id);
                  return <div key={candidateKey(row)} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-start gap-3">{canMaterialise ? <input type="checkbox" name="internal_transfer_candidate_key" value={candidatePairValue(row)} form="bulk-internal-transfer-materialise-form" defaultChecked data-accounting-row-select="true" className="mt-1 h-4 w-4 rounded border-slate-300" aria-label={`Select ${text(row.importer_name) || "internal transfer"} for materialisation`} /> : <span className="mt-1 text-xs text-slate-300">-</span>}<div><p className="font-bold text-slate-950">{text(row.importer_name) || "Importer/customer"}</p><p className="mt-1 text-sm text-slate-500">Debit: {walletLabel(row.destination_wallet_code)} · Credit: Main GBP bank</p><p className="mt-2 text-xs text-slate-400">OUT {text(row.source_out_date) || "-"} · IN {text(row.destination_in_date) || "-"}</p></div></div><div className="text-right"><p className="text-lg font-extrabold text-slate-950">{gbp(row.transfer_amount_gbp)}</p><StatusBadge value={alreadyMaterialised ? "already_materialised" : row.materialisation_status} /></div></div><div className="mt-3 grid gap-2 text-xs text-slate-600 sm:grid-cols-3"><div className="rounded-xl bg-slate-50 p-2"><span className="font-bold">Released loyalty:</span><br />{gbp(row.loyalty_released_amount_gbp)}</div><div className="rounded-xl bg-slate-50 p-2"><span className="font-bold">Wallet excess:</span><br />{gbp(row.excess_remaining_gbp)}</div><div className="rounded-xl bg-slate-50 p-2"><span className="font-bold">Mappings:</span><br />{text(row.source_mapping_code) || "-"} / {text(row.destination_mapping_code) || "-"}</div></div><p className="mt-2 break-all text-xs text-slate-500">OUT ref: {text(row.source_out_reference) || "-"}</p><p className="mt-1 break-all text-xs text-slate-500">IN ref: {text(row.destination_in_reference) || "-"}</p>{blocker ? <p className="mt-3 rounded-2xl border border-rose-200 bg-rose-50 p-3 text-xs font-semibold text-rose-700">Blocker: {pretty(blocker)}</p> : null}{alreadyMaterialised ? <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-900">Existing group: {text(row.existing_posting_group_ref)}</p> : null}<form action={materialiseInternalTransferJournalAction} className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]"><input type="hidden" name="source_out_statement_line_id" value={text(row.source_out_statement_line_id)} /><input type="hidden" name="destination_in_statement_line_id" value={text(row.destination_in_statement_line_id)} /><input name="notes" placeholder="Optional materialisation note" className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-950" /><button type="submit" disabled={!canMaterialise} className="rounded-2xl bg-cyan-700 px-4 py-2 text-sm font-bold text-white hover:bg-cyan-800 disabled:bg-slate-200 disabled:text-slate-500">Materialise / freeze</button></form></div>;
                })}
              </div>
            </div>

            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-bold text-slate-950">Internal-transfer journal groups</h3><p className="mt-1 text-xs leading-5 text-slate-500">Select locally validated internal-transfer groups and create a loyalty Sage batch.</p></div><div className="rounded-xl border border-cyan-200 bg-cyan-50 px-3 py-2 text-right text-xs font-bold text-cyan-900">{batchReadyGroups.length} ready<br />{gbp(batchReadyAmount)}</div></div>
