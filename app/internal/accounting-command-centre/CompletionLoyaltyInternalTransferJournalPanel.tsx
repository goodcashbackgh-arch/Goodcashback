...
function groupKey(row: Row) {
  return text(row.posting_group_id) || text(row.posting_group_ref) || `${text(row.source_out_statement_line_id)}-${text(row.destination_in_statement_line_id)}`;
}

function candidateKey(row: Row) {
  return `${text(row.source_out_statement_line_id)}-${text(row.destination_in_statement_line_id)}-${text(row.importer_id)}`;
}
...
function contextValue(group: Row, key: string) {
  return asObject(group.request_context_json)[key];
}
    ...
    <section id="step-3-internal-transfer" className="rounded-3xl border border-cyan-200 bg-white p-5 shadow-sm scroll-mt-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 max-w-5xl text-sm leading-6 text-slate-600">
            Same workbench, separate lane. Paired released main-bank OUT and DVA/card IN rows become direct journal groups: debit the resolved DVA GHS or Virtual GBP wallet ledger, credit the existing mapped main bank ledger. This does not change applied-loyalty customer settlement.
          </p>
        </div>
      ...
      {candidateError ? (
        <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-900">
          Internal-transfer candidate RPC unavailable: {candidateError.message}. Run the internal-transfer journal migration before using this section.
        </div>
      ) : null}
      {groupError ? (
        <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-900">
          Posting lifecycle RPC unavailable: {groupError.message}.
        </div>
      ) : null}
      {batchError ? (
        <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-900">
          Loyalty Sage batch RPC unavailable: {batchError.message}.
        </div>
      ) : null}
