...
function short(value: unknown, max = 40) {
  const raw = text(value);
...
function Chip({ value }: { value: unknown }) {
  return <span className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-bold ${statusClass(value)}`}>{pretty(value)}</span>;
}
...
function movement(row: Row) {
  ...
  const from = short(credit.details || "Main GBP bank", 32).replace("Completion loyalty transfer from ", "");
  const to = short(debit.details || "destination wallet", 32).replace("Completion loyalty transfer to ", "");
  return `${from} → ${to}`;
}
...
function SourceCell({ row, internal }: { row: Row; internal: boolean }) {
  return (
    <div className="space-y-1 text-[11px]">
      <p className="font-bold text-slate-900">{internal ? movement(row) : `Order ${text(row.order_ref) || "—"}`}</p>
      <p className="break-all text-slate-600">Group: {short(row.posting_group_ref, 38)}</p>
    </div>
  );
}
...
function TargetCell({ internal }: { internal: boolean }) {
  return (
    <div className="space-y-1 text-[11px]">
      <p className="font-semibold text-slate-900">{internal ? "Internal bank movement" : "Customer settlement"}</p>
      <p className="text-slate-600">{internal ? "No customer invoice allocation." : "Uses frozen allocation target."}</p>
    </div>
  );
}
...
function StepsCell({ row }: { row: Row }) {
  ...
  return (
    <div className="space-y-1">
      {steps.length === 0 ? <span className="text-[11px] text-rose-700">No steps</span> : steps.map((step, index) => (
        <p key={`${text(row.item_id)}-${index}`} className="flex flex-wrap items-center gap-1 text-[11px]">
          <span className="font-bold text-slate-900">{pretty(step.step_type)}</span>
...
function Payloads({ rows }: { rows: Row[] }) {
  return (
    <div className="mt-4 grid gap-3 lg:grid-cols-2">
      {rows.flatMap((row, rowIndex) => orderedSteps(row).flatMap((step, stepIndex) => [
        <details key={`${rowIndex}-${stepIndex}-request`} className="rounded-2xl border border-slate-200 bg-slate-50 p-3 text-xs">
          <summary className="cursor-pointer font-bold text-slate-900">{pretty(step.step_type)} · request payload</summary>
          <pre className="mt-2 max-h-72 overflow-auto rounded-xl bg-white p-3 text-[11px] leading-5 text-slate-700">{JSON.stringify(step.request_payload ?? {}, null, 2)}</pre>
        </details>,
        <details key={`${rowIndex}-${stepIndex}-response`} className="rounded-2xl border border-slate-200 bg-slate-50 p-3 text-xs">
          <summary className="cursor-pointer font-bold text-slate-900">{pretty(step.step_type)} · Sage response</summary>
          <pre className="mt-2 max-h-72 overflow-auto rounded-xl bg-white p-3 text-[11px] leading-5 text-slate-700">{JSON.stringify(step.response_payload ?? {}, null, 2)}</pre>
        </details>,
      ]))}
    </div>
  );
      ...
      <div className="mx-auto max-w-[1900px] space-y-3">
        <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 max-w-5xl text-sm leading-5 text-slate-600">Local batch lock plus approval. Retry continues the same batch and skips posted steps.</p>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <form action={approveCompletionLoyaltySageBatchAction}>
                  ...
                  <button type="submit" disabled={!canApprove} className="rounded-2xl bg-violet-700 px-4 py-2 text-sm font-bold text-white shadow-sm disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-600">Approve batch</button>
                </form>
                <form action={postCompletionLoyaltySageBatchAction}>
                  <input type="hidden" name="batch_id" value={batchId} />
                  <button type="submit" disabled={!canPost} className="rounded-2xl bg-emerald-700 px-4 py-2 text-sm font-bold text-white shadow-sm disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-600">{postButtonLabel}</button>
                </form>
                <Link href={`/internal/accounting-command-centre/loyalty-controls/batches/${batchId}/retire`} aria-disabled={!canRetire} className={`rounded-2xl px-4 py-2 text-sm font-bold shadow-sm ${canRetire ? "border border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100" : "pointer-events-none bg-slate-200 text-slate-500"}`}>Retire local batch</Link>
                <span className={`rounded-2xl border px-3 py-2 text-xs font-bold ${livePostingEnabled ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-amber-200 bg-amber-50 text-amber-900"}`}>Live Sage posting {livePostingEnabled ? "enabled" : "disabled"}.</span>
              </div>
            </div>
            <div className="flex shrink-0 flex-wrap gap-1.5 xl:max-w-[900px] xl:justify-end">
              <Stat label="Status" value={pretty(batchStatus)} />
              <Stat label="Lane" value={pretty(laneLabel)} />
              <Stat label="Rows" value={String(first.batch_row_count ?? rows.length)} />
              <Stat label="Total" value={gbp(first.batch_total_amount_gbp)} />
              <Stat label="Approved" value={approved ? "yes" : "no"} />
          ...
          {notice ? <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-900">{notice}</p> : null}
          {error ? <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-900">Batch detail RPC unavailable: {error.message}</p> : null}
        </section>
          ...
          <div className="overflow-x-auto rounded-b-3xl">
            <table className="min-w-[1320px] table-fixed divide-y divide-slate-200 text-xs">
              <thead className="sticky top-0 z-10 bg-slate-100 text-[11px] uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="w-[112px] px-2 py-2 text-left">Status</th>
                  <th className="w-[150px] px-2 py-2 text-left">Lane</th>
                  <th className="w-[260px] px-2 py-2 text-left">Source facts</th>
                  <th className="w-[260px] px-2 py-2 text-left">Sage target / control</th>
                  <th className="w-[96px] px-2 py-2 text-right">Amount</th>
                  <th className="w-[180px] px-2 py-2 text-left">Validation</th>
                  <th className="w-[180px] px-2 py-2 text-left">Steps</th>
                  <th className="w-[250px] px-2 py-2 text-left">Reason / error</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 bg-white">
                {rows.length === 0 ? <tr><td colSpan={8} className="px-3 py-8 text-center text-sm text-slate-500">No rows.</td></tr> : rows.map((row) => (
                  <tr key={text(row.item_id) || text(row.posting_group_ref)} className="align-top hover:bg-slate-50">
                    ...
                    <td className="px-2 py-2"><SourceCell row={row} internal={internal} /></td>
                    <td className="px-2 py-2"><TargetCell internal={internal} /></td>
                    ...
                    <td className="px-2 py-2"><Chip value={row.group_validation_status} /></td>
                    <td className="px-2 py-2"><StepsCell row={row} /></td>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          <details>
            ...
            <p className="mt-1 text-sm text-slate-500">Frozen request payloads and Sage responses are kept here for audit/retry checks.</p>
            <Payloads rows={rows} />
          </details>
        </section>
      </div>
