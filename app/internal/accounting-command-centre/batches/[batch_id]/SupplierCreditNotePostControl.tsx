...
function batchIdFromParams(value: string | string[] | undefined) {
  if (Array.isArray(value)) return value[0] ?? "";
  return value ?? "";
}

export default function SupplierCreditNotePostControl() {
  const params = useParams<{ batch_id?: string | string[] }>();
  const batchId = batchIdFromParams(params?.batch_id);
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm leading-5 text-emerald-900">
              Uses the purchase credit note endpoint. The server-side action refuses the wrong lane, missing dry-run validation, disabled live flag, or already-posted rows.
            </p>
          </div>
          <form action={postSupplierCreditNoteBatchToSageAction} className="shrink-0">
            ...
            <button
              type="submit"
              className="rounded-2xl bg-emerald-700 px-5 py-3 text-sm font-extrabold text-white shadow-sm hover:bg-emerald-800"
            >
