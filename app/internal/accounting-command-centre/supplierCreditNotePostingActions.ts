...
import { postSupplierCreditNoteBatchToSage } from "@/lib/sage/supplierCreditNotePosting";
...
export async function postSupplierCreditNoteBatchToSageAction(formData: FormData) {
  ...
  try {
    const result = await postSupplierCreditNoteBatchToSage({
      batchId,
      staffId,
      origin: appOrigin(),
    });
    ...
    if (result.failed > 0) {
      ...
      redirectTo = `/internal/accounting-command-centre/batches/${batchId}?success=${encodeURIComponent(`Supplier credit note Sage posting finished: ${result.posted} posted, ${result.failed} failed, ${result.total} total. Endpoint /purchase_credit_notes.`)}`;
    }
  } catch (error) {
