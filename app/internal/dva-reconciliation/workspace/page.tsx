...
function lineTone(row: Row, selectedLineId: string) {
  if (text(row.dva_statement_line_id) === selectedLineId) return "border-sky-400 bg-sky-50 ring-2 ring-sky-200";
...
function opTone(row: Row, selectedTargetId: string) {
  const id = text(row.id) || text(row.invoice_id) || text(row.dispute_id);
  return id === selectedTargetId ? "border-sky-400 bg-sky-50 ring-2 ring-sky-200" : "border-slate-200 bg-white";
}
...
export default async function DvaMatchingWorkspacePage({
  ...
  const rightStatus = firstParam(params.right_status) || "open";
  ...
  const statementLines = ((statementLinesResult.data ?? []) as unknown as Row[]).filter((row) => !selectedImporterId || text(row.importer_id) === selectedImporterId);
  ...
  const exceptionRows = disputes.map((dispute) => {
  ...
  const operationalRows = [...invoiceRows, ...exceptionRows].filter((row) => {
    const retailerOk = !rightRetailer || row.retailerName.toLowerCase().includes(rightRetailer.toLowerCase()) || row.title.toLowerCase().includes(rightRetailer.toLowerCase());
    const statusOk = rightStatus === "all" || (rightStatus === "open" ? row.status !== "resolved" && row.status !== "closed" : row.status === rightStatus);
  ...
  const selectedTargetAmount = selectedTarget?.amount ?? 0;
  const suggestedAllocation = Math.min(Math.max(0, selectedStatementAmount - selectedStatementAllocated), selectedTargetAmount || Math.max(0, selectedStatementAmount - selectedStatementAllocated));
  const remainingAfterSelection = Math.max(0, selectedStatementAmount - selectedStatementAllocated - suggestedAllocation);
      ...
      <div className="mx-auto max-w-7xl space-y-6 pb-36">
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-3 max-w-5xl text-sm leading-6 text-slate-600">
            Read-only workspace for fast supervisor review. Pick a statement line on the left, filter operational truth on the right, then confirm allocations in the next build after the selection logic is proven.
          </p>
        </section>

        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {metric("Importer", selectedImporter ? (text(selectedImporter.trading_name) || text(selectedImporter.company_name) || "Selected") : "All / unfiltered", "Filter by importer before serious matching.")}
            ...
            <div className="mt-4 space-y-3">
              ...
              {operationalRows.map((row) => (
                <Link
                  ...
                  className={`block rounded-2xl border p-4 ${opTone(row as unknown as Row, selectedTargetId)}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
