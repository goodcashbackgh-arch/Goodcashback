...
export default async function DvaAccountingReviewPackPage({
  ...
  const [statementResult, importersResult, allocationResult, retailersResult] = await Promise.all([
    statementQuery,
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
            <div className="min-w-0">
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600 sm:text-base">
                Read-only pack showing statement lines with supplier invoice, refund, FX/card, fee and exception allocations before accounting handoff. No posting and no financial state changes happen here.
              </p>
            </div>
                  ...
                  <div className="grid gap-4 lg:grid-cols-[1.25fr_1fr]">
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-5">
                        <div className="rounded-xl bg-slate-50 p-3 ring-1 ring-slate-200"><p className="text-[11px] font-bold uppercase tracking-wide text-slate-500">Supplier</p><p className="mt-1 font-extrabold text-slate-950">{gbp(state.supplier)}</p></div>
