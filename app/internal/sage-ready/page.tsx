...
function statusClass(status: string | null | undefined) {
  ...
  if (status.startsWith("posted")) return "bg-slate-200 text-slate-800";
...
function matchesFilter(row: Row, lane: string, status: string) {
  ...
  const statusOk =
    status === "all" ||
    (status === "blocked" && row.readiness_status?.startsWith("blocked")) ||
    (status === "ready" && row.readiness_status?.startsWith("ready")) ||
    (status === "posted" && row.readiness_status?.startsWith("posted")) ||
    row.sage_status === status;
...
export default async function ReadyForSagePage({ searchParams }: { searchParams?: Promise<{ lane?: string; status?: string }> }) {
  ...
  const { data, error } = await (supabase as any).rpc("internal_ready_for_sage_queue_v1");
  const allRows = (data ?? []) as Row[];
  const rows = allRows.filter((row) => matchesFilter(row, lane, status));
  const customerRows = allRows.filter((row) => row.document_lane === "customer_sales");
  const apRows = allRows.filter((row) => row.document_lane === "shipper_ap");
  const blockedRows = allRows.filter((row) => row.readiness_status?.startsWith("blocked"));
  const readyRows = allRows.filter((row) => row.readiness_status?.startsWith("ready"));
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-950 sm:px-6 sm:py-8">
      <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="mt-2 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-600">
                One accounting queue for internal documents that are ready or nearly ready for Sage. This page is read-only for now: it does not test Sage endpoints, post to Sage, clear VAT, or mark anything posted.
              </p>
            </div>
        ...
        <section className="grid gap-4 md:grid-cols-4">
          <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs uppercase tracking-wide text-slate-500">Queue rows</p><p className="mt-1 text-2xl font-semibold">{allRows.length}</p></div>
            ...
            {rows.map((row) => (
              <article key={row.queue_row_id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="rounded-xl bg-white p-3"><p className="text-xs uppercase tracking-wide text-slate-500">Sage status</p><p className="mt-1 font-semibold">{friendly(row.sage_status)}</p></div>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {rows.map((row) => (
                  <tr key={row.queue_row_id}>
                    ...
                    <td className="px-3 py-3 align-top">{friendly(row.sage_status)}</td>
                    <td className="px-3 py-3 align-top"><span className={`rounded-full px-2 py-1 text-xs font-semibold ${statusClass(row.readiness_status)}`}>{friendly(row.readiness_status)}</span>{row.blocker ? <p className="mt-1 text-xs text-rose-700">{friendly(row.blocker)}</p> : null}</td>
