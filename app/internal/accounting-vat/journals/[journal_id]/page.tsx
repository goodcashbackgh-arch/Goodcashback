...
export default async function VatJournalDetailPage({ params, searchParams }: any) {
  ...
  const canDryRun = status === "platform_calculated" || status === "dry_run_failed";
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <form action={dryRunVatAdjustmentJournalAction}>
              ...
              <button disabled={!canDryRun} className={`rounded-xl px-4 py-2 text-sm font-bold ${canDryRun ? "border border-sky-200 bg-sky-50 text-sky-800 hover:bg-sky-100" : "cursor-not-allowed border border-slate-200 bg-slate-100 text-slate-400"}`}>Dry-run validate journal</button>
            </form>
            <form action={approveVatAdjustmentJournalAction}>
              <input type="hidden" name="journal_id" value={journalId} />
              <button disabled={!canApprove} className={`rounded-xl px-4 py-2 text-sm font-bold ${canApprove ? "border border-emerald-200 bg-emerald-50 text-emerald-800 hover:bg-emerald-100" : "cursor-not-allowed border border-slate-200 bg-slate-100 text-slate-400"}`}>Admin approve journal</button>
            </form>
          </div>
        </section>
