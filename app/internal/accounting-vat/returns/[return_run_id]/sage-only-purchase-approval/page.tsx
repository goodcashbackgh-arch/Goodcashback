...
function directSageDisplayLabel(row: Row): string { const existing = text(row.document_label); if (existing && !isGenericDocumentLabel(existing)) return existing; const parts = [text(row.supplier_contact) || "Sage purchase document"]; const rawDate = text(row.document_date).match(/^\d{4}-\d{2}-\d{2}/)?.[0] ?? ""; if (rawDate) parts.push(rawDate); const gross = num(row.gross_amount); if (Math.abs(gross) > 0.005) parts.push(`£${Math.abs(gross).toFixed(2)}`); return parts.join(" — "); }
