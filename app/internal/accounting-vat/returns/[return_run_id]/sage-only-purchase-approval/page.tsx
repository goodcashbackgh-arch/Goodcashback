...
export default async function SageOnlyPurchaseApprovalPage({ params, searchParams }: any) {
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-8 text-slate-950"><div className="mx-auto flex max-w-5xl flex-col gap-6">
    ...
    {docs.length ? <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Sage document evidence</h2><p className="mt-1 text-sm text-slate-600">Read-only documents pulled from Sage API for this period. Use the Sage document ID/reference to review the transaction in Sage.</p><div className="mt-4 overflow-x-auto"><table className="min-w-full divide-y divide-slate-200 text-sm"><thead className="text-left text-xs uppercase text-slate-500"><tr><th className="px-3 py-2">Document</th><th className="px-3 py-2">Sage ID</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Net</th><th className="px-3 py-2">VAT</th><th className="px-3 py-2">Total</th></tr></thead><tbody className="divide-y divide-slate-100">{docs.map((d) => <tr key={text(d.id) || text(d.document_number)}><td className="px-3 py-2 font-semibold">{label(d.document_number || d.displayed_as)}</td><td className="px-3 py-2">{label(d.id)}</td><td className="px-3 py-2">{label(d.status_displayed_as || d.status)}</td><td className="px-3 py-2">{gbp(d.net)}</td><td className="px-3 py-2">{gbp(d.tax)}</td><td className="px-3 py-2">{gbp(d.total)}</td></tr>)}</tbody></table></div></section> : null}
