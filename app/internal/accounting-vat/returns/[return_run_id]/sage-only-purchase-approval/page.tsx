...
export default async function SageOnlyPurchaseApprovalPage({ params, searchParams }: any) {
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-8 text-slate-950"><div className="mx-auto flex max-w-6xl flex-col gap-6">
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><Link href={`/internal/accounting-vat/returns/${runId}?tab=purchases`} className="text-sm font-semibold text-sky-600">← Back to Box 4 / Box 7 purchases</Link><p className="mt-6 text-sm font-medium uppercase tracking-[0.2em] text-sky-500">Final direct Sage posting confirmation</p><h1 className="mt-3 text-3xl font-semibold tracking-tight">{label((run as Row).return_period_label || (run as Row).run_ref)}</h1><p className="mt-2 text-sm leading-6 text-slate-600">Confirmation-only approval for selected direct Sage postings not on platform. This creates source-linked platform VAT lines only; it does not create or post Sage adjustment journals.</p>{err ? <p className="mt-4 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-900">{err}</p> : null}</section>
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Selected direct Sage postings not on platform</h2><p className="mt-1 text-sm text-slate-600">Only the selected line-level postings are shown on this confirmation page.</p><div className="mt-4 overflow-x-auto"><table className="min-w-full divide-y divide-slate-200 text-left text-sm"><thead className="text-xs uppercase text-slate-500"><tr><th className="px-3 py-2">Index</th><th className="px-3 py-2">Document/ref</th><th className="px-3 py-2">Supplier/contact</th><th className="px-3 py-2">Document date</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Ledger</th><th className="px-3 py-2">Tax rate</th><th className="px-3 py-2">Description</th><th className="px-3 py-2">Net</th><th className="px-3 py-2">VAT</th><th className="px-3 py-2">Gross</th><th className="px-3 py-2">Box 4 effect</th><th className="px-3 py-2">Box 7 effect</th><th className="px-3 py-2">Reason</th><th className="px-3 py-2">Action/help text</th></tr></thead><tbody className="divide-y divide-slate-100">{selectedRows.map(({ index, row }) => <tr key={`${index}-${text(row.sage_document_id)}`}><td className="px-3 py-2 font-semibold">{index}</td><td className="px-3 py-2 font-semibold">{cut(row.document_label || row.sage_document_id, 36)}</td><td className="px-3 py-2">{cut(row.supplier_contact, 34)}</td><td className="px-3 py-2">{date(row.document_date)}</td><td className="px-3 py-2">{cut(row.document_status, 24)}</td><td className="px-3 py-2">{cut(row.ledger_account, 34)}</td><td className="px-3 py-2">{cut(row.tax_rate, 28)}</td><td className="px-3 py-2">{cut(row.line_description, 60)}</td><td className="px-3 py-2 font-semibold">{gbp(row.net_amount)}</td><td className="px-3 py-2 font-semibold">{gbp(row.vat_amount)}</td><td className="px-3 py-2 font-semibold">{gbp(row.gross_amount)}</td><td className="px-3 py-2 font-semibold">{gbp(row.effective_box4_amount)}</td><td className="px-3 py-2 font-semibold">{gbp(row.effective_box7_amount)}</td><td className="px-3 py-2">{cut(row.reason, 80)}</td><td className="px-3 py-2">Copy ref / open Sage manually</td></tr>)}</tbody></table></div></section>
