...
export default async function VatReturnPackDetailPage({ params, searchParams }: any) {
  ...
  const journalCols: Col[] = [{ label: "Open", render: (row) => <Link href={`/internal/accounting-vat/journals/${text(row.id)}`} className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-bold text-sky-700 hover:bg-sky-50">Open</Link> }, { label: "Type", render: (row) => pretty(row.adjustment_type) }, { label: "Box", render: (row) => boxLabel(row.target_box) }, { label: "Direction", render: (row) => pretty(row.direction) }, { label: "Amount", render: (row) => gbp(row.amount_gbp) }, { label: "Status", render: (row) => pretty(row.status) }, { label: "Sage ref", render: (row) => <Link href={`/internal/accounting-vat/journals/${text(row.id)}`} className="font-semibold text-sky-700 hover:underline">{cut(row.sage_journal_ref ?? row.sage_journal_id)}</Link> }];
  const journalLineCols: Col[] = [{ label: "Journal", render: (row) => <Link href={`/internal/accounting-vat/journals/${text(row.vat_return_adjustment_journal_id)}`} className="font-semibold text-sky-700 hover:underline">{cut(row.vat_return_adjustment_journal_id, 18)}</Link> }, { label: "No", render: (row) => cut(row.line_no) }, { label: "Role", render: (row) => pretty(row.line_role) }, { label: "Account", render: (row) => cut(row.sage_ledger_account_display ?? row.account_role, 40) }, { label: "Debit", render: (row) => gbp(row.debit_amount_gbp) }, { label: "Credit", render: (row) => gbp(row.credit_amount_gbp) }, { label: "Tax return", render: (row) => pretty(row.include_on_tax_return) }, { label: "Box", render: (row) => boxLabel(row.target_box) }];
