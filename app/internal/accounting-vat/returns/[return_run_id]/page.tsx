...
function VatBoxGrid({ run }: { run: Row }) {
  const b1 = num(run.expected_box1_gbp);
  const b2 = num(run.expected_box2_gbp);
  const b3 = b1 + b2;
  const b4 = num(run.expected_box4_gbp);
  const b5Net = b3 - b4;
  const b5Label = b5Net >= 0 ? "to pay" : "to reclaim";
  const boxes = [
    ["Box 1", b1, "VAT due on sales/outputs"],
    ["Box 2", b2, "VAT due on acquisitions"],
    ["Box 3", b3, "Total VAT due: Box 1 + Box 2"],
    ["Box 4", b4, "VAT reclaimed on purchases/inputs"],
    ["Box 5", Math.abs(b5Net), `Net VAT ${b5Label}`],
    ["Box 6", num(run.expected_box6_gbp), "Net sales/outputs"],
    ["Box 7", num(run.expected_box7_gbp), "Net purchases/inputs"],
    ["Box 8", num(run.expected_box8_gbp), "EU dispatches"],
    ["Box 9", num(run.expected_box9_gbp), "EU acquisitions"],
  ];
  return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-semibold tracking-tight">VAT return draft</h2><p className="mt-1 text-sm leading-6 text-slate-600">Draft boxes for review before any Sage/MTD submission. Boxes 3 and 5 are calculated from the draft values.</p></div><span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-bold text-slate-600">Boxes 1–9</span></div><div className="mt-5 grid gap-3 md:grid-cols-3 xl:grid-cols-5">{boxes.map(([label, value, note]) => <div key={String(label)} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">{label}</p><p className="mt-1 text-2xl font-extrabold text-slate-950">{gbp(value)}</p><p className="mt-2 text-xs leading-5 text-slate-600">{note}</p></div>)}</div></section>;
}
...
export default async function VatReturnPackDetailPage({ params, searchParams }: any) {
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-8 text-slate-950"><div className="mx-auto flex max-w-7xl flex-col gap-6">
    ...
    {activeTab === "purchases" ? <div className="grid gap-4"><section className="rounded-3xl border border-slate-200 bg-white p-5 text-sm leading-6 text-slate-700"><h2 className="font-semibold text-slate-950">Box 4 / Box 7 Purchases</h2><p className="mt-2">Purchase VAT and purchase-net values should appear here when supplier/AP source rules are captured.</p></section><Table title="Box 4 / Box 7 source lines" data={{ ...lines, rows: purchaseRows }} columns={sourceCols} empty="No Box 4 or Box 7 source lines captured yet." /><Table title="Supplier invoice evidence" data={purchaseInvoices} columns={purchaseCols} /></div> : null}
