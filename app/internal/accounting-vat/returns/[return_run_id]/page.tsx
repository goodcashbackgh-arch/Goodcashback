...
function cut(value: unknown, max = 42): string {
  const raw = s(value).trim();
  if (!raw) return "—";
...
function Workflow() {
  ...
  return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold tracking-tight">Contract workflow</h2><p className="mt-2 text-sm leading-6 text-slate-600">This return pack follows the saved repo contract. Posting, submission and lock remain unavailable until blockers are clear and the required adjustment pack is approved.</p><div className="mt-5 grid gap-3 md:grid-cols-3 xl:grid-cols-6">{steps.map((step, index) => <div key={step} className={`rounded-2xl border p-4 ${index < 2 ? tone("info") : tone("muted")}`}><div className="flex items-center gap-2"><span className="inline-flex h-8 min-w-8 items-center justify-center rounded-full bg-white px-2 text-xs font-extrabold ring-1 ring-slate-200">{index + 1}</span><h3 className="font-semibold">{step}</h3></div><p className="mt-2 text-xs leading-5 opacity-90">{index < 2 ? "Current read-only build." : "Locked until prior gates tie."}</p></div>)}</div></section>;
}
...
export default async function VatReturnPackDetailPage({ params, searchParams }: any) {
  ...
  const { data: runData, error: runError } = await db
    ...
    .select("id, run_ref, return_period_label, period_start_date, period_end_date, status, expected_box1_gbp, expected_box2_gbp, expected_box3_gbp, expected_box4_gbp, expected_box5_gbp, expected_box6_gbp, expected_box7_gbp, expected_box8_gbp, expected_box9_gbp, locked_at, created_at")
    .eq("id", runId)
    .maybeSingle();
  const run = (runData ?? {}) as Row;
  if (!runData && !runError) redirect("/internal/accounting-vat");

  const periodStart = s(run.period_start_date).slice(0, 10);
  const periodEnd = s(run.period_end_date).slice(0, 10);

  const [lines, blockers, journals, journalLines, recon, salesInvoices, purchaseInvoices, supplierCredits, matchEvidence] = await Promise.all([
    listRows(db, "vat_return_run_lines", "id, line_type, source_table, source_id, source_ref, target_box, direction, amount_gbp, vat_amount_gbp, tax_point_date, reporting_period, sage_natural_coverage_status, adjustment_required, created_at", (x) => x.eq("vat_return_run_id", runId).order("created_at", { ascending: false })),
    listRows(db, "vat_return_blockers", "id, blocker_code, severity, status, source_table, source_ref, message, required_action, created_at", (x) => x.eq("vat_return_run_id", runId).order("created_at", { ascending: false })),
    listRows(db, "vat_return_adjustment_journals", "id, adjustment_type, target_box, direction, amount_gbp, status, sage_journal_ref, posted_at, created_at", (x) => x.eq("vat_return_run_id", runId).order("created_at", { ascending: false })),
    listRows(db, "vat_return_adjustment_journal_lines", "id, adjustment_journal_id, account_name, ledger_account_id, debit_gbp, credit_gbp, include_on_tax_return, target_box, created_at", (x) => x.order("created_at", { ascending: false })),
    ...
    listRows(db, "sales_invoices", "id, invoice_type, amount_gbp, sage_status, consideration_received_date, sage_invoice_date, vat_box6_reported_period, zero_rating_deadline_date, zero_rating_status, export_evidence_complete_date, sage_invoice_id, created_at", (x) => x.order("created_at", { ascending: false })),
    ...
    listRows(db, "supplier_credit_notes", "id, supplier_credit_note_ref, status, credit_note_date, total_gbp, net_gbp, vat_gbp, sage_posting_status, sage_credit_note_id, created_at", (x) => x.order("created_at", { ascending: false })),
    listRows(db, "vat_return_sage_match_evidence", "id, vat_return_run_id, sage_return_ref, submitted_box1_gbp, submitted_box4_gbp, submitted_box6_gbp, submitted_box7_gbp, matched_at, created_at", (x) => x.eq("vat_return_run_id", runId).order("created_at", { ascending: false })),
  ]);

  const sourceCols: Col[] = [
    { label: "Type", render: (row) => pretty(row.line_type) },
    ...
    { label: "Sage coverage", render: (row) => pretty(row.sage_natural_coverage_status) },
    { label: "Adjustment", render: (row) => pretty(row.adjustment_required) },
  ];
  ...
  const invoiceCols: Col[] = [
    ...
    { label: "Box6 period", render: (row) => cut(row.vat_box6_reported_period) },
  ];
  const evidenceCols: Col[] = [
    { label: "Invoice", render: (row) => cut(row.sage_invoice_id) },
    { label: "Amount", render: (row) => gbp(row.amount_gbp) },
    { label: "Deadline", render: (row) => date(row.zero_rating_deadline_date) },
    { label: "Evidence complete", render: (row) => date(row.export_evidence_complete_date) },
    { label: "Zero-rate", render: (row) => pretty(row.zero_rating_status) },
  ];
  const purchaseCols: Col[] = [
    { label: "Ref", render: (row) => cut(row.supplier_invoice_ref ?? row.supplier_credit_note_ref) },
    { label: "Status", render: (row) => pretty(row.status) },
    { label: "Date", render: (row) => date(row.invoice_date ?? row.credit_note_date) },
  ...
  const journalLineCols: Col[] = [
    { label: "Journal", render: (row) => cut(row.adjustment_journal_id, 18) },
    { label: "Account", render: (row) => cut(row.account_name ?? row.ledger_account_id, 40) },
    { label: "Debit", render: (row) => gbp(row.debit_gbp) },
    { label: "Credit", render: (row) => gbp(row.credit_gbp) },
  ...
  const evidenceMatchCols: Col[] = [
    { label: "Sage ref", render: (row) => cut(row.sage_return_ref) },
    { label: "Box 1", render: (row) => gbp(row.submitted_box1_gbp) },
    { label: "Box 4", render: (row) => gbp(row.submitted_box4_gbp) },
    { label: "Box 6", render: (row) => gbp(row.submitted_box6_gbp) },
    { label: "Box 7", render: (row) => gbp(row.submitted_box7_gbp) },
    { label: "Matched", render: (row) => date(row.matched_at) },
  ];

  const sourceBox6 = lines.rows.filter((row) => s(row.target_box).toLowerCase() === "box6");
  const sourceBox1 = lines.rows.filter((row) => s(row.target_box).toLowerCase() === "box1");
  const sourcePurchases = lines.rows.filter((row) => ["box4", "box7"].includes(s(row.target_box).toLowerCase()));
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-8 text-slate-950"><div className="mx-auto flex max-w-7xl flex-col gap-6">
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><Link href="/internal/accounting-vat" className="text-sm font-semibold text-sky-600">← Back to VAT dashboard</Link><p className="mt-6 text-sm font-medium uppercase tracking-[0.2em] text-sky-500">VAT return pack detail</p><div className="mt-3 flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><h1 className="text-3xl font-semibold tracking-tight">{cut(run.return_period_label || run.run_ref || run.id, 80)}</h1><p className="mt-2 max-w-4xl text-sm leading-6 text-slate-600">Contract route: <code>/internal/accounting-vat/returns/[return_run_id]</code>. This page is the detailed working area for source lines, Box 6 timing, export evidence, purchases, journals and submission evidence.</p></div><div className="rounded-2xl bg-slate-100 px-4 py-3 text-sm text-slate-700"><div className="font-medium text-slate-950">{s((staff as Row).full_name) || "Admin"}</div><div>{s((staff as Row).role_type)}</div></div></div></section>
    ...
    {activeTab === "purchases" ? <div className="grid gap-4"><section className="rounded-3xl border border-slate-200 bg-white p-5 text-sm leading-6 text-slate-700"><h2 className="font-semibold text-slate-950">Box 4 / Box 7 purchase rule</h2><p className="mt-2">Sage should naturally drive supplier AP and supplier credit notes where valid VAT evidence exists. Statement/card spend alone must not create input VAT recovery.</p></section><Table title="Box 4 / Box 7 source lines" data={{ ...lines, rows: sourcePurchases }} columns={sourceCols} /><Table title="Supplier invoices" data={purchaseInvoices} columns={purchaseCols} /><Table title="Supplier credit notes" data={supplierCredits} columns={purchaseCols} /></div> : null}
