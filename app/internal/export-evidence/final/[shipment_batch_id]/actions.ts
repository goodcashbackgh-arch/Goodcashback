...
export async function reviewFinalExportEvidenceDocumentAction(formData: FormData) {
  ...
  if (!documentId) redirect(`/internal/export-evidence/final/${shipmentBatchId}?error=${encodeURIComponent("Missing document.")}`);
  ...
  if (error) {
    redirect(`/internal/export-evidence/final/${shipmentBatchId}?error=${encodeURIComponent(error.message)}`);
  }
  ...
  redirect(`/internal/export-evidence/final/${shipmentBatchId}?success=${encodeURIComponent("Document reviewed")}`);
}
