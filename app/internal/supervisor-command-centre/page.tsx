...
function exceptionLane(row: StatusRow) {
...
function dvaLane(row: StatusRow) {
...
function customerSalesLane(row: StatusRow) {
  ...
  if (state === "posted") {
    ...
    return lane("Posted / marked", parts.join(" · "), "complete", "/internal/accounting-command-centre");
  }
...
function currentAction(row: StatusRow) {
  ...
  return {
    owner: text(row.next_owner) || "None",
    label: text(row.next_action) || "No action required",
    reason: text(row.current_stage_label) || pretty(row.current_stage),
    href: text(row.next_href) || "/internal/supervisor-command-centre",
    tone,
  };
}
...
export default async function SupervisorCommandCentrePage({ searchParams }: { searchParams?: Promise<{ q?: string; only_action?: string }> }) {
  ...
  const sourceRows = ((data ?? []) as StatusRow[]).sort((a, b) => num(a.status_priority) - num(b.status_priority));
  const cards = sourceRows.map((row) => {
    const states = {
      funding: fundingLane(row),
      supplier: supplierLane(row),
      exceptions: exceptionLane(row),
      shipping: shippingLane(row),
      customerSales: customerSalesLane(row),
      shipperAp: shipperApLane(row),
      exportDelivery: exportLane(row),
      dva: dvaLane(row),
    };
    return { row, states, actions: [currentAction(row)], progress: progress(states) };
  }).filter((card) => {
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-950 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1600px] space-y-5">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold"><span className="rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-sky-900">Supervisor owns operational readiness</span><span className="rounded-full border border-violet-200 bg-violet-50 px-3 py-1 text-violet-900">No Sage posting happens here</span><span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-slate-700">Accounting status is read-only handoff</span><Link href="/internal/accounting-command-centre" className="rounded-full border border-violet-200 bg-white px-3 py-1 font-bold text-violet-900 underline">Open Accounting Command Centre — read-only handoff</Link></div>
          {error ? <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><p className="font-bold">Some lanes could not be read</p><p className="mt-1">Canonical status source: {error.message}</p></div> : null}
        </section>
