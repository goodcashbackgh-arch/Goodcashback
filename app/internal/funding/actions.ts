...
export async function reconcileDvaLineToOrderAction(formData: FormData) {
  ...
  const { data, error } = exceedsGap && fxGainConfirmed
  ...
  const message = Number.isFinite(fxGainAmount) && fxGainAmount > 0
    ...
    : Number.isFinite(pendingSurplusAmount) && pendingSurplusAmount > 0
      ? `Reconciled £${appliedAmount} DVA funding to order and retained £${pendingSurplusAmount.toFixed(2)} for later surplus determination.`
