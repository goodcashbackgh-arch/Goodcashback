...
export default async function StaffPhysicalReceiptsPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="min-h-screen space-y-5 bg-slate-50 p-4 md:p-6">
    <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-2 flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-2xl font-semibold text-slate-950">Physical Receipt Reviews</h1><p className="mt-1 text-sm text-slate-600">Initial route decisions before the existing dispute workflow.</p></div><Link href="/internal" className="rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold">Back to internal workspace</Link></div>
    </header>
    {query.error ? <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-rose-800">{query.error}</div> : null}
    <section className="grid gap-3">
      {reviews.length === 0 ? <div className="rounded-2xl border border-slate-200 bg-white p-6 text-slate-600">No physical receipt reviews are visible.</div> : reviews.map((review) => <Link key={review.id} href={`/internal/physical-receipts/${review.id}`} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:border-violet-300">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="font-semibold text-slate-950">{review.order_ref ?? review.id}</div><div className="mt-1 text-sm text-slate-600">{review.retailer_name ?? "Retailer"} · {review.tracking_ref ?? "Tracking"}</div></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${review.status === "awaiting_supervisor_review" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-700"}`}>{review.status.replaceAll("_", " ")}</span></div>
